defmodule Travel.Travel.Integrations.TravelOrder.MarkPaymentSucceededSupplierBookingSubmitBridge do
  ...
  def change(changeset, _opts, context) do
    request_id = resolve_context_path(context, "correlation.request_id") || Map.get(context, :request_id) || "travel_order.mark_payment_succeeded.supplier_booking_submit"
    dedup_key = resolve_dedup_key(changeset, context, request_id)
    payload =
      %{
        "entity" => "travel_order",
        "action" => "mark_payment_succeeded",
        "integration" => "supplier_booking_submit",
        "provider" => @provider,
  ...
  defp resolve_dedup_key(changeset, context, request_id) do
    ...
    "integration:travel_order:mark_payment_succeeded:supplier_booking_submit:" <> key
  end
