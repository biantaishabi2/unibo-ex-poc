defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  # status.keys preview (first ~40): page_title, rows, rows[], rows[].approval_mode, rows[].id, rows[].refund_amount, rows[].refund_fee, rows[].refund_reason, rows[].status, rows_empty
  # Defaults are used for dev/mock transitions (e.g. toggle_list_empty restore).
  @status_defaults_raw Jason.decode!("{
  \"page_title\": \"TravelRefundOrder 列表\",
  \"rows_empty\": true,
  \"rows\": [
    {
      \"refund_reason\": \"\",
  ...
  @backend_api_map %{
    "list" => %{module: __MODULE__.Backend, fun: :handle_event, api: "TravelExt.TravelRefundOrder.list"}
  }
