...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/personal_center");
  if (path.includes(':id')) {
...
test.describe("personal_center", () => {
  test('DTO smoke', async ({ page }) => {
    ...
    await expect(locatorFor(page, `#profile_title`)).toContainText(`个人中心`, { timeout: 15000 });
  });
});
