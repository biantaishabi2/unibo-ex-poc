defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  # status.keys preview (first ~40): rows, rows[], rows[].city_code, rows[].hotel_code, rows[].hotel_name, rows[].hotel_star, rows[].status, rows_empty, travel_hotel, travel_hotel.page, travel_hotel.total_pages
  ...
  @backend_load_selection "city_code: cityCode city_id: cityId hotel_code: hotelCode hotel_name: hotelName hotel_star: hotelStar id status"
  @backend_load_assigns %{}
  @backend_params_accept ["status"]
  @backend_info_reload_messages ["page_host_reload"]
  @backend_api_map %{
    "list" => %{module: __MODULE__.Backend, fun: :handle_event, api: "Travel.TravelHotel.list"}
  }
