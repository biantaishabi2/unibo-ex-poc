defmodule UniboExPocWeb.Pages.Travel.TravelHotelListLive do
  @moduledoc """
  ...
  @page_id "travel_hotel_list"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): rows, rows[], rows[].city_code, rows[].hotel_code, rows[].hotel_name, rows[].hotel_star, rows[].id, rows[].status, rows_empty, travel_hotel, travel_hotel.page, travel_hotel.total_pages
  ...
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelHotel.list"}
  }
  @backend_embedded_page %{page_id: "travel_hotel_list", page_kind: "list", api_map: %{list: "Travel.TravelHotel.list"}, backend: %{load: %{selection: "city_code: cityCode hotel_code: hotelCode hotel_name: hotelName hotel_star: hotelStar id status"}}, route: %{path: "/pages/travel/travel_hotel", query: "", kind: "list"}, state_schema: %{defaults: %{travel_hotel: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", hotel_code: "", hotel_name: "", city_code: "", hotel_star: "", status: ""}, %{id: "row_02", hotel_code: "", hotel_name: "", city_code: "", hotel_star: "", status: ""}, %{id: "row_03", hotel_code: "", hotel_name: "", city_code: "", hotel_star: "", status: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading", "active_status"]}
  ...
  @impl true
  def handle_event("travel_hotel_page_next", params, socket) do
    # UI action event name: travel_hotel_page_next
    socket = dispatch_backend("travel_hotel_page_next", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("travel_hotel_page_prev", params, socket) do
    # UI action event name: travel_hotel_page_prev
    socket = dispatch_backend("travel_hotel_page_prev", params, socket)
    {:noreply, socket}
  end
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_hotel/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_hotel_list">
        ...
        <.table id="data_table">
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_hotel_row">
                ...
                <.table_cell id="td_hotel_star">
                  <%= get_in(row, [:hotel_star]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_hotel_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_hotel, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_hotel, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_hotel_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
