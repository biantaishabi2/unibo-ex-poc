defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  # status.keys preview (first ~40): is_flight_order, is_hotel_order, is_train_order, order, order.flight_offer, order.flight_offer.flight_no, order.hotel_offer, order.hotel_offer.hotel_name, order.id, order.order_no, order.product_type, order.total_amount, order.train_offer, order.train_offer.train_no, order.traveler_count, order_booking_success, order_status_failed
  ...
  @impl true
  def handle_event("navigate:/travel/orders/{{order.id|}}", params, socket) do
    # UI action event name: navigate:/travel/orders/{{order.id|}}
    socket = dispatch_backend("navigate:/travel/orders/{{order.id|}}", params, socket)
    {:noreply, socket}
  end
