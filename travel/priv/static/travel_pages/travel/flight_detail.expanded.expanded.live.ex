defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  # status.keys preview (first ~40): @flight_offer, @flight_offer.baggage_policy, @flight_offer.cabin_class, @flight_offer.currency, @flight_offer.fare_family, @flight_offer.flight_no, @flight_offer.flight_offer_lifecycle, @flight_offer.listed_price, @flight_offer.refund_change_policy, @flight_offer.seats_available, @flight_offer.settlement_price, @travel_order, @travel_order.booking_mode, @travel_order.change_status, @travel_order.contact_name, @travel_order.contact_phone, @travel_order.currency, @travel_order.host_enterprise_id, @travel_order.host_member_id, @travel_order.host_shop_id, @travel_order.order_no, @travel_order.original_order_ref, @travel_order.payment_external_ref, @travel_order.points_deduction_amount, @travel_order.points_to_use, @travel_order.product_type, @travel_order.recommended_payment_method, @travel_order.seat_selection_snapshot, @travel_order.status, @travel_order.supplier_order_ref, @travel_order.tenant_id, @travel_order.ticket_passenger_infos, @travel_order.total_amount, @travel_order.traveler_count, @travel_order.waitlist_status, flight_offer, flight_offer.arrival_airport_code, flight_offer.arrival_at, flight_offer.departure_airport_code, flight_offer.departure_at, ...
  ...
  @impl true
  def handle_event("flight_offer_activate", params, socket) do
    # UI action event name: flight_offer_activate
    socket = dispatch_backend("flight_offer_activate", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("flight_offer_create", params, socket) do
    # UI action event name: flight_offer_create
    socket = dispatch_backend("flight_offer_create", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("flight_offer_deactivate", params, socket) do
    # UI action event name: flight_offer_deactivate
    socket = dispatch_backend("flight_offer_deactivate", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("flight_offer_destroy", params, socket) do
    # UI action event name: flight_offer_destroy
    socket = dispatch_backend("flight_offer_destroy", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("flight_offer_expire", params, socket) do
    # UI action event name: flight_offer_expire
    socket = dispatch_backend("flight_offer_expire", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("flight_offer_update", params, socket) do
    # UI action event name: flight_offer_update
    socket = dispatch_backend("flight_offer_update", params, socket)
    {:noreply, socket}
  end
