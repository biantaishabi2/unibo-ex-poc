...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/refund_apply");
  if (path.includes(':id')) {
...
test.describe("refund_apply", () => {
  test('DTO smoke', async ({ page }) => {
    ...
    await expect(locatorFor(page, `#basic_info_title`)).toContainText(`基本信息`, { timeout: 15000 });
  });
});
