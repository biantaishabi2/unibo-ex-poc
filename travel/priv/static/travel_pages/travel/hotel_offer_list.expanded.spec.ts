...
const __WAIT_CONTRACT = {
  ...
  "navigate_create": {
    ...
    "until": {
      ...
      "state_key": null,
      "url_contains": "/pages/travel/hotel_offer/new"
    },
    "url_contains": "/pages/travel/hotel_offer/new"
  },
  "refetch": {
    ...
    "ui": [
      {
        "assert": "url_contains",
        "selector": null,
        "value": "/pages/travel/hotel_offer/new"
      },
      {
        "assert": "visible",
        "selector": "#hotel_offer_detail",
...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/hotel_offer");
  if (path.includes(':id')) {
...
test.describe("hotel_offer_list", () => {
  ...
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await test.step("页面加载 + 结构可见", async () => {
        ...
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      await expect(page.locator(`#hotel_offer_list`)).toBeVisible({ timeout: 15000 });
      ...
      await test.step("表格区域可见", async () => {
        ...
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      await expect(page.locator(`#hotel_offer_table, #table_section, table`).first()).toBeVisible({ timeout: 15000 });
      ...
      {
        const expectedUrl = resolveTemplateString(__ctx, "/pages/travel/hotel_offer/new");
      try {
