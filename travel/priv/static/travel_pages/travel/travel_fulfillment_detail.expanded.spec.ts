...
const __DATA_CONTRACT = {
  ...
  "setup": [
    ...
    },
    {
      ...
      "freshness": "match_only",
      "graphql_field": "listTravelTravelFulfillments",
...
const __WAIT_CONTRACT = {
  "action": {
    ...
    "selectors": [
      "#travel_fulfillment_detail"
    ],
    "state_keys": [],
    "timeout": 8000,
    "until": {
      "kind": "visible",
      "selector": "#travel_fulfillment_detail",
      "state_key": null,
      "url_contains": null
    },
    "url_contains": null
  ...
  "destroy": {
    ...
    "selectors": [
      "#travel_fulfillment_list"
    ],
    "state_keys": [],
    "timeout": 8000,
    "until": {
      "kind": "url_contains",
      "selector": "#travel_fulfillment_list",
      "state_key": null,
      "url_contains": "/pages/travel/travel_fulfillment"
    },
    "url_contains": "/pages/travel/travel_fulfillment"
  },
  "form_submit": {
    ...
    "selectors": [
      "#travel_fulfillment_edit_form, #main_form, form[phx-submit=\"form_submit\"]",
      "#travel_fulfillment_detail"
    ],
    "state_keys": [],
    "timeout": 8000,
    "until": {
      "kind": "form_settled",
      "selector": "#travel_fulfillment_edit_form, #main_form, form[phx-submit=\"form_submit\"]",
      "state_key": null,
      "url_contains": null
    },
    "url_contains": null
...
const __VERIFICATION_CONTRACT = {
  "create": {
    "backend": {
      ...
      "equals": "$form.supplier_booking_ref",
      "excludes_source": null,
      "graphql_field": "getTravelTravelFulfillment",
      "op": "field_equals",
      "path": "supplier_booking_ref",
      "source": "created_record_id"
    },
    ...
    "ui": [
      {
        "assert": "url_contains",
        "selector": null,
        "value": "/pages/travel/travel_fulfillment"
      },
      {
        "assert": "visible",
        "selector": "#travel_fulfillment_list",
...
const __BACKEND_API_MAP = {
  "list": "Travel.TravelFulfillment.list",
...
const __BACKEND_SELECTION = "id tenant_id: tenantId fulfillment_type: fulfillmentType status supplier_booking_ref: supplierBookingRef voucher_or_ticket_ref: voucherOrTicketRef ticket_refs: ticketRefs waitlist_result: waitlistResult change_result: changeResult boarding_status: boardingStatus confirmation_payload: confirmationPayload failure_reason: failureReason used_at: usedAt inserted_at: insertedAt updated_at: updatedAt";
...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/travel_fulfillment/:id");
  if (path.includes(':id')) {
...
test.describe("travel_fulfillment_detail", () => {
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator(`#travel_fulfillment_detail`)).toBeVisible({ timeout: 15000 });
      ...
      await expect(page.locator(`#travel_fulfillment_edit_form, #main_form, form[phx-submit="form_submit"]`).first()).not.toBeVisible({ timeout: 15000 });
      ...
      }
      {
        ...
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const resolvedValue = resolveTemplateString(__ctx, "UPDATED_{{__run_id}}_failure_reason");
        await loc.fill(resolvedValue, { timeout: 15000 });
        __ctx.form["failure_reason"] = resolvedValue; refreshDataBindings(__ctx);
      }
      {
        const loc = page.locator(`#travel_fulfillment_form_voucher_or_ticket_ref, [name='voucher_or_ticket_ref']`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const resolvedValue = resolveTemplateString(__ctx, "UPDATED_{{__run_id}}_voucher_or_ticket_ref");
        await loc.fill(resolvedValue, { timeout: 15000 });
      ...
      {
        const loc = page.locator(`#travel_fulfillment_edit_form button[type="submit"], #travel_fulfillment_edit_form [phx-click="form_submit"]`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
      ...
      await test.step("动作：confirm_booking", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_fulfillment_action_confirm_booking`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        if (clickedId) {
          __ctx.clicked_row = { ...( __ctx.clicked_row || {}), id: clickedId };
          __ctx["state_action_record_id_confirm_booking"] = clickedId;
          refreshDataBindings(__ctx);
        }
        if (!clickedId) {
          __ctx["state_action_record_id_confirm_booking"] = defaultRecordId(__ctx);
        }
        await waitForLiveViewReady(page, 15000);
        await syncRouteContext(page, __ctx);
      }
        await runCaseWait(page, __ctx, "action_confirm_booking", ["action_confirm_booking"]);
        await captureCreatedRecordId(__ctx, "custom", ["action_confirm_booking"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_confirm_booking"]);
      });
      await test.step("动作：issue_voucher_or_ticket", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_fulfillment_action_issue_voucher_or_ticket`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        if (clickedId) {
          __ctx.clicked_row = { ...( __ctx.clicked_row || {}), id: clickedId };
          __ctx["state_action_record_id_issue_voucher_or_ticket"] = clickedId;
          refreshDataBindings(__ctx);
        }
        if (!clickedId) {
          __ctx["state_action_record_id_issue_voucher_or_ticket"] = defaultRecordId(__ctx);
        }
        await waitForLiveViewReady(page, 15000);
        await syncRouteContext(page, __ctx);
      }
        await runCaseWait(page, __ctx, "action_issue_voucher_or_ticket", ["action_issue_voucher_or_ticket"]);
        await captureCreatedRecordId(__ctx, "custom", ["action_issue_voucher_or_ticket"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_issue_voucher_or_ticket"]);
      });
      await test.step("动作：mark_in_use", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_fulfillment_action_mark_in_use`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        if (clickedId) {
          __ctx.clicked_row = { ...( __ctx.clicked_row || {}), id: clickedId };
          __ctx["state_action_record_id_mark_in_use"] = clickedId;
          refreshDataBindings(__ctx);
        }
        if (!clickedId) {
          __ctx["state_action_record_id_mark_in_use"] = defaultRecordId(__ctx);
        }
        await waitForLiveViewReady(page, 15000);
        await syncRouteContext(page, __ctx);
      }
        await runCaseWait(page, __ctx, "action_mark_in_use", ["action_mark_in_use"]);
        await captureCreatedRecordId(__ctx, "custom", ["action_mark_in_use"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_mark_in_use"]);
      });
      await test.step("动作：complete_fulfillment", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_fulfillment_action_complete_fulfillment`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        if (clickedId) {
          __ctx.clicked_row = { ...( __ctx.clicked_row || {}), id: clickedId };
          __ctx["state_action_record_id_complete_fulfillment"] = clickedId;
          refreshDataBindings(__ctx);
        }
        if (!clickedId) {
          __ctx["state_action_record_id_complete_fulfillment"] = defaultRecordId(__ctx);
        }
        await waitForLiveViewReady(page, 15000);
        await syncRouteContext(page, __ctx);
      }
        await runCaseWait(page, __ctx, "action_complete_fulfillment", ["action_complete_fulfillment"]);
        await captureCreatedRecordId(__ctx, "custom", ["action_complete_fulfillment"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_complete_fulfillment"]);
      });
      await test.step("动作：cancel_fulfillment", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_fulfillment_action_cancel_fulfillment`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        if (clickedId) {
          __ctx.clicked_row = { ...( __ctx.clicked_row || {}), id: clickedId };
          __ctx["state_action_record_id_cancel_fulfillment"] = clickedId;
          refreshDataBindings(__ctx);
        }
        if (!clickedId) {
          __ctx["state_action_record_id_cancel_fulfillment"] = defaultRecordId(__ctx);
        }
        await waitForLiveViewReady(page, 15000);
        await syncRouteContext(page, __ctx);
      }
        await runCaseWait(page, __ctx, "action_cancel_fulfillment", ["action_cancel_fulfillment"]);
        await captureCreatedRecordId(__ctx, "custom", ["action_cancel_fulfillment"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_cancel_fulfillment"]);
      });
      await test.step("动作：fail_fulfillment", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_fulfillment_action_fail_fulfillment`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        if (clickedId) {
          __ctx.clicked_row = { ...( __ctx.clicked_row || {}), id: clickedId };
          __ctx["state_action_record_id_fail_fulfillment"] = clickedId;
          refreshDataBindings(__ctx);
        }
        if (!clickedId) {
          __ctx["state_action_record_id_fail_fulfillment"] = defaultRecordId(__ctx);
        }
        await waitForLiveViewReady(page, 15000);
        await syncRouteContext(page, __ctx);
      }
        await runCaseWait(page, __ctx, "action_fail_fulfillment", ["action_fail_fulfillment"]);
        await captureCreatedRecordId(__ctx, "custom", ["action_fail_fulfillment"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_fail_fulfillment"]);
      });
      ...
      {
        const expectedUrl = resolveTemplateString(__ctx, "/pages/travel/travel_fulfillment");
      try {
