defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  # status.keys preview (first ~40): page_title, rows, rows[], rows[].cabin_class_code, rows[].cabin_class_name, rows[].cabin_rank, rows[].id, rows[].status, rows_empty
  # Defaults are used for dev/mock transitions (e.g. toggle_list_empty restore).
  @status_defaults_raw Jason.decode!("{
  \"page_title\": \"TravelCabinClass 列表\",
  \"rows_empty\": true,
  \"rows\": [
    {
      \"cabin_class_code\": \"\",
  ...
  @backend_api_map %{
    "list" => %{module: __MODULE__.Backend, fun: :handle_event, api: "Travel.TravelCabinClass.list"}
  }
