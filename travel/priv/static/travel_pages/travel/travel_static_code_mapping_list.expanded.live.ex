defmodule UniboExPocWeb.Pages.Travel.TravelStaticCodeMappingListLive do
  @moduledoc """
  ...
  @page_id "travel_static_code_mapping_list"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): rows, rows[], rows[].canonical_entity, rows[].external_code, rows[].external_name, rows[].id, rows[].object_type, rows[].status, rows[].supplier_code, rows_empty, travel_static_code_mapping, travel_static_code_mapping.page, travel_static_code_mapping.total_pages
  ...
  @backend_params_accept ["object_type", "status"]
  @backend_info_reload_messages []
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelStaticCodeMapping.list"}
  }
  @backend_embedded_page %{page_id: "travel_static_code_mapping_list", page_kind: "list", api_map: %{list: "Travel.TravelStaticCodeMapping.list"}, backend: %{load: %{selection: "canonical_entity: canonicalEntity canonical_id: canonicalId external_code: externalCode external_name: externalName id object_type: objectType status supplier_code: supplierCode"}}, route: %{path: "/pages/travel/travel_static_code_mapping", query: "", kind: "list"}, state_schema: %{defaults: %{travel_static_code_mapping: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", supplier_code: "", object_type: "", canonical_entity: "", external_code: "", external_name: "", status: ""}, %{id: "row_02", supplier_code: "", object_type: "", canonical_entity: "", external_code: "", external_name: "", status: ""}, %{id: "row_03", supplier_code: "", object_type: "", canonical_entity: "", external_code: "", external_name: "", status: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_static_code_mapping/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_static_code_mapping_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              ...
              <.table_head id="th_object_type">
                主数据对象类型
              </.table_head>
              <.table_head id="th_canonical_entity">
                Travel 实体名
              </.table_head>
              <.table_head id="th_external_code">
                供应商侧编码
              </.table_head>
              <.table_head id="th_external_name">
                供应商侧名称
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_static_code_mapping_row">
                <.table_cell id="td_supplier_code">
                  <%= get_in(row, [:supplier_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_object_type">
                  <%= if (val = get_in(row, [:object_type])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_object_type">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                <.table_cell id="td_canonical_entity">
                  <%= get_in(row, [:canonical_entity]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_external_code">
                  <%= get_in(row, [:external_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_external_name">
                  <%= get_in(row, [:external_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_static_code_mapping_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_static_code_mapping, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_static_code_mapping, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_static_code_mapping_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
