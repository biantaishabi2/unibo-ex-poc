defmodule UniboExPocWeb.Pages.Travel.TravelCabinClassDetailLive do
  @moduledoc """
  ...
  @backend_params_accept ["id", "cabin_rank", "cabin_class_name", "status", "cabin_class_code"]
  ...
  @backend_api_map %{
    ...
    "get" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelCabinClass.get"},
    "update" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelCabinClass.update"}
  }
  @backend_embedded_page %{page_id: "travel_cabin_class_detail", page_kind: "detail", api_map: %{create: "Travel.TravelCabinClass.create", get: "Travel.TravelCabinClass.get", update: "Travel.TravelCabinClass.update"}, backend: %{load: %{selection: "cabin_class_code: cabinClassCode cabin_class_name: cabinClassName cabin_rank: cabinRank id status"}}, route: %{path: "/pages/travel/travel_cabin_class/:id", query: "", kind: "detail"}, state_schema: %{defaults: %{travel_cabin_class: %{cabin_class_code: "", cabin_class_name: "", cabin_rank: "", status: ""}, editing: false}}, status_keys: ["record", "editing", "form", "loading"]}
    ...
    if has_id do
      ...
      id =
        Map.get(params, "id") ||
          ...
          get_in(socket.assigns, [:record, "id"]) ||
          get_in(socket.assigns, [:travel_cabin_class, :id]) ||
          get_in(socket.assigns, [:travel_cabin_class, "id"])
      case id do
  ...
  defp __sync_entity_assign(socket, dto) when is_map(dto) do
    case dto do
      %{travel_cabin_class: entity} when is_map(entity) and map_size(entity) > 0 -> assign(socket, :travel_cabin_class, entity)
      %{record: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
          true -> assign(socket, :travel_cabin_class, entity)
          false -> socket
        end
      _ ->
        source =
        ...
        if map_size(source) == 0 do
          ...
          entity = Map.get(socket.assigns, :travel_cabin_class, %{})
          entity = if is_map(entity), do: Map.merge(entity, source), else: source
          assign(socket, :travel_cabin_class, entity)
        end
    end
  end
  ...
  defp __sync_record_alias(socket) do
    case socket.assigns do
      %{travel_cabin_class: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
  ...
  def render(assigns) do
    ~H"""
    <.page title="详情" id="travel_cabin_class_detail">
        ...
        <.flex gap={3} align="center" direction="row" id="title_group">
          <.stack direction="column" id="title_meta">
            ...
            <.text variant="h1" id="page_title">
              舱位主数据（Travel 层,来源 OFBiz Enumeration）
            </.text>
          </.stack>
          <%= case get_in(@travel_cabin_class, [:status]) do %>
            <% "active" -> %>
            ...
            <%= if !(@editing && @editing != []) do %>
              <.grid columns={2} gap={4} id="info_grid">
                <.grid columns={2} gap={3} id="detail_fields">
                  <.stack gap={0} direction="column" id="detail_fields_cabin_class_code">
                    <.text variant="caption" color="muted" id="detail_fields_cabin_class_code_label">
                      舱位规范编码
                    </.text>
                    <.text id="detail_fields_cabin_class_code_value">
                      <%= get_in(@travel_cabin_class, [:cabin_class_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_cabin_class_name">
                    <.text variant="caption" color="muted" id="detail_fields_cabin_class_name_label">
                      舱位名称
                    </.text>
                    <.text id="detail_fields_cabin_class_name_value">
                      <%= get_in(@travel_cabin_class, [:cabin_class_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_cabin_rank">
                    <.text variant="caption" color="muted" id="detail_fields_cabin_rank_label">
                      舱位等级序
                    </.text>
                    <.text id="detail_fields_cabin_rank_value">
                      <%= get_in(@travel_cabin_class, [:cabin_rank]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_status">
                    ...
                    <.text id="detail_fields_status_value">
                      <%= get_in(@travel_cabin_class, [:status]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                </.grid>
              </.grid>
            <% else %>
              <.form for={%{}} phx-submit="form_submit" phx-change="form_change" id="travel_cabin_class_edit_form">
                <.grid columns={2} gap={4} id="form_grid">
                  <.stack direction="column" id="form_fields">
                    <.input size="md" name="travel_cabin_class[cabin_class_code]" id="travel_cabin_class_form_cabin_class_code" value={get_in(@travel_cabin_class, [:cabin_class_code]) || ""} />
                    <.input size="md" name="travel_cabin_class[cabin_class_name]" id="travel_cabin_class_form_cabin_class_name" value={get_in(@travel_cabin_class, [:cabin_class_name]) || ""} />
                    <.input size="md" name="travel_cabin_class[cabin_rank]" id="travel_cabin_class_form_cabin_rank" value={get_in(@travel_cabin_class, [:cabin_rank]) || ""} />
                    <.select size="md" name="travel_cabin_class[status]" id="travel_cabin_class_form_status" :let={builder} value={get_in(@travel_cabin_class, [:status]) || ""}>
