...
const __WAIT_CONTRACT = {
  ...
  "navigate_create": {
    ...
    "until": {
      ...
      "state_key": null,
      "url_contains": "/pages/travel/travel_change_order/new"
    },
    "url_contains": "/pages/travel/travel_change_order/new"
  },
  "refetch": {
    ...
    "ui": [
      {
        "assert": "url_contains",
        "selector": null,
        "value": "/pages/travel/travel_change_order/new"
      },
      {
        "assert": "visible",
        "selector": "#travel_change_order_detail",
    ...
    "ui": [
      {
        "assert": "url_contains",
        "selector": null,
        "value": "/pages/travel/travel_change_order/"
      },
      {
        "assert": "visible",
        "selector": "#travel_change_order_detail",
...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/travel_change_order");
  if (path.includes(':id')) {
...
test.describe("travel_change_order_list", () => {
  ...
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await test.step("页面加载 + 结构可见", async () => {
        ...
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      await expect(page.locator(`#travel_change_order_list`)).toBeVisible({ timeout: 15000 });
      ...
      await test.step("表格区域可见", async () => {
        ...
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      await expect(page.locator(`#travel_change_order_table, #table_section, table`).first()).toBeVisible({ timeout: 15000 });
      ...
      {
        const expectedUrl = resolveTemplateString(__ctx, "/pages/travel/travel_change_order/new");
      try {
