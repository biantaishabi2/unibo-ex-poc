...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/home");
  if (path.includes(':id')) {
...
test.describe("home", () => {
  test('DTO smoke', async ({ page }) => {
    ...
    await expect(locatorFor(page, `#hero_title`)).toContainText(`欢迎使用`, { timeout: 15000 });
    await expect(locatorFor(page, `#hero_subtitle`)).toContainText(`便捷管理,高效协作`, { timeout: 15000 });
    await expect(locatorFor(page, `#hero_action`)).toContainText(`立即开始`, { timeout: 15000 });
    await expect(locatorFor(page, `#shortcut_1_label`)).toContainText(`功能一`, { timeout: 15000 });
    await expect(locatorFor(page, `#shortcut_2_label`)).toContainText(`功能二`, { timeout: 15000 });
    await expect(locatorFor(page, `#shortcut_3_label`)).toContainText(`功能三`, { timeout: 15000 });
    await expect(locatorFor(page, `#shortcut_4_label`)).toContainText(`功能四`, { timeout: 15000 });
    await expect(locatorFor(page, `#pending_value`)).toContainText(`0`, { timeout: 15000 });
    await expect(locatorFor(page, `#message_value`)).toContainText(`0`, { timeout: 15000 });
    await expect(locatorFor(page, `#schedule_value`)).toContainText(`0`, { timeout: 15000 });
  });
});
