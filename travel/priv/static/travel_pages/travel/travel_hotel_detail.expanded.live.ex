defmodule UniboExPocWeb.Pages.Travel.TravelHotelDetailLive do
  @moduledoc """
  ...
  @page_id "travel_hotel_detail"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): editing, travel_hotel, travel_hotel.city_code, travel_hotel.hotel_code, travel_hotel.hotel_name, travel_hotel.hotel_star, travel_hotel.status
  ...
  @backend_params_accept ["id", "hotel_name", "city_id", "city_code", "hotel_star", "status", "hotel_code"]
  @backend_info_reload_messages []
  @backend_api_map %{
    "create" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelHotel.create"},
    "get" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelHotel.get"},
    "update" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelHotel.update"}
  }
  @graphql_field_map %{"create" => "create_travel_travel_hotel", "get" => "get_travel_travel_hotel", "update" => "update_travel_travel_hotel"}
  @input_allowlist %{"create" => ~w(city_code city_id hotel_code hotel_name hotel_star status), "update" => ~w(city_code city_id hotel_name hotel_star status)}
  @input_type_name_map %{"create" => "CreateTravelTravelHotelInput", "update" => "UpdateTravelTravelHotelInput"}
  @input_type_map %{"create" => %{"city_code" => "string", "city_id" => "string", "hotel_code" => "string", "hotel_name" => "string", "hotel_star" => "string", "status" => "enum"}, "update" => %{"city_code" => "string", "city_id" => "string", "hotel_name" => "string", "hotel_star" => "string", "status" => "enum"}}
  @entity_assign_fields ["hotel_code", "hotel_name", "city_code", "hotel_star", "status"]
    ...
    case normalized do
      "form_submit" ->
        record_id = get_in(socket.assigns, [:record, :id]) || get_in(socket.assigns, [:record, "id"]) ||
          get_in(socket.assigns, [:travel_hotel, :id]) || get_in(socket.assigns, [:travel_hotel, "id"])
  ...
  defp __resolve_compiled_id(params, socket) do
    Map.get(params, "id") ||
      ...
      get_in(socket.assigns, [:record, "id"]) ||
      get_in(socket.assigns, [:travel_hotel, :id]) ||
      get_in(socket.assigns, [:travel_hotel, "id"]) ||
      (socket.assigns[:record] && socket.assigns[:record]["id"]) ||
      (socket.assigns[:travel_hotel] && socket.assigns[:travel_hotel]["id"]) || ""
  end
  ...
  defp __sync_entity_assign(socket, dto) when is_map(dto) do
    case dto do
      %{travel_hotel: entity} when is_map(entity) and map_size(entity) > 0 -> assign(socket, :travel_hotel, entity)
      %{record: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
          true -> assign(socket, :travel_hotel, entity)
          false -> socket
        end
      _ ->
        source =
        ...
        if map_size(source) == 0 do
          ...
          entity = Map.get(socket.assigns, :travel_hotel, %{})
          entity = if is_map(entity), do: Map.merge(entity, source), else: source
          assign(socket, :travel_hotel, entity)
        end
    end
  end
  ...
  defp __sync_record_alias(socket) do
    case socket.assigns do
      %{travel_hotel: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
  ...
  assigns = assigns
  effects = []
  effects = [%{type: "navigate", to: "/pages/travel/travel_hotel"} | effects]
  ...
  def render(assigns) do
    ~H"""
    <.page title="详情" id="travel_hotel_detail">
        ...
        <.flex gap={3} align="center" direction="row" id="title_group">
          <.stack direction="column" id="title_meta">
            ...
            <.text variant="h1" id="page_title">
              酒店主数据（Travel 层,来源 OFBiz Product）
            </.text>
          </.stack>
          <%= case get_in(@travel_hotel, [:status]) do %>
            <% "active" -> %>
                ...
                <.grid columns={2} gap={3} id="detail_fields">
                  <.stack gap={0} direction="column" id="detail_fields_hotel_code">
                    ...
                    <.text id="detail_fields_hotel_code_value">
                      <%= get_in(@travel_hotel, [:hotel_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_hotel_name">
                    ...
                    <.text id="detail_fields_hotel_name_value">
                      <%= get_in(@travel_hotel, [:hotel_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_city_code">
                    ...
                    <.text id="detail_fields_city_code_value">
                      <%= get_in(@travel_hotel, [:city_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_hotel_star">
                    <.text variant="caption" color="muted" id="detail_fields_hotel_star_label">
                      酒店星级
                    </.text>
                    <.text id="detail_fields_hotel_star_value">
                      <%= get_in(@travel_hotel, [:hotel_star]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_status">
                    ...
                    <.text id="detail_fields_status_value">
                      <%= get_in(@travel_hotel, [:status]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                </.grid>
              </.grid>
            <% else %>
              <.form for={%{}} phx-submit="form_submit" phx-change="form_change" id="travel_hotel_edit_form">
                <.grid columns={2} gap={4} id="form_grid">
                  <.stack direction="column" id="form_fields">
                    <.input size="md" name="travel_hotel[hotel_code]" id="travel_hotel_form_hotel_code" value={get_in(@travel_hotel, [:hotel_code]) || ""} />
                    <.input size="md" name="travel_hotel[hotel_name]" id="travel_hotel_form_hotel_name" value={get_in(@travel_hotel, [:hotel_name]) || ""} />
                    <.input size="md" name="travel_hotel[city_id]" id="travel_hotel_form_city_id" value={get_in(@travel_hotel, [:city_id]) || ""} />
                    <.input size="md" name="travel_hotel[city_code]" id="travel_hotel_form_city_code" value={get_in(@travel_hotel, [:city_code]) || ""} />
                    <.input size="md" name="travel_hotel[hotel_star]" id="travel_hotel_form_hotel_star" value={get_in(@travel_hotel, [:hotel_star]) || ""} />
                    <.select size="md" name="travel_hotel[status]" id="travel_hotel_form_status" :let={builder} value={get_in(@travel_hotel, [:status]) || ""}>
