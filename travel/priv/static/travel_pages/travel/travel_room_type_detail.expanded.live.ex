defmodule UniboExPocWeb.Pages.Travel.TravelRoomTypeDetailLive do
  @moduledoc """
  ...
  @page_id "travel_room_type_detail"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): editing, travel_room_type, travel_room_type.bed_type, travel_room_type.hotel_code, travel_room_type.room_type_code, travel_room_type.room_type_name, travel_room_type.status
  ...
  @backend_params_accept ["id", "bed_type", "status", "room_type_code", "room_type_name", "hotel_code"]
  ...
  @backend_api_map %{
    ...
    "get" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRoomType.get"},
    "update" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRoomType.update"}
  }
  @backend_embedded_page %{page_id: "travel_room_type_detail", page_kind: "detail", api_map: %{create: "Travel.TravelRoomType.create", get: "Travel.TravelRoomType.get", update: "Travel.TravelRoomType.update"}, backend: %{load: %{selection: "bed_type: bedType hotel_code: hotelCode id room_type_code: roomTypeCode room_type_name: roomTypeName status"}}, route: %{path: "/pages/travel/travel_room_type/:id", query: "", kind: "detail"}, state_schema: %{defaults: %{travel_room_type: %{room_type_code: "", room_type_name: "", hotel_code: "", bed_type: "", status: ""}, editing: false}}, status_keys: ["record", "editing", "form", "loading"]}
    ...
    if has_id do
      ...
      id =
        Map.get(params, "id") ||
          ...
          get_in(socket.assigns, [:record, "id"]) ||
          get_in(socket.assigns, [:travel_room_type, :id]) ||
          get_in(socket.assigns, [:travel_room_type, "id"])
      case id do
  ...
  defp __sync_entity_assign(socket, dto) when is_map(dto) do
    case dto do
      %{travel_room_type: entity} when is_map(entity) and map_size(entity) > 0 -> assign(socket, :travel_room_type, entity)
      %{record: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
          true -> assign(socket, :travel_room_type, entity)
          false -> socket
        end
      _ ->
        source =
        ...
        if map_size(source) == 0 do
          ...
          entity = Map.get(socket.assigns, :travel_room_type, %{})
          entity = if is_map(entity), do: Map.merge(entity, source), else: source
          assign(socket, :travel_room_type, entity)
        end
    end
  end
  ...
  defp __sync_record_alias(socket) do
    case socket.assigns do
      %{travel_room_type: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
  ...
  assigns = assigns
  effects = []
  effects = [%{type: "navigate", to: "/pages/travel/travel_room_type"} | effects]
  ...
  def render(assigns) do
    ~H"""
    <.page title="详情" id="travel_room_type_detail">
        ...
        <.flex gap={3} align="center" direction="row" id="title_group">
          <.stack direction="column" id="title_meta">
            ...
            <.text variant="h1" id="page_title">
              酒店房型主数据（Travel 层,来源 OFBiz ProductFeature）
            </.text>
          </.stack>
          <%= case get_in(@travel_room_type, [:status]) do %>
            <% "active" -> %>
                ...
                <.grid columns={2} gap={3} id="detail_fields">
                  <.stack gap={0} direction="column" id="detail_fields_room_type_code">
                    ...
                    <.text id="detail_fields_room_type_code_value">
                      <%= get_in(@travel_room_type, [:room_type_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_room_type_name">
                    <.text variant="caption" color="muted" id="detail_fields_room_type_name_label">
                      房型名称
                    </.text>
                    <.text id="detail_fields_room_type_name_value">
                      <%= get_in(@travel_room_type, [:room_type_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_hotel_code">
                    ...
                    <.text id="detail_fields_hotel_code_value">
                      <%= get_in(@travel_room_type, [:hotel_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_bed_type">
                    <.text variant="caption" color="muted" id="detail_fields_bed_type_label">
                      床型
                    </.text>
                    <.text id="detail_fields_bed_type_value">
                      <%= get_in(@travel_room_type, [:bed_type]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_status">
                    ...
                    <.text id="detail_fields_status_value">
                      <%= get_in(@travel_room_type, [:status]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                </.grid>
              </.grid>
            <% else %>
              <.form for={%{}} phx-submit="form_submit" phx-change="form_change" id="travel_room_type_edit_form">
                <.grid columns={2} gap={4} id="form_grid">
                  <.stack direction="column" id="form_fields">
                    <.input size="md" name="travel_room_type[room_type_code]" id="travel_room_type_form_room_type_code" value={get_in(@travel_room_type, [:room_type_code]) || ""} />
                    <.input size="md" name="travel_room_type[room_type_name]" id="travel_room_type_form_room_type_name" value={get_in(@travel_room_type, [:room_type_name]) || ""} />
                    <.input size="md" name="travel_room_type[hotel_code]" id="travel_room_type_form_hotel_code" value={get_in(@travel_room_type, [:hotel_code]) || ""} />
                    <.input size="md" name="travel_room_type[bed_type]" id="travel_room_type_form_bed_type" value={get_in(@travel_room_type, [:bed_type]) || ""} />
                    <.select size="md" name="travel_room_type[status]" id="travel_room_type_form_status" :let={builder} value={get_in(@travel_room_type, [:status]) || ""}>
