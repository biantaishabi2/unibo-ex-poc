...
const __VERIFICATION_CONTRACT = {
  ...
  "update": {
    "backend": {
      ...
      "binds": [],
      "equals": "$form.personal_pay_amount",
      "excludes_source": null,
      "graphql_field": "getTravelTravelPolicyCheck",
      "op": "equals",
      "path": "personal_pay_amount",
      "source": "active_record_id"
    },
    "ui": [
      {
...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/travel_policy_check/:id");
  if (path.includes(':id')) {
...
test.describe("travel_policy_check_detail", () => {
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator(`#travel_policy_check_detail`)).toBeVisible({ timeout: 15000 });
      ...
      await expect(page.locator(`#travel_policy_check_edit_form, #main_form, form[phx-submit="form_submit"]`).first()).not.toBeVisible({ timeout: 15000 });
      ...
      }
      {
        ...
        const resolvedValue = resolveTemplateString(__ctx, "exceeded");
