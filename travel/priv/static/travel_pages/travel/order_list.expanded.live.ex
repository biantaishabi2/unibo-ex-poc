defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  @impl true
  def handle_event("navigate:/travel/orders/{{item.id|}}", params, socket) do
    # UI action event name: navigate:/travel/orders/{{item.id|}}
    socket = dispatch_backend("navigate:/travel/orders/{{item.id|}}", params, socket)
    {:noreply, socket}
  end
