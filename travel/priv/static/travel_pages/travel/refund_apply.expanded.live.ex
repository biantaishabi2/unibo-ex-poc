defmodule UniboExPocWeb.Pages.Travel.RefundApplyLive do
  @moduledoc """
  ...
  @graphql_field_map %{"create" => "create_travel_travel_refund_order", "list" => "list_travel_travel_refund_orders"}
  @input_allowlist %{"create" => ~w(approval_mode original_order_id refund_amount refund_fee refund_reason)}
  @input_type_name_map %{"create" => "CreateTravelTravelRefundOrderInput"}
  @input_type_map %{"create" => %{"approval_mode" => "enum", "original_order_id" => "string", "refund_amount" => "string", "refund_fee" => "string", "refund_reason" => "string"}}
    ...
    <.page title="表单" id="refund_apply">
      ...
      <.section layout="none" id="header">
        <.flex justify="between" align="center" direction="row" id="header_bar">
          ...
          <.flex gap={2} direction="row" id="header_actions">
            <.button variant="ghost" phx-click="form_cancel" size="md" id="cancel_btn">
              取消
            </.button>
            <.button variant="secondary" phx-click="save_draft" size="md" id="save_draft_btn">
              保存草稿
            </.button>
            <.button variant="primary" phx-click="form_submit" size="md" id="submit_btn">
              提交
            </.button>
          </.flex>
        </.flex>
      </.section>
      <.section layout="none" id="form_section">
        <.form for={%{}} phx-submit="form_submit" phx-change="form_change" id="main_form">
          <.card variant="default" id="basic_info_card">
            <.card_header id="basic_info_header">
              <.card_title id="basic_info_title">
                <%= @section_1_title %>
              </.card_title>
            </.card_header>
            <.card_content id="basic_info_content">
              <.stack direction="column" id="basic_info_grid">
                <.input size="md" name="travel_refund_order[refund_reason]" id="form_refund_reason" />
                <.input size="md" name="travel_refund_order[refund_fee]" id="form_refund_fee" />
                <.input size="md" name="travel_refund_order[refund_amount]" id="form_refund_amount" />
                <.select size="md" name="travel_refund_order[approval_mode]" id="form_approval_mode" :let={builder}>
      ...
      <.section layout="none" id="form_footer">
        <.flex justify="end" gap={2} direction="row" id="footer_actions">
          <.button variant="secondary" phx-click="form_cancel" size="md" id="footer_cancel">
            取消
          </.button>
          <.button variant="primary" phx-click="form_submit" size="md" id="footer_submit">
            提交
          </.button>
        </.flex>
      </.section>
