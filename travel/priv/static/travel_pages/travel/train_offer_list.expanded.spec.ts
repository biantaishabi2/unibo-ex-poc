...
const __WAIT_CONTRACT = {
  ...
  "navigate_create": {
    ...
    "until": {
      ...
      "state_key": null,
      "url_contains": "/pages/travel/train_offer/new"
    },
    "url_contains": "/pages/travel/train_offer/new"
  },
  "refetch": {
    ...
    "ui": [
      {
        "assert": "url_contains",
        "selector": null,
        "value": "/pages/travel/train_offer/new"
      },
      {
        "assert": "visible",
        "selector": "#train_offer_detail",
...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/train_offer");
  if (path.includes(':id')) {
...
test.describe("train_offer_list", () => {
  test('DTO smoke', async ({ page }) => {
    const __ctx = { form: {}, route: {}, clicked_row: null, __seed_ids: new Set(), __cleanup_queue: [] };
    await ensureContractSetup(page, __ctx);
    ...
    await expect(locatorFor(page, `#page_info`)).toContainText(`/`, { timeout: 15000 });
  });
  ...
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await test.step("页面加载 + 结构可见", async () => {
        ...
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      await expect(page.locator(`#train_offer_list`)).toBeVisible({ timeout: 15000 });
        await runCaseWait(page, __ctx, "load", []);
        await captureCreatedRecordId(__ctx, "load", []);
        await runCaseVerification(page, __ctx, "load", "load", []);
      });
      await test.step("表格区域可见", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      await expect(page.locator(`#train_offer_table, #table_section, table`).first()).toBeVisible({ timeout: 15000 });
        await runCaseWait(page, __ctx, "__AUTO__", []);
        await captureCreatedRecordId(__ctx, "structure", []);
        await runCaseVerification(page, __ctx, "__AUTO__", "structure", []);
      });
      await test.step("筛选提交", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#filter_submit`).first();
      ...
      await expect(page.locator(`#train_offer_table, #table_section, table`).first()).toBeVisible({ timeout: 15000 });
        await runCaseWait(page, __ctx, "filter_submit", ["filter_submit"]);
        await captureCreatedRecordId(__ctx, "flow", ["filter_submit"]);
        await runCaseVerification(page, __ctx, "filter_submit", "flow", ["filter_submit"]);
      });
      await test.step("新建导航", async () => {
        ...
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#create_btn`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
      ...
      {
        const expectedUrl = resolveTemplateString(__ctx, "/pages/travel/train_offer/new");
      try {
      ...
      await waitForLiveViewReady(page, 15000);
      await syncRouteContext(page, __ctx);
        await runCaseWait(page, __ctx, "navigate_create", ["navigate_create"]);
        await captureCreatedRecordId(__ctx, "crud", ["navigate_create"]);
        await runCaseVerification(page, __ctx, "navigate_create", "crud", ["navigate_create"]);
      });
    } finally {
