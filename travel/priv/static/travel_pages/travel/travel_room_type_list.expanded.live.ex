defmodule UniboExPocWeb.Pages.Travel.TravelRoomTypeListLive do
  @moduledoc """
  ...
  # status.keys preview (first ~40): rows, rows[], rows[].bed_type, rows[].hotel_code, rows[].id, rows[].room_type_code, rows[].room_type_name, rows[].status, rows_empty, travel_room_type, travel_room_type.page, travel_room_type.total_pages
  ...
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRoomType.list"}
  }
  @backend_embedded_page %{page_id: "travel_room_type_list", page_kind: "list", api_map: %{list: "Travel.TravelRoomType.list"}, backend: %{load: %{selection: "bed_type: bedType hotel_code: hotelCode id room_type_code: roomTypeCode room_type_name: roomTypeName status"}}, route: %{path: "/pages/travel/travel_room_type", query: "", kind: "list"}, state_schema: %{defaults: %{travel_room_type: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", room_type_code: "", room_type_name: "", hotel_code: "", bed_type: "", status: ""}, %{id: "row_02", room_type_code: "", room_type_name: "", hotel_code: "", bed_type: "", status: ""}, %{id: "row_03", room_type_code: "", room_type_name: "", hotel_code: "", bed_type: "", status: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading", "active_status"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_room_type/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_room_type_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              ...
              <.table_head id="th_room_type_name">
                房型名称
              </.table_head>
              <.table_head id="th_hotel_code">
                酒店编码冗余（便于兼容检索）
              </.table_head>
              <.table_head id="th_bed_type">
                床型
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_room_type_row">
                <.table_cell id="td_room_type_code">
                  <%= get_in(row, [:room_type_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_room_type_name">
                  <%= get_in(row, [:room_type_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_hotel_code">
                  <%= get_in(row, [:hotel_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_bed_type">
                  <%= get_in(row, [:bed_type]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_room_type_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_room_type, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_room_type, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_room_type_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
