...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/travel_static_code_mapping/:id");
  if (path.includes(':id')) {
...
test.describe("travel_static_code_mapping_detail", () => {
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator(`#travel_static_code_mapping_detail`)).toBeVisible({ timeout: 15000 });
      ...
      await expect(page.locator(`#travel_static_code_mapping_edit_form, #main_form, form[phx-submit="form_submit"]`).first()).not.toBeVisible({ timeout: 15000 });
