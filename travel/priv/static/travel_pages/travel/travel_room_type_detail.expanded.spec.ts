...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/travel_room_type/:id");
  if (path.includes(':id')) {
...
test.describe("travel_room_type_detail", () => {
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator(`#travel_room_type_detail`)).toBeVisible({ timeout: 15000 });
      ...
      await expect(page.locator(`#travel_room_type_edit_form, #main_form, form[phx-submit="form_submit"]`).first()).not.toBeVisible({ timeout: 15000 });
      ...
      }
      {
        ...
        await loc.fill(resolvedValue, { timeout: 15000 });
        __ctx.form["hotel_code"] = resolvedValue; refreshDataBindings(__ctx);
      }
      {
        const loc = page.locator(`#travel_room_type_form_bed_type, [name='bed_type']`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
