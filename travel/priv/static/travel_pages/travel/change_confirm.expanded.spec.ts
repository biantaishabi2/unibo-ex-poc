...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/change_confirm");
  if (path.includes(':id')) {
...
test.describe("change_confirm", () => {
  test('DTO smoke', async ({ page }) => {
    ...
    await expect(locatorFor(page, `#wz_step_1_label`)).toContainText(`基本信息`, { timeout: 15000 });
    await expect(locatorFor(page, `#wz_step_1_title`)).toContainText(`基本信息`, { timeout: 15000 });
    await expect(locatorFor(page, `#wz_step_2_label`)).toContainText(`详细配置`, { timeout: 15000 });
    await expect(locatorFor(page, `#wz_step_2_title`)).toContainText(`详细配置`, { timeout: 15000 });
    await expect(locatorFor(page, `#wz_step_3_label`)).toContainText(`确认提交`, { timeout: 15000 });
    await expect(locatorFor(page, `#wz_step_3_title`)).toContainText(`确认提交`, { timeout: 15000 });
  });
});
