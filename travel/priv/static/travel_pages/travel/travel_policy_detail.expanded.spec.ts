...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/travel_policy/:id");
  if (path.includes(':id')) {
...
test.describe("travel_policy_detail", () => {
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator(`#travel_policy_detail`)).toBeVisible({ timeout: 15000 });
      ...
      await expect(page.locator(`#travel_policy_edit_form, #main_form, form[phx-submit="form_submit"]`).first()).not.toBeVisible({ timeout: 15000 });
      ...
      await test.step("动作：activate", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_policy_action_activate`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        await captureCreatedRecordId(__ctx, "custom", ["action_activate"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_activate"]);
      });
      await test.step("动作：deactivate", async () => {
        await ensureSeedRecord(page, __ctx);
        await page.goto(resolveContractUrl(__ctx));
        await syncRouteContext(page, __ctx);
        await waitForLiveViewReady(page, 15000);
      {
        const loc = page.locator(`#travel_policy_action_deactivate`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        const clickedId = await loc.getAttribute('phx-value-id') || await loc.getAttribute('data-id');
        const confirmText = await loc.getAttribute('data-confirm');
        await loc.scrollIntoViewIfNeeded();
        ...
        await captureCreatedRecordId(__ctx, "custom", ["action_deactivate"]);
        await runCaseVerification(page, __ctx, null, "custom", ["action_deactivate"]);
      });
    } finally {
