...
function resolveContractUrl(ctx) {
  let path = resolveTemplateString(ctx, "/pages/travel/travel_hotel/:id");
  if (path.includes(':id')) {
...
test.describe("travel_hotel_detail", () => {
  test('E2E flow', async ({ page }) => {
    ...
    try {
      ...
      await expect(page.locator(`#travel_hotel_detail`)).toBeVisible({ timeout: 15000 });
      ...
      await expect(page.locator(`#travel_hotel_edit_form, #main_form, form[phx-submit="form_submit"]`).first()).not.toBeVisible({ timeout: 15000 });
      ...
      {
        const loc = page.locator(`#travel_hotel_form_city_code, [name='city_code']`).first();
        await loc.waitFor({ state: 'visible', timeout: 15000 });
