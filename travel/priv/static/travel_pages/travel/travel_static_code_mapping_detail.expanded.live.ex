defmodule UniboExPocWeb.Pages.Travel.TravelStaticCodeMappingDetailLive do
  @moduledoc """
  ...
  @backend_params_accept ["id", "supplier_code", "object_type", "canonical_entity", "canonical_id", "external_name", "status", "external_code"]
  ...
  @backend_api_map %{
    ...
    "get" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelStaticCodeMapping.get"},
    "update" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelStaticCodeMapping.update"}
  }
  @backend_embedded_page %{page_id: "travel_static_code_mapping_detail", page_kind: "detail", api_map: %{create: "Travel.TravelStaticCodeMapping.create", get: "Travel.TravelStaticCodeMapping.get", update: "Travel.TravelStaticCodeMapping.update"}, backend: %{load: %{selection: "canonical_entity: canonicalEntity canonical_id: canonicalId external_code: externalCode external_name: externalName id object_type: objectType status supplier_code: supplierCode"}}, route: %{path: "/pages/travel/travel_static_code_mapping/:id", query: "", kind: "detail"}, state_schema: %{defaults: %{travel_static_code_mapping: %{supplier_code: "", object_type: "", canonical_entity: "", external_code: "", external_name: "", status: ""}, editing: false}}, status_keys: ["record", "editing", "form", "loading"]}
    ...
    if has_id do
      ...
      id =
        Map.get(params, "id") ||
          ...
          get_in(socket.assigns, [:record, "id"]) ||
          get_in(socket.assigns, [:travel_static_code_mapping, :id]) ||
          get_in(socket.assigns, [:travel_static_code_mapping, "id"])
      case id do
  ...
  defp __sync_entity_assign(socket, dto) when is_map(dto) do
    case dto do
      %{travel_static_code_mapping: entity} when is_map(entity) and map_size(entity) > 0 -> assign(socket, :travel_static_code_mapping, entity)
      %{record: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
          true -> assign(socket, :travel_static_code_mapping, entity)
          false -> socket
        end
      _ ->
        source =
        ...
        if map_size(source) == 0 do
          ...
          entity = Map.get(socket.assigns, :travel_static_code_mapping, %{})
          entity = if is_map(entity), do: Map.merge(entity, source), else: source
          assign(socket, :travel_static_code_mapping, entity)
        end
    end
  end
  ...
  defp __sync_record_alias(socket) do
    case socket.assigns do
      %{travel_static_code_mapping: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
  ...
  assigns = assigns
  effects = []
  effects = [%{type: "navigate", to: "/pages/travel/travel_static_code_mapping"} | effects]
  ...
  def render(assigns) do
    ~H"""
    <.page title="详情" id="travel_static_code_mapping_detail">
        ...
        <.flex gap={3} align="center" direction="row" id="title_group">
          <.stack direction="column" id="title_meta">
            ...
            <.text variant="h1" id="page_title">
              供应商静态码到 Travel 主数据的映射（Travel 层统一适配）
            </.text>
          </.stack>
          <%= case get_in(@travel_static_code_mapping, [:status]) do %>
            <% "active" -> %>
                ...
                <.grid columns={2} gap={3} id="detail_fields">
                  <.stack gap={0} direction="column" id="detail_fields_supplier_code">
                    ...
                    <.text id="detail_fields_supplier_code_value">
                      <%= get_in(@travel_static_code_mapping, [:supplier_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_object_type">
                    <.text variant="caption" color="muted" id="detail_fields_object_type_label">
                      主数据对象类型
                    </.text>
                    <.text id="detail_fields_object_type_value">
                      <%= get_in(@travel_static_code_mapping, [:object_type]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_canonical_entity">
                    <.text variant="caption" color="muted" id="detail_fields_canonical_entity_label">
                      Travel 实体名
                    </.text>
                    <.text id="detail_fields_canonical_entity_value">
                      <%= get_in(@travel_static_code_mapping, [:canonical_entity]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_external_code">
                    <.text variant="caption" color="muted" id="detail_fields_external_code_label">
                      供应商侧编码
                    </.text>
                    <.text id="detail_fields_external_code_value">
                      <%= get_in(@travel_static_code_mapping, [:external_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_external_name">
                    <.text variant="caption" color="muted" id="detail_fields_external_name_label">
                      供应商侧名称
                    </.text>
                    <.text id="detail_fields_external_name_value">
                      <%= get_in(@travel_static_code_mapping, [:external_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_status">
                    ...
                    <.text id="detail_fields_status_value">
                      <%= get_in(@travel_static_code_mapping, [:status]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                </.grid>
              </.grid>
            <% else %>
              <.form for={%{}} phx-submit="form_submit" phx-change="form_change" id="travel_static_code_mapping_edit_form">
                <.grid columns={2} gap={4} id="form_grid">
                  <.stack direction="column" id="form_fields">
                    <.input size="md" name="travel_static_code_mapping[supplier_code]" id="travel_static_code_mapping_form_supplier_code" value={get_in(@travel_static_code_mapping, [:supplier_code]) || ""} />
                    <.select size="md" name="travel_static_code_mapping[object_type]" id="travel_static_code_mapping_form_object_type" :let={builder} value={get_in(@travel_static_code_mapping, [:object_type]) || ""}>
                      <.select_trigger builder={builder} />
                      <.select_content builder={builder}>
                        <.select_group>
                          <.select_item builder={builder} value="city" label="city">city</.select_item>
                          <.select_item builder={builder} value="airport" label="airport">airport</.select_item>
                          <.select_item builder={builder} value="station" label="station">station</.select_item>
                          <.select_item builder={builder} value="hotel" label="hotel">hotel</.select_item>
                          <.select_item builder={builder} value="room_type" label="room_type">room_type</.select_item>
                          <.select_item builder={builder} value="airline" label="airline">airline</.select_item>
                          <.select_item builder={builder} value="cabin_class" label="cabin_class">cabin_class</.select_item>
                        </.select_group>
                      </.select_content>
                    </.select>
                    <.input size="md" name="travel_static_code_mapping[canonical_entity]" id="travel_static_code_mapping_form_canonical_entity" value={get_in(@travel_static_code_mapping, [:canonical_entity]) || ""} />
                    <.input size="md" name="travel_static_code_mapping[canonical_id]" id="travel_static_code_mapping_form_canonical_id" value={get_in(@travel_static_code_mapping, [:canonical_id]) || ""} />
                    <.input size="md" name="travel_static_code_mapping[external_code]" id="travel_static_code_mapping_form_external_code" value={get_in(@travel_static_code_mapping, [:external_code]) || ""} />
                    <.input size="md" name="travel_static_code_mapping[external_name]" id="travel_static_code_mapping_form_external_name" value={get_in(@travel_static_code_mapping, [:external_name]) || ""} />
                    <.select size="md" name="travel_static_code_mapping[status]" id="travel_static_code_mapping_form_status" :let={builder} value={get_in(@travel_static_code_mapping, [:status]) || ""}>
                      <.select_trigger builder={builder} />
                      <.select_content builder={builder}>
                        <.select_group>
