defmodule UniboExPocWeb.Pages.Travel.TravelRefundOrderListLive do
  @moduledoc """
  ...
  @page_id "travel_refund_order_list"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): rows, rows[], rows[].approval_mode, rows[].id, rows[].refund_amount, rows[].refund_fee, rows[].refund_reason, rows[].status, rows_empty, travel_refund_order, travel_refund_order.page, travel_refund_order.total_pages
  ...
  @backend_embedded_page %{page_id: "travel_refund_order_list", page_kind: "list", api_map: %{list: "Travel.TravelRefundOrder.list"}, backend: %{load: %{selection: "approval_mode: approvalMode id refund_amount: refundAmount refund_fee: refundFee refund_reason: refundReason status"}}, route: %{path: "/pages/travel/travel_refund_order", query: "", kind: "list"}, state_schema: %{defaults: %{travel_refund_order: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", refund_reason: "", refund_fee: "", refund_amount: "", status: "", approval_mode: ""}, %{id: "row_02", refund_reason: "", refund_fee: "", refund_amount: "", status: "", approval_mode: ""}, %{id: "row_03", refund_reason: "", refund_fee: "", refund_amount: "", status: "", approval_mode: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  @impl true
  def handle_event("travel_refund_order_page_next", params, socket) do
    # UI action event name: travel_refund_order_page_next
    socket = dispatch_backend("travel_refund_order_page_next", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("travel_refund_order_page_prev", params, socket) do
    # UI action event name: travel_refund_order_page_prev
    socket = dispatch_backend("travel_refund_order_page_prev", params, socket)
    {:noreply, socket}
  end
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_refund_order/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_refund_order_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              <.table_head id="th_refund_reason">
                退票原因
              </.table_head>
              <.table_head id="th_refund_fee">
                退票手续费
              </.table_head>
              <.table_head id="th_refund_amount">
                实退金额
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_refund_order_row">
                <.table_cell id="td_refund_reason">
                  <%= get_in(row, [:refund_reason]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_refund_fee">
                  <%= get_in(row, [:refund_fee]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_refund_amount">
                  <%= get_in(row, [:refund_amount]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_refund_order_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_refund_order, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_refund_order, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_refund_order_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
