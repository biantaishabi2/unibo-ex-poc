defmodule UniboExPoc.Loyalty.Workflows.Coupon.CouponLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Loyalty.Coupon
...
{"steps":[{"idempotency_key":null,"next":["update","apply","cancel_coupon"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","apply","cancel_coupon"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["apply","cancel_coupon"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"apply"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel_coupon"}],"workflow":"coupon_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Coupon, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :apply ->
        Ash.update(Ash.Changeset.for_update(record, :apply, params), ash_opts)
      :cancel_coupon ->
        Ash.update(Ash.Changeset.for_update(record, :cancel_coupon, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :apply, :cancel_coupon]
      :update -> [:update, :apply, :cancel_coupon]
      :apply -> [:apply, :cancel_coupon]
      :cancel_coupon -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :apply -> %{max_attempts: 1, backoff_ms: 0}
      :cancel_coupon -> %{max_attempts: 1, backoff_ms: 0}
