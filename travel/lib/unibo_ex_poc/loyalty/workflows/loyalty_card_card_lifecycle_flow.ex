defmodule UniboExPoc.Loyalty.Workflows.LoyaltyCard.CardLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Loyalty.LoyaltyCard
...
{"steps":[{"idempotency_key":null,"next":["earn_points","redeem_points","freeze","expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["earn_points","redeem_points","freeze","expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"earn_points"},{"idempotency_key":null,"next":["earn_points","redeem_points","freeze","expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"redeem_points"},{"idempotency_key":null,"next":["unfreeze"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"freeze"},{"idempotency_key":null,"next":["earn_points","redeem_points","freeze","expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unfreeze"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"expire_card"}],"workflow":"card_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LoyaltyCard, :create, params), ash_opts)
      :earn_points ->
        Ash.update(Ash.Changeset.for_update(record, :earn_points, params), ash_opts)
      :redeem_points ->
        Ash.update(Ash.Changeset.for_update(record, :redeem_points, params), ash_opts)
      :freeze ->
        Ash.update(Ash.Changeset.for_update(record, :freeze, params), ash_opts)
      :unfreeze ->
        Ash.update(Ash.Changeset.for_update(record, :unfreeze, params), ash_opts)
      :expire_card ->
        Ash.update(Ash.Changeset.for_update(record, :expire_card, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:earn_points, :redeem_points, :freeze, :expire_card]
      :earn_points -> [:earn_points, :redeem_points, :freeze, :expire_card]
      :redeem_points -> [:earn_points, :redeem_points, :freeze, :expire_card]
      :freeze -> [:unfreeze]
      :unfreeze -> [:earn_points, :redeem_points, :freeze, :expire_card]
      :expire_card -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :earn_points -> %{max_attempts: 1, backoff_ms: 0}
      :redeem_points -> %{max_attempts: 1, backoff_ms: 0}
      :freeze -> %{max_attempts: 1, backoff_ms: 0}
      :unfreeze -> %{max_attempts: 1, backoff_ms: 0}
      :expire_card -> %{max_attempts: 1, backoff_ms: 0}
