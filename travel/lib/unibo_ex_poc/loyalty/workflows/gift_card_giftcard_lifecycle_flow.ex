...
{"steps":[{"idempotency_key":null,"next":["activate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["topup","charge","cancel_card","expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":["topup","charge","cancel_card","expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"topup"},{"idempotency_key":null,"next":["topup","charge","cancel_card","expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"charge"},{"idempotency_key":null,"next":["expire_card"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel_card"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"expire_card"}],"workflow":"giftcard_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(GiftCard, :create, params), ash_opts)
      :activate ->
        Ash.update(Ash.Changeset.for_update(record, :activate, params), ash_opts)
      :topup ->
        Ash.update(Ash.Changeset.for_update(record, :topup, params), ash_opts)
      :charge ->
        Ash.update(Ash.Changeset.for_update(record, :charge, params), ash_opts)
      :cancel_card ->
        Ash.update(Ash.Changeset.for_update(record, :cancel_card, params), ash_opts)
      :expire_card ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:activate]
      :activate -> [:topup, :charge, :cancel_card, :expire_card]
      :topup -> [:topup, :charge, :cancel_card, :expire_card]
      :charge -> [:topup, :charge, :cancel_card, :expire_card]
      :cancel_card -> [:expire_card]
      :expire_card -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :topup -> %{max_attempts: 1, backoff_ms: 0}
      :charge -> %{max_attempts: 1, backoff_ms: 0}
      :cancel_card -> %{max_attempts: 1, backoff_ms: 0}
