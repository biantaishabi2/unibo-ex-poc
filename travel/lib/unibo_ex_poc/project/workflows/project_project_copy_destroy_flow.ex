defmodule UniboExPoc.Project.Workflows.Project.ProjectCopyDestroyFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
...
{"steps":[{"idempotency_key":null,"next":["copy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"copy"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"project_copy_destroy_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      ...
      :copy ->
        Ash.create(Ash.Changeset.for_create(Project, :copy, params), ash_opts)
      :destroy ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :copy -> %{max_attempts: 1, backoff_ms: 0}
