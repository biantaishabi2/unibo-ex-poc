defmodule UniboExPoc.Project.Workflows.PlanningSlot.PlanningSlotTemplateFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
...
{"steps":[{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create_from_template"}],"workflow":"planning_slot_template_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create_from_template ->
        Ash.create(Ash.Changeset.for_create(PlanningSlot, :create_from_template, params), ash_opts)
      _ -> {:ok, record}
    end
  end
