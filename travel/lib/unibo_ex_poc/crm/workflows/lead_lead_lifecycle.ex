defmodule UniboExPoc.CRM.Workflows.Lead.LeadLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.CRM.Lead
...
{"steps":[{"idempotency_key":null,"next":["update","convert_opportunity","lose","assign_salesperson","merge"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","win","lose","assign_salesperson"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"convert_opportunity"},{"idempotency_key":null,"next":["lose"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"win"},{"idempotency_key":null,"next":["assign_salesperson"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"lose"},{"idempotency_key":null,"next":["update","convert_opportunity","win","lose"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"assign_salesperson"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"merge"}],"workflow":"lead_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Lead, :create, params), ash_opts)
      :convert_opportunity ->
        Ash.update(Ash.Changeset.for_update(record, :convert_opportunity, params), ash_opts)
      :win ->
        Ash.update(Ash.Changeset.for_update(record, :win, params), ash_opts)
      :lose ->
        Ash.update(Ash.Changeset.for_update(record, :lose, params), ash_opts)
      :assign_salesperson ->
        Ash.update(Ash.Changeset.for_update(record, :assign_salesperson, params), ash_opts)
      :merge ->
        Ash.update(Ash.Changeset.for_update(record, :merge, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :convert_opportunity, :lose, :assign_salesperson, :merge]
      :convert_opportunity -> [:update, :win, :lose, :assign_salesperson]
      :win -> [:lose]
      :lose -> [:assign_salesperson]
      :assign_salesperson -> [:update, :convert_opportunity, :win, :lose]
      :merge -> []
      _ -> []
    end
  end
  ...
  defp branch_next(step, record) do
    case step do
      :create -> nil
      :convert_opportunity -> nil
      :win -> nil
      :lose -> nil
      :assign_salesperson -> nil
      :merge -> nil
      _ -> nil
    end
  end
  ...
  defp step_skipped?(step, record) do
    case step do
      :create -> false
      :convert_opportunity -> false
      :win -> false
      :lose -> false
      :assign_salesperson -> false
      :merge -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :convert_opportunity -> %{max_attempts: 1, backoff_ms: 0}
      :win -> %{max_attempts: 1, backoff_ms: 0}
      :lose -> %{max_attempts: 1, backoff_ms: 0}
      :assign_salesperson -> %{max_attempts: 1, backoff_ms: 0}
      :merge -> %{max_attempts: 1, backoff_ms: 0}
