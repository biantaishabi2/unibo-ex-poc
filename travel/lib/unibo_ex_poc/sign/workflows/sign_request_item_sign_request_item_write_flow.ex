defmodule UniboExPoc.Sign.Workflows.SignRequestItem.SignRequestItemWriteFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Sign.SignRequestItem

  def steps do
    [:create, :send, :sign, :refuse, :reset]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["send"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["sign"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"send"},{"idempotency_key":null,"next":["refuse"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"sign"},{"idempotency_key":null,"next":["reset"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"refuse"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reset"}],"workflow":"sign_request_item_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SignRequestItem, :create, params), ash_opts)
      :send ->
