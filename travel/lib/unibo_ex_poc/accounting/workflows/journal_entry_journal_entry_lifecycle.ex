defmodule UniboExPoc.Accounting.Workflows.JournalEntry.JournalEntryLifecycleWorkflow do
  ...
  def steps do
    [:s1_create, :s2_post, :s3_cancel, :s4_reset_to_draft]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["post"],"next_step_ids":["s2_post"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["cancel","reset_to_draft"],"next_step_ids":["s3_cancel","s4_reset_to_draft"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"post","step_id":"s2_post"},{"idempotency_key":null,"next":["reset_to_draft"],"next_step_ids":["s4_reset_to_draft"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel","step_id":"s3_cancel"},{"idempotency_key":null,"next":["post"],"next_step_ids":["s2_post"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reset_to_draft","step_id":"s4_reset_to_draft"}],"workflow":"journal_entry_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(JournalEntry, :create, params), ash_opts)
      :s2_post ->
        Ash.update(Ash.Changeset.for_update(record, :post, params), ash_opts)
      :s3_cancel ->
        Ash.update(Ash.Changeset.for_update(record, :cancel, params), ash_opts)
      :s4_reset_to_draft ->
        Ash.update(Ash.Changeset.for_update(record, :reset_to_draft, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_post]
      :s2_post -> [:s3_cancel, :s4_reset_to_draft]
      :s3_cancel -> [:s4_reset_to_draft]
      :s4_reset_to_draft -> [:s2_post]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s2_post -> %{max_attempts: 1, backoff_ms: 0}
      :s3_cancel -> %{max_attempts: 1, backoff_ms: 0}
      :s4_reset_to_draft -> %{max_attempts: 1, backoff_ms: 0}
