defmodule UniboExPoc.Accounting.Workflows.GlAccount.GlAccountLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Accounting.GlAccount
...
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(GlAccount, :create, params), ash_opts)
      :s2_update ->
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_update, :s3_deactivate]
      :s2_update -> [:s2_update, :s3_deactivate]
      :s3_deactivate -> []
      _ -> []
    end
  end
