defmodule UniboExPoc.Accounting.Workflows.FullReconcile.FullReconcileLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Accounting.FullReconcile

  def steps do
    [:s1_create, :s2_destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["destroy"],"next_step_ids":["s2_destroy"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy","step_id":"s2_destroy"}],"workflow":"full_reconcile_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(FullReconcile, :create, params), ash_opts)
      :s2_destroy ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s2_destroy -> %{max_attempts: 1, backoff_ms: 0}
