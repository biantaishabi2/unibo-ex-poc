defmodule UniboExPoc.Accounting.Workflows.Invoice.InvoiceLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Accounting.Invoice

  def steps do
    [:s1_create, :s2_approve, :s3_send, :s4_void]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["approve","void"],"next_step_ids":["s2_approve","s4_void"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["send","void"],"next_step_ids":["s3_send","s4_void"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"approve","step_id":"s2_approve"},{"idempotency_key":null,"next":["void"],"next_step_ids":["s4_void"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"send","step_id":"s3_send"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"void","step_id":"s4_void"}],"workflow":"invoice_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(Invoice, :create, params), ash_opts)
      :s2_approve ->
        Ash.update(Ash.Changeset.for_update(record, :approve, params), ash_opts)
      :s3_send ->
        Ash.update(Ash.Changeset.for_update(record, :send, params), ash_opts)
      :s4_void ->
        Ash.update(Ash.Changeset.for_update(record, :void, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_approve, :s4_void]
      :s2_approve -> [:s3_send, :s4_void]
      :s3_send -> [:s4_void]
      :s4_void -> []
      _ -> []
    end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s3_send -> %{max_attempts: 1, backoff_ms: 0}
      :s4_void -> %{max_attempts: 1, backoff_ms: 0}
