...
"
  ...
  actions do
    defaults [:read]
    create :create do
      description "Create Journal Entry via Create. doc_url: graphql://contract/accounting/create_accounting_journal_entry"
      primary? true
...
过账 [R-ACC-001 ~ R-ACC-007]. doc_url: graphql://contract/accounting/post_accounting_journal_entry"
      primary? true
      accept []
...
取消（可能生成冲销分录）. doc_url: graphql://contract/accounting/cancel_accounting_journal_entry"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :status)
...
重置为草稿. doc_url: graphql://contract/accounting/reset_to_draft_accounting_journal_entry"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :status)
