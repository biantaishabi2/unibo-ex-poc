...
defmodule UniboExPoc.Accounting.Payment do
  ...
  postgres do
    ...
    identity_index_names unique_payment_number: "idx_accounting_payments_unique_payment_number"
  end
...
"
  ...
  actions do
    defaults [:read]
    create :create do
      description "Create Payment via Create. doc_url: graphql://contract/accounting/create_accounting_payment"
      primary? true
...
确认付款（过账底层 move）[R-PAY-003]. doc_url: graphql://contract/accounting/post_accounting_payment"
      primary? true
      accept []
...
取消付款. doc_url: graphql://contract/accounting/cancel_accounting_payment"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :status)
...
重置为草稿. doc_url: graphql://contract/accounting/reset_to_draft_accounting_payment"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :status)
