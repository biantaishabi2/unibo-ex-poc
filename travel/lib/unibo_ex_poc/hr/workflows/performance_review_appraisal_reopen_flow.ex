...
{"steps":[{"idempotency_key":null,"next":["action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_back"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_cancel"},{"idempotency_key":null,"next":["action_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_back"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_confirm"}],"workflow":"appraisal_reopen_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(PerformanceReview, :create, params), ash_opts)
      :action_cancel ->
        Ash.update(Ash.Changeset.for_update(record, :action_cancel, params), ash_opts)
      :action_back ->
        Ash.update(Ash.Changeset.for_update(record, :action_back, params), ash_opts)
      :action_confirm ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_back -> %{max_attempts: 1, backoff_ms: 0}
