...
{"steps":[{"idempotency_key":null,"next":["action_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_approve"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_confirm"},{"idempotency_key":null,"next":["action_validate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_approve"},{"idempotency_key":null,"next":["action_refuse"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_validate"},{"idempotency_key":null,"next":["action_draft"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_refuse"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_draft"}],"workflow":"leave_request_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LeaveRequest, :create, params), ash_opts)
      :action_confirm ->
        Ash.update(Ash.Changeset.for_update(record, :action_confirm, params), ash_opts)
      :action_approve ->
        Ash.update(Ash.Changeset.for_update(record, :action_approve, params), ash_opts)
      :action_validate ->
        Ash.update(Ash.Changeset.for_update(record, :action_validate, params), ash_opts)
      :action_refuse ->
        Ash.update(Ash.Changeset.for_update(record, :action_refuse, params), ash_opts)
      :action_draft ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :action_approve -> [:action_validate]
      :action_validate -> [:action_refuse]
      :action_refuse -> [:action_draft]
      :action_draft -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_approve -> %{max_attempts: 1, backoff_ms: 0}
      :action_validate -> %{max_attempts: 1, backoff_ms: 0}
      :action_refuse -> %{max_attempts: 1, backoff_ms: 0}
