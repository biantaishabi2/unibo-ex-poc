defmodule UniboExPoc.HR.Workflows.EmploymentContract.EmploymentContractWriteFlowWorkflow do
  ...
  def steps do
    [:s1_create, :s2_activate, :s3_terminate]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["activate"],"next_step_ids":["s2_activate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["terminate"],"next_step_ids":["s3_terminate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate","step_id":"s2_activate"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"terminate","step_id":"s3_terminate"}],"workflow":"employment_contract_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(EmploymentContract, :create, params), ash_opts)
      :s2_activate ->
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_activate]
      :s2_activate -> [:s3_terminate]
      :s3_terminate -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s2_activate -> %{max_attempts: 1, backoff_ms: 0}
