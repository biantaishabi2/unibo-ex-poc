...
{"steps":[{"idempotency_key":null,"next":["advance"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["hire"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"advance"},{"idempotency_key":null,"next":["archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"hire"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"}],"workflow":"recruitment_pipeline_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(JobApplication, :create, params), ash_opts)
      :advance ->
        Ash.update(Ash.Changeset.for_update(record, :advance, params), ash_opts)
      :hire ->
        Ash.update(Ash.Changeset.for_update(record, :hire, params), ash_opts)
      :archive ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :advance -> %{max_attempts: 1, backoff_ms: 0}
      :hire -> %{max_attempts: 1, backoff_ms: 0}
