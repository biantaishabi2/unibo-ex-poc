defmodule UniboExPoc.HR.Workflows.PaySlip.PayslipRefundFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  ...
  def steps do
    [:create, :action_payslip_cancel, :refund_sheet, :compute_sheet]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["action_payslip_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["refund_sheet"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_payslip_cancel"},{"idempotency_key":null,"next":["compute_sheet"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"refund_sheet"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"compute_sheet"}],"workflow":"payslip_refund_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(PaySlip, :create, params), ash_opts)
      :action_payslip_cancel ->
        Ash.update(Ash.Changeset.for_update(record, :action_payslip_cancel, params), ash_opts)
      :refund_sheet ->
        Ash.update(Ash.Changeset.for_update(record, :refund_sheet, params), ash_opts)
      :compute_sheet ->
        Ash.update(Ash.Changeset.for_update(record, :compute_sheet, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:action_payslip_cancel]
      :action_payslip_cancel -> [:refund_sheet]
      :refund_sheet -> [:compute_sheet]
      :compute_sheet -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_payslip_cancel -> %{max_attempts: 1, backoff_ms: 0}
      :refund_sheet -> %{max_attempts: 1, backoff_ms: 0}
      :compute_sheet -> %{max_attempts: 1, backoff_ms: 0}
