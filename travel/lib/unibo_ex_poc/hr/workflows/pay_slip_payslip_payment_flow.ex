...
{"steps":[{"idempotency_key":null,"next":["compute_sheet"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_payslip_done"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"compute_sheet"},{"idempotency_key":null,"next":["action_payslip_paid"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_payslip_done"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_payslip_paid"}],"workflow":"payslip_payment_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      ...
      :action_payslip_done ->
        Ash.update(Ash.Changeset.for_update(record, :action_payslip_done, params), ash_opts)
      :action_payslip_paid ->
        Ash.update(Ash.Changeset.for_update(record, :action_payslip_paid, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:compute_sheet]
      :compute_sheet -> [:action_payslip_done]
      :action_payslip_done -> [:action_payslip_paid]
      :action_payslip_paid -> []
      _ -> []
    end
  end
  ...
  defp branch_next(step, record) do
    case step do
      :create -> nil
      :compute_sheet -> nil
      :action_payslip_done -> nil
      :action_payslip_paid -> nil
      _ -> nil
    end
  end
  ...
  defp step_skipped?(step, record) do
    case step do
      :create -> false
      :compute_sheet -> false
      :action_payslip_done -> false
      :action_payslip_paid -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_payslip_done -> %{max_attempts: 1, backoff_ms: 0}
      :action_payslip_paid -> %{max_attempts: 1, backoff_ms: 0}
