defmodule UniboExPoc.HR.Workflows.PersonnelAction.PersonnelActionWriteFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.HR.PersonnelAction

  def steps do
    [:s1_create, :s2_submit, :s3_approve, :s4_execute]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["submit"],"next_step_ids":["s2_submit"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["approve"],"next_step_ids":["s3_approve"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"submit","step_id":"s2_submit"},{"idempotency_key":null,"next":["execute"],"next_step_ids":["s4_execute"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"approve","step_id":"s3_approve"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"execute","step_id":"s4_execute"}],"workflow":"personnel_action_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(PersonnelAction, :create, params), ash_opts)
      :s2_submit ->
      ...
      :s4_execute ->
        Ash.update(Ash.Changeset.for_update(record, :execute, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_submit]
      :s2_submit -> [:s3_approve]
      :s3_approve -> [:s4_execute]
      :s4_execute -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s3_approve -> %{max_attempts: 1, backoff_ms: 0}
      :s4_execute -> %{max_attempts: 1, backoff_ms: 0}
