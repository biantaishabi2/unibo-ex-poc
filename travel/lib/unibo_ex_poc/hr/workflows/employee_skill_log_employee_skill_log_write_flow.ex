defmodule UniboExPoc.HR.Workflows.EmployeeSkillLog.EmployeeSkillLogWriteFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.HR.EmployeeSkillLog
...
{"steps":[{"idempotency_key":null,"next":["read"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"read"}],"workflow":"employee_skill_log_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(EmployeeSkillLog, :create, params), ash_opts)
      :read ->
        {:ok, record}
      _ -> {:ok, record}
    end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :read -> %{max_attempts: 1, backoff_ms: 0}
