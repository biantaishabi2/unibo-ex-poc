defmodule UniboExPoc.HR.Workflows.PerformanceReview.AppraisalCycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  ...
  def steps do
    [:create, :action_confirm, :action_done]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["action_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_done"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_confirm"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_done"}],"workflow":"appraisal_cycle_flow"}
"""
