defmodule UniboExPoc.HR.Workflows.Employee.EmployeeWriteFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.HR.Employee

  def steps do
    [:s1_create, :s2_update, :s3_terminate, :s4_suspend, :s5_reactivate]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"next_step_ids":["s2_update"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["terminate"],"next_step_ids":["s3_terminate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update","step_id":"s2_update"},{"idempotency_key":null,"next":["suspend"],"next_step_ids":["s4_suspend"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"terminate","step_id":"s3_terminate"},{"idempotency_key":null,"next":["reactivate"],"next_step_ids":["s5_reactivate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"suspend","step_id":"s4_suspend"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reactivate","step_id":"s5_reactivate"}],"workflow":"employee_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(Employee, :create, params), ash_opts)
      :s2_update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :s3_terminate ->
        Ash.update(Ash.Changeset.for_update(record, :terminate, params), ash_opts)
      :s4_suspend ->
        Ash.update(Ash.Changeset.for_update(record, :suspend, params), ash_opts)
      :s5_reactivate ->
        Ash.update(Ash.Changeset.for_update(record, :reactivate, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_update]
      :s2_update -> [:s3_terminate]
      :s3_terminate -> [:s4_suspend]
      :s4_suspend -> [:s5_reactivate]
      :s5_reactivate -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s3_terminate -> %{max_attempts: 1, backoff_ms: 0}
      :s4_suspend -> %{max_attempts: 1, backoff_ms: 0}
      :s5_reactivate -> %{max_attempts: 1, backoff_ms: 0}
