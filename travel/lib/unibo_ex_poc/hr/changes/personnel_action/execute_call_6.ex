defmodule UniboExPoc.HR.Changes.PersonnelAction.ExecuteCall6 do
  use Ash.Resource.Change

  @impl true
  def change(changeset, _opts, context) do
    module_ref = "UniboExPoc.HR"
    module = resolve_call_module(module_ref)
    if is_atom(module) and Code.ensure_loaded?(module) and function_exported?(module, :custom, 2) do
      apply(module, :custom, [changeset, context])
    else
      Ash.Changeset.add_error(changeset, "call 目标不存在: #UniboExPoc.HR.custom/2")
    end
  end
