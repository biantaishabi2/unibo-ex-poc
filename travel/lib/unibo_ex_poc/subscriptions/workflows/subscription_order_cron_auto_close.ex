defmodule UniboExPoc.Subscriptions.Workflows.SubscriptionOrder.CronAutoCloseWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
...
{"steps":[{"idempotency_key":null,"next":["action_close"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"evaluate_auto_close_due"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_close"}],"workflow":"cron_auto_close"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :evaluate_auto_close_due ->
        Ash.update(Ash.Changeset.for_update(record, :evaluate_auto_close_due, params), ash_opts)
      :action_close ->
  ...
  defp retry_policy(step) do
    case step do
      :evaluate_auto_close_due -> %{max_attempts: 1, backoff_ms: 0}
