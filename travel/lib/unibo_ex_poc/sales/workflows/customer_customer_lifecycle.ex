defmodule UniboExPoc.Sales.Workflows.Customer.CustomerLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Sales.Customer

  def steps do
    [:s1_create, :s2_update, :s3_block]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","block"],"next_step_ids":["s2_update","s3_block"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["update","block"],"next_step_ids":["s2_update","s3_block"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update","step_id":"s2_update"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"block","step_id":"s3_block"}],"workflow":"customer_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(Customer, :create, params), ash_opts)
      :s2_update ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s3_block -> %{max_attempts: 1, backoff_ms: 0}
