...
{"steps":[{"idempotency_key":null,"next":["update","publish","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","start","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"publish"},{"idempotency_key":null,"next":["complete","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start"},{"idempotency_key":null,"next":["archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"complete"},{"idempotency_key":null,"next":["archive","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"event_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Event, :create, params), ash_opts)
      :publish ->
