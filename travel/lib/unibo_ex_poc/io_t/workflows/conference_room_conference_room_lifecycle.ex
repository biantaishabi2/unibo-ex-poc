defmodule UniboExPoc.IoT.Workflows.ConferenceRoom.ConferenceRoomLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.ConferenceRoom

  def steps do
    [:create, :update, :start_conference, :end_conference, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["start_conference"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["end_conference"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start_conference"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"end_conference"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"conference_room_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(ConferenceRoom, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :start_conference ->
        Ash.update(Ash.Changeset.for_update(record, :start_conference, params), ash_opts)
      :end_conference ->
        Ash.update(Ash.Changeset.for_update(record, :end_conference, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update]
      :update -> [:start_conference]
      :start_conference -> [:end_conference]
      :end_conference -> [:destroy]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :start_conference -> %{max_attempts: 1, backoff_ms: 0}
      :end_conference -> %{max_attempts: 1, backoff_ms: 0}
