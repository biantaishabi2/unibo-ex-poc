defmodule UniboExPoc.IoT.Workflows.EventLog.EventLogProcessingFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.EventLog

  def steps do
    [:create, :mark_processed]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["mark_processed"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_processed"}],"workflow":"event_log_processing_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(EventLog, :create, params), ash_opts)
      :mark_processed ->
        Ash.update(Ash.Changeset.for_update(record, :mark_processed, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :mark_processed -> %{max_attempts: 1, backoff_ms: 0}
