defmodule UniboExPoc.IoT.Workflows.VoIpUserConfig.VoipUserConfigLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.VoIPUserConfig

  def steps do
    [:create, :update, :toggle_dnd, :set_forwarding]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["toggle_dnd"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["set_forwarding"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"toggle_dnd"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"set_forwarding"}],"workflow":"voip_user_config_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(VoIPUserConfig, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :toggle_dnd ->
        Ash.update(Ash.Changeset.for_update(record, :toggle_dnd, params), ash_opts)
      :set_forwarding ->
        Ash.update(Ash.Changeset.for_update(record, :set_forwarding, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :toggle_dnd -> %{max_attempts: 1, backoff_ms: 0}
      :set_forwarding -> %{max_attempts: 1, backoff_ms: 0}
