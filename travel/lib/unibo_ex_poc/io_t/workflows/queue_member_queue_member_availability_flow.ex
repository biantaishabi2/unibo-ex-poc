defmodule UniboExPoc.IoT.Workflows.QueueMember.QueueMemberAvailabilityFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.QueueMember
...
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["pause"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["unpause"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"pause"},{"idempotency_key":null,"next":["record_call"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unpause"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"record_call"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"queue_member_availability_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(QueueMember, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :pause ->
        Ash.update(Ash.Changeset.for_update(record, :pause, params), ash_opts)
      :unpause ->
        Ash.update(Ash.Changeset.for_update(record, :unpause, params), ash_opts)
      :record_call ->
        Ash.update(Ash.Changeset.for_update(record, :record_call, params), ash_opts)
      :destroy ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :pause -> %{max_attempts: 1, backoff_ms: 0}
      :unpause -> %{max_attempts: 1, backoff_ms: 0}
      :record_call -> %{max_attempts: 1, backoff_ms: 0}
