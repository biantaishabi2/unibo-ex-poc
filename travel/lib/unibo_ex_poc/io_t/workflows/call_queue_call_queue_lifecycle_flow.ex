defmodule UniboExPoc.IoT.Workflows.CallQueue.CallQueueLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.CallQueue

  def steps do
    [:create, :update, :login_agent, :logout_agent, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["login_agent"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["logout_agent"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"login_agent"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"logout_agent"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"call_queue_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(CallQueue, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :login_agent ->
        Ash.update(Ash.Changeset.for_update(record, :login_agent, params), ash_opts)
      :logout_agent ->
        Ash.update(Ash.Changeset.for_update(record, :logout_agent, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update]
      :update -> [:login_agent]
      :login_agent -> [:logout_agent]
      :logout_agent -> [:destroy]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :login_agent -> %{max_attempts: 1, backoff_ms: 0}
      :logout_agent -> %{max_attempts: 1, backoff_ms: 0}
