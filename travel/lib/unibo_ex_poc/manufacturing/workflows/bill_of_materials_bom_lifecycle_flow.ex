defmodule UniboExPoc.Manufacturing.Workflows.BillOfMaterials.BomLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Manufacturing.BillOfMaterials

  def steps do
    [:create, :update, :activate, :obsolete]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["activate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["obsolete"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"obsolete"}],"workflow":"bom_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(BillOfMaterials, :create, params), ash_opts)
      :update ->
      ...
      :obsolete ->
        Ash.update(Ash.Changeset.for_update(record, :obsolete, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :obsolete -> %{max_attempts: 1, backoff_ms: 0}
