...
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["block"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["unblock"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"block"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unblock"}],"workflow":"work_center_status_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(WorkCenter, :create, params), ash_opts)
      :update ->
      ...
      :unblock ->
        Ash.update(Ash.Changeset.for_update(record, :unblock, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :unblock -> %{max_attempts: 1, backoff_ms: 0}
