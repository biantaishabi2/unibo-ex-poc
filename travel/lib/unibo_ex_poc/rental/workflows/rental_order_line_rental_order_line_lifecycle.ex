defmodule UniboExPoc.Rental.Workflows.RentalOrderLine.RentalOrderLineLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Rental.RentalOrderLine

  def steps do
    [:create, :update, :validate_pickup, :validate_return, :finalize]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","validate_pickup"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["validate_pickup"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["validate_return"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"validate_pickup"},{"idempotency_key":null,"next":["finalize"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"validate_return"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"finalize"}],"workflow":"rental_order_line_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(RentalOrderLine, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :validate_pickup ->
        Ash.update(Ash.Changeset.for_update(record, :validate_pickup, params), ash_opts)
      :validate_return ->
        Ash.update(Ash.Changeset.for_update(record, :validate_return, params), ash_opts)
      :finalize ->
        Ash.update(Ash.Changeset.for_update(record, :finalize, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :validate_pickup -> %{max_attempts: 1, backoff_ms: 0}
      :validate_return -> %{max_attempts: 1, backoff_ms: 0}
      :finalize -> %{max_attempts: 1, backoff_ms: 0}
