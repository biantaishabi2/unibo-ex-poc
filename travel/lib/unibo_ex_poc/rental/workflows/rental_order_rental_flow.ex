defmodule UniboExPoc.Rental.Workflows.RentalOrder.RentalFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Rental.RentalOrder

  def steps do
    [:create, :action_confirm, :action_pickup, :action_return, :action_done]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["action_confirm","action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_pickup","action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_confirm"},{"idempotency_key":null,"next":["action_return"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_pickup"},{"idempotency_key":null,"next":["action_done"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_return"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_done"}],"workflow":"rental_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(RentalOrder, :create, params), ash_opts)
      :action_confirm ->
        Ash.update(Ash.Changeset.for_update(record, :action_confirm, params), ash_opts)
      :action_pickup ->
        Ash.update(Ash.Changeset.for_update(record, :action_pickup, params), ash_opts)
      :action_return ->
        Ash.update(Ash.Changeset.for_update(record, :action_return, params), ash_opts)
      :action_done ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:action_confirm, :action_cancel]
      :action_confirm -> [:action_pickup, :action_cancel]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_pickup -> %{max_attempts: 1, backoff_ms: 0}
      :action_return -> %{max_attempts: 1, backoff_ms: 0}
