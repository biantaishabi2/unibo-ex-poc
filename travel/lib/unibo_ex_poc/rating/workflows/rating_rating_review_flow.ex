defmodule UniboExPoc.Rating.Workflows.Rating.RatingReviewFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Rating.Rating
...
{"steps":[{"idempotency_key":null,"next":["update","approve","reject"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"submit"},{"idempotency_key":null,"next":["approve","reject"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["flag"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"approve"},{"idempotency_key":null,"next":["flag"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reject"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"flag"}],"workflow":"rating_review_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :submit ->
        Ash.create(Ash.Changeset.for_create(Rating, :submit, params), ash_opts)
      :update ->
