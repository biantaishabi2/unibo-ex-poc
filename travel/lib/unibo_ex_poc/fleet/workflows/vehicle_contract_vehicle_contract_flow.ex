defmodule UniboExPoc.Fleet.Workflows.VehicleContract.VehicleContractFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Fleet.VehicleContract
...
{"steps":[{"idempotency_key":null,"next":["activate","terminate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["renew","terminate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":["renew","terminate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"renew"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"terminate"}],"workflow":"vehicle_contract_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(VehicleContract, :create, params), ash_opts)
      :activate ->
        Ash.update(Ash.Changeset.for_update(record, :activate, params), ash_opts)
      :renew ->
        Ash.update(Ash.Changeset.for_update(record, :renew, params), ash_opts)
      :terminate ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :renew -> %{max_attempts: 1, backoff_ms: 0}
