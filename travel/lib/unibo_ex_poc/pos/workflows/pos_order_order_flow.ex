defmodule UniboExPoc.POS.Workflows.PosOrder.OrderFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.POS.PosOrder
...
{"steps":[{"idempotency_key":null,"next":["pay","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["done"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"pay"},{"idempotency_key":null,"next":["invoice","refund"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"done"},{"idempotency_key":null,"next":["refund"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"invoice"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"refund"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"order_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(PosOrder, :create, params), ash_opts)
      :pay ->
        Ash.update(Ash.Changeset.for_update(record, :pay, params), ash_opts)
      :done ->
        Ash.update(Ash.Changeset.for_update(record, :done, params), ash_opts)
      :invoice ->
        Ash.update(Ash.Changeset.for_update(record, :invoice, params), ash_opts)
      :refund ->
        Ash.create(Ash.Changeset.for_create(PosOrder, :refund, params), ash_opts)
      :cancel ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :pay -> %{max_attempts: 1, backoff_ms: 0}
      :done -> %{max_attempts: 1, backoff_ms: 0}
      :invoice -> %{max_attempts: 1, backoff_ms: 0}
