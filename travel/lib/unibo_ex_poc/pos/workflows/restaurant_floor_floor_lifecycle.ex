defmodule UniboExPoc.POS.Workflows.RestaurantFloor.FloorLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.POS.RestaurantFloor

  def steps do
    [:create, :update, :deactivate, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","deactivate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","deactivate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"deactivate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"floor_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(RestaurantFloor, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :deactivate, :destroy]
      :update -> [:update, :deactivate, :destroy]
      :deactivate -> []
      :destroy -> []
      _ -> []
    end
