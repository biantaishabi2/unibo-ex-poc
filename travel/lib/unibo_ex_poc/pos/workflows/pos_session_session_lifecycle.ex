defmodule UniboExPoc.POS.Workflows.PosSession.SessionLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.POS.PosSession
...
{"steps":[{"idempotency_key":null,"next":["open"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["start_close"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"open"},{"idempotency_key":null,"next":["close"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start_close"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"close"}],"workflow":"session_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(PosSession, :create, params), ash_opts)
      :open ->
        Ash.update(Ash.Changeset.for_update(record, :open, params), ash_opts)
      :start_close ->
        Ash.update(Ash.Changeset.for_update(record, :start_close, params), ash_opts)
      :close ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :open -> %{max_attempts: 1, backoff_ms: 0}
      :start_close -> %{max_attempts: 1, backoff_ms: 0}
