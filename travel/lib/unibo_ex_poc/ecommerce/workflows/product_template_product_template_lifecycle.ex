...
{"steps":[{"idempotency_key":null,"next":["update","publish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","unpublish","discontinue"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"publish"},{"idempotency_key":null,"next":["update","publish","unpublish","discontinue"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["publish","discontinue"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unpublish"},{"idempotency_key":null,"next":["reactivate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"discontinue"},{"idempotency_key":null,"next":["publish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reactivate"}],"workflow":"product_template_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(ProductTemplate, :create, params), ash_opts)
      :publish ->
      ...
      :unpublish ->
        Ash.update(Ash.Changeset.for_update(record, :unpublish, params), ash_opts)
      :discontinue ->
        Ash.update(Ash.Changeset.for_update(record, :discontinue, params), ash_opts)
      :reactivate ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :publish -> [:update, :unpublish, :discontinue]
      :update -> [:update, :publish, :unpublish, :discontinue]
      :unpublish -> [:publish, :discontinue]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :publish -> %{max_attempts: 1, backoff_ms: 0}
      :update -> %{max_attempts: 1, backoff_ms: 0}
      :unpublish -> %{max_attempts: 1, backoff_ms: 0}
      :discontinue -> %{max_attempts: 1, backoff_ms: 0}
      :reactivate -> %{max_attempts: 1, backoff_ms: 0}
