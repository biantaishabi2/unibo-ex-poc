...
{"steps":[{"idempotency_key":null,"next":["update","add_points","share"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["add_points","update","share"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"add_points"},{"idempotency_key":null,"next":["update","add_points","share"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["add_points","update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"share"}],"workflow":"card_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      ...
      :add_points ->
        Ash.update(Ash.Changeset.for_update(record, :add_points, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :share ->
        Ash.update(Ash.Changeset.for_update(record, :share, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :add_points, :share]
      :add_points -> [:add_points, :update, :share]
      :update -> [:update, :add_points, :share]
      :share -> [:add_points, :update]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :add_points -> %{max_attempts: 1, backoff_ms: 0}
      :update -> %{max_attempts: 1, backoff_ms: 0}
      :share -> %{max_attempts: 1, backoff_ms: 0}
