defmodule UniboExPoc.Ecommerce.Workflows.Theme.ThemeLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Ecommerce.Theme

  def steps do
    [:create, :update, :apply, :switch]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","apply"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","apply"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","switch"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"apply"},{"idempotency_key":null,"next":["apply"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"switch"}],"workflow":"theme_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Theme, :create, params), ash_opts)
      :update ->
      ...
      :switch ->
        Ash.update(Ash.Changeset.for_update(record, :switch, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :switch -> %{max_attempts: 1, backoff_ms: 0}
