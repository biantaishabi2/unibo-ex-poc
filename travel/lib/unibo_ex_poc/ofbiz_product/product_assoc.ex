defmodule UniboExPoc.Ofbiz.Product.ProductAssoc do
  use Ash.Resource,
    otp_app: :unibo_ex_poc,
    domain: UniboExPoc.Ofbiz.Product,
    data_layer: AshPostgres.DataLayer,
    extensions: [AshGraphql.Resource, AshPaperTrail.Resource, AshArchival.Resource]

  postgres do
    table "product_assocs"
    repo UniboExPoc.Repo
  end

  graphql do
    type :product_product_assoc

    queries do
      get :get_product_product_assoc, :read
      list :list_product_product_assocs, :read
    end

    mutations do
      create :create_product_product_assoc, :create
      update :update_product_product_assoc, :update
      destroy :delete_product_product_assoc, :destroy
    end

  end

  attributes do
    attribute :from_date, :utc_datetime do
      allow_nil? false
      primary_key? true
      public? true
    end
    attribute :thru_date, :utc_datetime, public?: true
    attribute :sequence_num, :integer, public?: true
    attribute :reason, :string, public?: true
    attribute :quantity, :decimal, public?: true
    attribute :scrap_factor, :decimal, public?: true
    attribute :instruction, :string, public?: true
    attribute :routing_work_effort_id, :string, public?: true
    attribute :estimate_calc_method, :string, public?: true
    attribute :recurrence_info_id, :string, public?: true
    attribute :archived_at, :utc_datetime_usec, allow_nil?: true, public?: false
  end

  relationships do
    belongs_to :product_assoc_type, UniboExPoc.Ofbiz.Product.ProductAssocType do
      public? true
    end
    belongs_to :main_product, UniboExPoc.Ofbiz.Product.Product do
      public? true
      source_attribute :product_id
    end
    belongs_to :assoc_product, UniboExPoc.Ofbiz.Product.Product do
      public? true
      source_attribute :product_id_to
    end
  end

  actions do
    defaults [:read, :create, :update, :destroy]
  end

  paper_trail do
    change_tracking_mode :full_diff
    store_action_name? true
    ignore_attributes [:inserted_at, :updated_at]
  end

  archive do
  end

end
