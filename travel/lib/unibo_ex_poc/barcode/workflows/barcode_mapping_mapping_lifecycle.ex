defmodule UniboExPoc.Barcode.Workflows.BarcodeMapping.MappingLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Barcode.BarcodeMapping

  def steps do
    [:register, :update, :deactivate, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","deactivate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"register"},{"idempotency_key":null,"next":["deactivate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"deactivate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"mapping_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :register ->
        Ash.create(Ash.Changeset.for_create(BarcodeMapping, :register, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      :register -> [:update, :deactivate, :destroy]
      :update -> [:deactivate, :destroy]
      :deactivate -> [:update, :destroy]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :register -> %{max_attempts: 1, backoff_ms: 0}
