defmodule UniboExPoc.Marketing.Workflows.SmsMessage.SmsMessageLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Marketing.SmsMessage

  def steps do
    [:create, :send, :cancel, :resend_failed]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["send","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["resend_failed"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"send"},{"idempotency_key":null,"next":["resend_failed"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"resend_failed"}],"workflow":"sms_message_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SmsMessage, :create, params), ash_opts)
      :send ->
      ...
      :resend_failed ->
        Ash.update(Ash.Changeset.for_update(record, :resend_failed, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :resend_failed -> %{max_attempts: 1, backoff_ms: 0}
