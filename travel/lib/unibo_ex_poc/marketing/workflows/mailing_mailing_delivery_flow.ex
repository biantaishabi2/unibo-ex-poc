defmodule UniboExPoc.Marketing.Workflows.Mailing.MailingDeliveryFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Marketing.Mailing
...
{"steps":[{"idempotency_key":null,"next":["schedule"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["put_in_queue"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"schedule"},{"idempotency_key":null,"next":["launch"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"put_in_queue"},{"idempotency_key":null,"next":["retry_failed"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"launch"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"retry_failed"}],"workflow":"mailing_delivery_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Mailing, :create, params), ash_opts)
      :schedule ->
        Ash.update(Ash.Changeset.for_update(record, :schedule, params), ash_opts)
      :put_in_queue ->
        Ash.update(Ash.Changeset.for_update(record, :put_in_queue, params), ash_opts)
      :launch ->
        Ash.update(Ash.Changeset.for_update(record, :launch, params), ash_opts)
      :retry_failed ->
        Ash.update(Ash.Changeset.for_update(record, :retry_failed, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :put_in_queue -> %{max_attempts: 1, backoff_ms: 0}
      :launch -> %{max_attempts: 1, backoff_ms: 0}
      :retry_failed -> %{max_attempts: 1, backoff_ms: 0}
