defmodule UniboExPoc.Marketing.Workflows.MailingListMember.MailingListMemberLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Marketing.MailingListMember

  def steps do
    [:create, :unsubscribe, :subscribe]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["unsubscribe"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["subscribe"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unsubscribe"},{"idempotency_key":null,"next":["unsubscribe"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"subscribe"}],"workflow":"mailing_list_member_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(MailingListMember, :create, params), ash_opts)
      :unsubscribe ->
        Ash.update(Ash.Changeset.for_update(record, :unsubscribe, params), ash_opts)
      :subscribe ->
        Ash.update(Ash.Changeset.for_update(record, :subscribe, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :unsubscribe -> %{max_attempts: 1, backoff_ms: 0}
      :subscribe -> %{max_attempts: 1, backoff_ms: 0}
