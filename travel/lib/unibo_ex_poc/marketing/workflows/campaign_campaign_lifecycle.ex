...
{"steps":[{"idempotency_key":null,"next":["update","launch"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["launch"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["pause","complete","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"launch"},{"idempotency_key":null,"next":["resume","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"pause"},{"idempotency_key":null,"next":["pause","complete","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"resume"},{"idempotency_key":null,"next":["cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"complete"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"}],"workflow":"campaign_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Campaign, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :update -> [:launch]
      :launch -> [:pause, :complete, :cancel]
      :pause -> [:resume, :cancel]
      :resume -> [:pause, :complete, :cancel]
      :complete -> [:cancel]
      :cancel -> []
      _ -> []
    end
