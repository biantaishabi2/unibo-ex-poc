defmodule UniboExPoc.Studio.Workflows.App.AppLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Studio.App
...
{"steps":[{"idempotency_key":null,"next":["update","publish","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["publish","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["unpublish","update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"publish"},{"idempotency_key":null,"next":["publish","update","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unpublish"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"app_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(App, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :update -> [:publish, :destroy]
      :publish -> [:unpublish, :update]
      :unpublish -> [:publish, :update, :destroy]
      :destroy -> []
      _ -> []
    end
  end
