...
{"steps":[{"idempotency_key":null,"next":["confirm","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["start_repair","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"confirm"},{"idempotency_key":null,"next":["complete_repair","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start_repair"},{"idempotency_key":null,"next":["cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"complete_repair"},{"idempotency_key":null,"next":["reset_to_draft"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"},{"idempotency_key":null,"next":["confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reset_to_draft"}],"workflow":"repair_order_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(RepairOrder, :create, params), ash_opts)
      :confirm ->
        Ash.update(Ash.Changeset.for_update(record, :confirm, params), ash_opts)
      :start_repair ->
        Ash.update(Ash.Changeset.for_update(record, :start_repair, params), ash_opts)
      :complete_repair ->
        Ash.update(Ash.Changeset.for_update(record, :complete_repair, params), ash_opts)
      :cancel ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :confirm -> [:start_repair, :cancel]
      :start_repair -> [:complete_repair, :cancel]
      :complete_repair -> [:cancel]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :start_repair -> %{max_attempts: 1, backoff_ms: 0}
      :complete_repair -> %{max_attempts: 1, backoff_ms: 0}
      :cancel -> %{max_attempts: 1, backoff_ms: 0}
      :reset_to_draft -> %{max_attempts: 1, backoff_ms: 0}
