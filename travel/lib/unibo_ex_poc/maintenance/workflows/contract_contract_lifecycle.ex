defmodule UniboExPoc.Maintenance.Workflows.Contract.ContractLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Maintenance.Contract
...
{"steps":[{"idempotency_key":null,"next":["action_open"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_expire"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_open"},{"idempotency_key":null,"next":["action_close","action_draft"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_expire"},{"idempotency_key":null,"next":["action_draft"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_close"},{"idempotency_key":null,"next":["action_open"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_draft"}],"workflow":"contract_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Contract, :create, params), ash_opts)
      :action_open ->
        Ash.update(Ash.Changeset.for_update(record, :action_open, params), ash_opts)
      :action_expire ->
        Ash.update(Ash.Changeset.for_update(record, :action_expire, params), ash_opts)
      :action_close ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :action_expire -> [:action_close, :action_draft]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_open -> %{max_attempts: 1, backoff_ms: 0}
      :action_expire -> %{max_attempts: 1, backoff_ms: 0}
