...
{"steps":[{"idempotency_key":null,"next":["advance_stage"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["advance_stage","apply_changes","rebase"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"advance_stage"},{"idempotency_key":null,"next":["rebase"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"apply_changes"},{"idempotency_key":null,"next":["advance_stage"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"rebase"}],"workflow":"eco_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Eco, :create, params), ash_opts)
      :advance_stage ->
        Ash.update(Ash.Changeset.for_update(record, :advance_stage, params), ash_opts)
      :apply_changes ->
        Ash.update(Ash.Changeset.for_update(record, :apply_changes, params), ash_opts)
      :rebase ->
        Ash.update(Ash.Changeset.for_update(record, :rebase, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      ...
      :advance_stage -> [:advance_stage, :apply_changes, :rebase]
      :apply_changes -> [:rebase]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :advance_stage -> %{max_attempts: 1, backoff_ms: 0}
      :apply_changes -> %{max_attempts: 1, backoff_ms: 0}
      :rebase -> %{max_attempts: 1, backoff_ms: 0}
