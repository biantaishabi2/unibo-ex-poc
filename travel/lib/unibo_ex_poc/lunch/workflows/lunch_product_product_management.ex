...
{"steps":[{"idempotency_key":null,"next":["update","toggle_favorite","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","toggle_favorite","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["toggle_favorite","update","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"toggle_favorite"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"product_management"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LunchProduct, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :toggle_favorite, :destroy]
      :update -> [:update, :toggle_favorite, :destroy]
      :toggle_favorite -> [:toggle_favorite, :update, :destroy]
      :destroy -> []
      _ -> []
    end
  end
