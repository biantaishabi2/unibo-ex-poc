defmodule UniboExPoc.Gamification.Workflows.Challenge.ChallengeFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Gamification.Challenge

  def steps do
    [:create, :update, :action_start, :action_check, :action_done]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","action_start"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","action_start"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["action_check","action_done"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_start"},{"idempotency_key":null,"next":["action_check","action_done"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_check"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_done"}],"workflow":"challenge_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Challenge, :create, params), ash_opts)
      :update ->
      ...
      :action_check ->
        Ash.update(Ash.Changeset.for_update(record, :action_check, params), ash_opts)
      :action_done ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :update -> [:update, :action_start]
      :action_start -> [:action_check, :action_done]
      :action_check -> [:action_check, :action_done]
      :action_done -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_check -> %{max_attempts: 1, backoff_ms: 0}
