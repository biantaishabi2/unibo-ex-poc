...
{"steps":[{"idempotency_key":null,"next":["upload"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["lock"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"upload"},{"idempotency_key":null,"next":["unlock"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"lock"},{"idempotency_key":null,"next":["archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unlock"},{"idempotency_key":null,"next":["restore"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"},{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"restore"},{"idempotency_key":null,"next":["toggle_favorite"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"toggle_favorite"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"document_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Document, :create, params), ash_opts)
      :upload ->
        Ash.update(Ash.Changeset.for_update(record, :upload, params), ash_opts)
      :lock ->
        Ash.update(Ash.Changeset.for_update(record, :lock, params), ash_opts)
      :unlock ->
        Ash.update(Ash.Changeset.for_update(record, :unlock, params), ash_opts)
      :archive ->
        Ash.update(Ash.Changeset.for_update(record, :archive, params), ash_opts)
      :restore ->
        Ash.update(Ash.Changeset.for_update(record, :restore, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :toggle_favorite ->
        Ash.update(Ash.Changeset.for_update(record, :toggle_favorite, params), ash_opts)
      :destroy ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :upload -> %{max_attempts: 1, backoff_ms: 0}
      :lock -> %{max_attempts: 1, backoff_ms: 0}
      :unlock -> %{max_attempts: 1, backoff_ms: 0}
      :archive -> %{max_attempts: 1, backoff_ms: 0}
      :restore -> %{max_attempts: 1, backoff_ms: 0}
      :update -> %{max_attempts: 1, backoff_ms: 0}
      :toggle_favorite -> %{max_attempts: 1, backoff_ms: 0}
