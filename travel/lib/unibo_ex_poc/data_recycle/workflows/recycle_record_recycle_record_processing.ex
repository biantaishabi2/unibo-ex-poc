defmodule UniboExPoc.DataRecycle.Workflows.RecycleRecord.RecycleRecordProcessingWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.DataRecycle.RecycleRecord
...
{"steps":[{"idempotency_key":null,"next":["validate","discard"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["discard"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"validate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"discard"}],"workflow":"recycle_record_processing"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(RecycleRecord, :create, params), ash_opts)
      :validate ->
        Ash.update(Ash.Changeset.for_update(record, :validate, params), ash_opts)
      :discard ->
        Ash.update(Ash.Changeset.for_update(record, :discard, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :discard -> %{max_attempts: 1, backoff_ms: 0}
