...
{"steps":[{"idempotency_key":null,"next":["update","recycle_records_action","cron_recycle","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","recycle_records_action","cron_recycle","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","cron_recycle","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"recycle_records_action"},{"idempotency_key":null,"next":["update","recycle_records_action","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cron_recycle"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"recycle_model_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(RecycleModel, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :recycle_records_action ->
        Ash.update(Ash.Changeset.for_update(record, :recycle_records_action, params), ash_opts)
      :cron_recycle ->
        Ash.update(Ash.Changeset.for_update(record, :cron_recycle, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :recycle_records_action, :cron_recycle, :destroy]
      :update -> [:update, :recycle_records_action, :cron_recycle, :destroy]
      :recycle_records_action -> [:update, :cron_recycle, :destroy]
      :cron_recycle -> [:update, :recycle_records_action, :destroy]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :recycle_records_action -> %{max_attempts: 1, backoff_ms: 0}
      :cron_recycle -> %{max_attempts: 1, backoff_ms: 0}
