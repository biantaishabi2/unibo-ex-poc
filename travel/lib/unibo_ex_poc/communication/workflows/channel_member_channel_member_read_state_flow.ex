defmodule UniboExPoc.Communication.Workflows.ChannelMember.ChannelMemberReadStateFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Communication.ChannelMember

  def steps do
    [:create, :update, :mark_fetched, :mark_seen, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["mark_fetched"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["mark_seen"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_fetched"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_seen"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"channel_member_read_state_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(ChannelMember, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :mark_fetched ->
        Ash.update(Ash.Changeset.for_update(record, :mark_fetched, params), ash_opts)
      :mark_seen ->
        Ash.update(Ash.Changeset.for_update(record, :mark_seen, params), ash_opts)
      :destroy ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :mark_fetched -> %{max_attempts: 1, backoff_ms: 0}
      :mark_seen -> %{max_attempts: 1, backoff_ms: 0}
