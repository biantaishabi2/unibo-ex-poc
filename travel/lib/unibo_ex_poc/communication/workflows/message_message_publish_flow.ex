defmodule UniboExPoc.Communication.Workflows.Message.MessagePublishFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Communication.Message

  def steps do
    [:create, :update, :toggle_star, :send_transient, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["toggle_star"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["send_transient"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"toggle_star"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"send_transient"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"message_publish_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Message, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :toggle_star ->
        Ash.update(Ash.Changeset.for_update(record, :toggle_star, params), ash_opts)
      :send_transient ->
        Ash.update(Ash.Changeset.for_update(record, :send_transient, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update]
      :update -> [:toggle_star]
      :toggle_star -> [:send_transient]
      :send_transient -> [:destroy]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :toggle_star -> %{max_attempts: 1, backoff_ms: 0}
      :send_transient -> %{max_attempts: 1, backoff_ms: 0}
