defmodule UniboExPoc.ELearning.Workflows.SlideProgress.ProgressTrackingWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.ELearning.SlideProgress
...
{"steps":[{"idempotency_key":null,"next":["mark_completed","complete_quiz","action_like","action_dislike"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["mark_uncompleted","action_like","action_dislike"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_completed"},{"idempotency_key":null,"next":["mark_completed","complete_quiz","action_like","action_dislike"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_uncompleted"},{"idempotency_key":null,"next":["mark_uncompleted","action_like","action_dislike"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"complete_quiz"},{"idempotency_key":null,"next":["action_like","action_dislike"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_like"},{"idempotency_key":null,"next":["action_like","action_dislike"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_dislike"}],"workflow":"progress_tracking"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SlideProgress, :create, params), ash_opts)
      :mark_completed ->
        Ash.update(Ash.Changeset.for_update(record, :mark_completed, params), ash_opts)
      :mark_uncompleted ->
        Ash.update(Ash.Changeset.for_update(record, :mark_uncompleted, params), ash_opts)
      :complete_quiz ->
        Ash.update(Ash.Changeset.for_update(record, :complete_quiz, params), ash_opts)
      :action_like ->
        Ash.update(Ash.Changeset.for_update(record, :action_like, params), ash_opts)
      :action_dislike ->
        Ash.update(Ash.Changeset.for_update(record, :action_dislike, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:mark_completed, :complete_quiz, :action_like, :action_dislike]
      :mark_completed -> [:mark_uncompleted, :action_like, :action_dislike]
      :mark_uncompleted -> [:mark_completed, :complete_quiz, :action_like, :action_dislike]
      :complete_quiz -> [:mark_uncompleted, :action_like, :action_dislike]
      :action_like -> [:action_like, :action_dislike]
      :action_dislike -> [:action_like, :action_dislike]
      _ -> []
    end
  end
  ...
  defp step_skipped?(step, record) do
    case step do
      ...
      :mark_uncompleted -> false
      :complete_quiz -> false
      :action_like -> false
      :action_dislike -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :mark_uncompleted -> %{max_attempts: 1, backoff_ms: 0}
      :complete_quiz -> %{max_attempts: 1, backoff_ms: 0}
      :action_like -> %{max_attempts: 1, backoff_ms: 0}
      :action_dislike -> %{max_attempts: 1, backoff_ms: 0}
