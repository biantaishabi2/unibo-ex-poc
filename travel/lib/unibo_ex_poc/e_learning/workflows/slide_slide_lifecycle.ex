defmodule UniboExPoc.ELearning.Workflows.Slide.SlideLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.ELearning.Slide
...
{"steps":[{"idempotency_key":null,"next":["update","publish","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","publish","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["unpublish","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"publish"},{"idempotency_key":null,"next":["update","publish","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unpublish"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"slide_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Slide, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :publish, :destroy]
      :update -> [:update, :publish, :destroy]
      :publish -> [:unpublish, :destroy]
      :unpublish -> [:update, :publish, :destroy]
      :destroy -> []
      _ -> []
    end
  end
