defmodule UniboExPoc.ELearning.Workflows.Course.CourseLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.ELearning.Course
...
{"steps":[{"idempotency_key":null,"next":["update","publish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","publish","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["unpublish","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"publish"},{"idempotency_key":null,"next":["update","publish","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unpublish"},{"idempotency_key":null,"next":["restore"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"},{"idempotency_key":null,"next":["update","publish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"restore"}],"workflow":"course_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Course, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :update -> [:update, :publish, :archive]
      :publish -> [:unpublish, :archive]
      :unpublish -> [:update, :publish, :archive]
      :archive -> [:restore]
      :restore -> [:update, :publish]
      _ -> []
    end
  end
