defmodule UniboExPoc.Spreadsheet.Workflows.SpreadsheetDocument.SpreadsheetDocumentLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Spreadsheet.SpreadsheetDocument

  def steps do
    [:create, :update, :create_from_template, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["create_from_template"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create_from_template"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"spreadsheet_document_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SpreadsheetDocument, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :create_from_template ->
        Ash.create(Ash.Changeset.for_create(SpreadsheetDocument, :create_from_template, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update]
      :update -> [:create_from_template]
      :create_from_template -> [:destroy]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :create_from_template -> %{max_attempts: 1, backoff_ms: 0}
