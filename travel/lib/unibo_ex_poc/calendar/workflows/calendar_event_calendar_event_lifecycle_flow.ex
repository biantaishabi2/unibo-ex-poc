...
{"steps":[{"idempotency_key":null,"next":["update","confirm","cancel","revert_to_draft","mark_tentative","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["confirm","cancel","revert_to_draft","mark_tentative","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","cancel","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"confirm"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"},{"idempotency_key":null,"next":["confirm","cancel","mark_tentative","update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"revert_to_draft"},{"idempotency_key":null,"next":["confirm","cancel","revert_to_draft","update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_tentative"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"calendar_event_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(CalendarEvent, :create, params), ash_opts)
      :update ->
      ...
      :revert_to_draft ->
        Ash.update(Ash.Changeset.for_update(record, :revert_to_draft, params), ash_opts)
      :mark_tentative ->
        Ash.update(Ash.Changeset.for_update(record, :mark_tentative, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :confirm, :cancel, :revert_to_draft, :mark_tentative, :destroy]
      :update -> [:confirm, :cancel, :revert_to_draft, :mark_tentative, :destroy]
      :confirm -> [:update, :cancel, :destroy]
      :cancel -> [:destroy]
      :revert_to_draft -> [:confirm, :cancel, :mark_tentative, :update]
      :mark_tentative -> [:confirm, :cancel, :revert_to_draft, :update]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :revert_to_draft -> %{max_attempts: 1, backoff_ms: 0}
      :mark_tentative -> %{max_attempts: 1, backoff_ms: 0}
