...
{"steps":[{"idempotency_key":null,"next":["update","activate","deactivate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["activate","deactivate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","deactivate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":["activate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"deactivate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"work_schedule_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(WorkSchedule, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :activate, :deactivate, :destroy]
      :update -> [:activate, :deactivate, :destroy]
      :activate -> [:update, :deactivate]
      :deactivate -> [:activate, :destroy]
      :destroy -> []
      _ -> []
    end
  end
