defmodule UniboExPoc.Calendar.Workflows.Attendee.AttendeeResponseFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Calendar.Attendee

  def steps do
    [:invite, :update_response, :remove]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update_response","remove"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"invite"},{"idempotency_key":null,"next":["update_response","remove"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update_response"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"remove"}],"workflow":"attendee_response_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :invite ->
        Ash.create(Ash.Changeset.for_create(Attendee, :invite, params), ash_opts)
      :update_response ->
        Ash.update(Ash.Changeset.for_update(record, :update_response, params), ash_opts)
      :remove ->
        Ash.destroy(Ash.Changeset.for_destroy(record, :remove, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :invite -> [:update_response, :remove]
      :update_response -> [:update_response, :remove]
      :remove -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :invite -> %{max_attempts: 1, backoff_ms: 0}
      :update_response -> %{max_attempts: 1, backoff_ms: 0}
      :remove -> %{max_attempts: 1, backoff_ms: 0}
