defmodule UniboExPoc.Website.Workflows.WebPage.PageLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Website.WebPage

  def steps do
    [:create, :update, :publish, :unpublish, :clone_page]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","publish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","publish","unpublish","clone_page"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","unpublish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"publish"},{"idempotency_key":null,"next":["update","publish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unpublish"},{"idempotency_key":null,"next":["update","publish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"clone_page"}],"workflow":"page_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(WebPage, :create, params), ash_opts)
      :update ->
      ...
      :clone_page ->
        Ash.create(Ash.Changeset.for_create(WebPage, :clone_page, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      ...
      :update -> [:update, :publish, :unpublish, :clone_page]
      :publish -> [:update, :unpublish]
      :unpublish -> [:update, :publish]
      :clone_page -> [:update, :publish]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :clone_page -> %{max_attempts: 1, backoff_ms: 0}
