defmodule UniboExPoc.Delivery.Workflows.ShipmentStatus.ShipmentStatusRecordFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Delivery.ShipmentStatus
...
{"steps":[{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"}],"workflow":"shipment_status_record_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(ShipmentStatus, :create, params), ash_opts)
      _ -> {:ok, record}
    end
  end
