defmodule UniboExPoc.Purchasing.Workflows.PurchaseOrder.PurchaseOrderEditingWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
...
{"steps":[{"idempotency_key":null,"next":["update"],"next_step_ids":["s2_update"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["update","send_rfq","button_confirm"],"next_step_ids":["s2_update","send_rfq","button_confirm"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update","step_id":"s2_update"}],"workflow":"purchase_order_editing"}
"""
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_update]
      :s2_update -> [:s2_update, :send_rfq, :button_confirm]
      _ -> []
    end
  end
