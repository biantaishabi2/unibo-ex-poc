...
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","send_rfq","button_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"}],"workflow":"purchase_order_editing"}
"""
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update]
      :update -> [:update, :send_rfq, :button_confirm]
      _ -> []
    end
  end
