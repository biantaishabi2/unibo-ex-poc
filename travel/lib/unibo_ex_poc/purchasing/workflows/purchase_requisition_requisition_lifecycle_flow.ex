defmodule UniboExPoc.Purchasing.Workflows.PurchaseRequisition.RequisitionLifecycleFlowWorkflow do
  ...
  def steps do
    [:s1_create, :s2_action_in_progress, :s3_action_open, :s4_action_done, :s5_action_cancel]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["action_in_progress"],"next_step_ids":["s2_action_in_progress"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["action_open"],"next_step_ids":["s3_action_open"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_in_progress","step_id":"s2_action_in_progress"},{"idempotency_key":null,"next":["action_done"],"next_step_ids":["s4_action_done"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_open","step_id":"s3_action_open"},{"idempotency_key":null,"next":["action_cancel"],"next_step_ids":["s5_action_cancel"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_done","step_id":"s4_action_done"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_cancel","step_id":"s5_action_cancel"}],"workflow":"requisition_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(PurchaseRequisition, :create, params), ash_opts)
      :s2_action_in_progress ->
        Ash.update(Ash.Changeset.for_update(record, :action_in_progress, params), ash_opts)
      :s3_action_open ->
        Ash.update(Ash.Changeset.for_update(record, :action_open, params), ash_opts)
      :s4_action_done ->
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_action_in_progress]
      :s2_action_in_progress -> [:s3_action_open]
      :s3_action_open -> [:s4_action_done]
      :s4_action_done -> [:s5_action_cancel]
      :s5_action_cancel -> []
      _ -> []
    end
  end
  ...
  defp branch_next(step, _record) do
    case step do
      :s1_create -> nil
      :s2_action_in_progress -> nil
      :s3_action_open -> nil
      :s4_action_done -> nil
      :s5_action_cancel -> nil
      _ -> nil
    end
  end
  ...
  defp step_skipped?(step, _record) do
    case step do
      :s1_create -> false
      :s2_action_in_progress -> false
      :s3_action_open -> false
      :s4_action_done -> false
      :s5_action_cancel -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s2_action_in_progress -> %{max_attempts: 1, backoff_ms: 0}
      :s3_action_open -> %{max_attempts: 1, backoff_ms: 0}
      :s4_action_done -> %{max_attempts: 1, backoff_ms: 0}
      :s5_action_cancel -> %{max_attempts: 1, backoff_ms: 0}
