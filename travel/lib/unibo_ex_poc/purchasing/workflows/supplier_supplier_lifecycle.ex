defmodule UniboExPoc.Purchasing.Workflows.Supplier.SupplierLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Purchasing.Supplier

  def steps do
    [:s1_create, :s2_update, :s3_activate, :s4_block]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","block"],"next_step_ids":["s2_update","s4_block"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["update","activate","block"],"next_step_ids":["s2_update","s3_activate","s4_block"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update","step_id":"s2_update"},{"idempotency_key":null,"next":["update","block"],"next_step_ids":["s2_update","s4_block"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate","step_id":"s3_activate"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"block","step_id":"s4_block"}],"workflow":"supplier_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(Supplier, :create, params), ash_opts)
      :s2_update ->
      ...
      :s4_block ->
        Ash.update(Ash.Changeset.for_update(record, :block, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_update, :s4_block]
      :s2_update -> [:s2_update, :s3_activate, :s4_block]
      :s3_activate -> [:s2_update, :s4_block]
      :s4_block -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s4_block -> %{max_attempts: 1, backoff_ms: 0}
