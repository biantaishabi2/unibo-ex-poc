defmodule UniboExPoc.Purchasing.Workflows.Supplier.SupplierLifecycleWorkflow do
  ...
  def steps do
    [:create, :update, :activate, :block]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","block"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","activate","block"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","block"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"block"}],"workflow":"supplier_lifecycle"}
"""
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :block]
      :update -> [:update, :activate, :block]
      :activate -> [:update, :block]
      :block -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :block -> %{max_attempts: 1, backoff_ms: 0}
