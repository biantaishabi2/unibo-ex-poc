defmodule UniboExPoc.Purchasing.Workflows.GoodsReceipt.GoodsReceiptLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Purchasing.GoodsReceipt

  def steps do
    [:s1_create, :s2_confirm, :s3_cancel]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["confirm"],"next_step_ids":["s2_confirm"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["cancel"],"next_step_ids":["s3_cancel"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"confirm","step_id":"s2_confirm"},{"idempotency_key":null,"next":[],"next_step_ids":[],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel","step_id":"s3_cancel"}],"workflow":"goods_receipt_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(GoodsReceipt, :create, params), ash_opts)
      :s2_confirm ->
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_confirm]
      :s2_confirm -> [:s3_cancel]
      :s3_cancel -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s2_confirm -> %{max_attempts: 1, backoff_ms: 0}
