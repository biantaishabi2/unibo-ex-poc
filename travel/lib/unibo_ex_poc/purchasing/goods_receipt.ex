...
defmodule UniboExPoc.Purchasing.GoodsReceipt do
  ...
  postgres do
    ...
    identity_index_names unique_receipt_number: "idx_purchasing_goods_receipts_unique_receipt_number"
  end
  ...
  actions do
    defaults [:read]
    create :create do
      description "Create Goods Receipt via Create. doc_url: graphql://contract/purchasing/create_purchasing_goods_receipt"
      primary? true
...
确认收货. doc_url: graphql://contract/purchasing/confirm_purchasing_goods_receipt"
      primary? true
      accept []
...
取消收货. doc_url: graphql://contract/purchasing/cancel_purchasing_goods_receipt"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :status)
        if current == :draft do
