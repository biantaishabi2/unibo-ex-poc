...
defmodule UniboExPoc.Purchasing.PurchaseRequisition do
  ...
  postgres do
    ...
    identity_index_names unique_requisition_name: "idx_purchasing_purchase_requisitions_unique_requisition_name"
  end
  ...
  actions do
    defaults [:read]
    create :create do
      description "Create Purchase Requisition via Create. doc_url: graphql://contract/purchasing/create_purchasing_purchase_requisition"
      primary? true
...
确认协议（draft → in_progress / ongoing，Blanket Order 校验价格和数量）. doc_url: graphql://contract/purchasing/action_in_progress_purchasing_purchase_requisition"
      primary? true
      accept []
...
进入投标选择（in_progress/ongoing → open）. doc_url: graphql://contract/purchasing/action_open_purchasing_purchase_requisition"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :state)
...
关闭协议（open → done，检查无未确认的 RFQ）. doc_url: graphql://contract/purchasing/action_done_purchasing_purchase_requisition"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :state)
        if current == :open do
...
取消协议及关联报价单. doc_url: graphql://contract/purchasing/action_cancel_purchasing_purchase_requisition"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :state)
...
重置为草稿（cancel → draft）. doc_url: graphql://contract/purchasing/action_draft_purchasing_purchase_requisition"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :state)
