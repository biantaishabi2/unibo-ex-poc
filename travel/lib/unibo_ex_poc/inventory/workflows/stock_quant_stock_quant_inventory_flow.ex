...
{"steps":[{"idempotency_key":null,"next":["apply_inventory"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"apply_inventory"}],"workflow":"stock_quant_inventory_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      ...
      :apply_inventory ->
        Ash.update(Ash.Changeset.for_update(record, :apply_inventory, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :apply_inventory -> %{max_attempts: 1, backoff_ms: 0}
