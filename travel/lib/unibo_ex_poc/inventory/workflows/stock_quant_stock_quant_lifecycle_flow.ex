defmodule UniboExPoc.Inventory.Workflows.StockQuant.StockQuantLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  ...
  def steps do
    [:create, :update, :update_available_quantity]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update_available_quantity"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update_available_quantity"}],"workflow":"stock_quant_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(StockQuant, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :update_available_quantity ->
        Ash.update(Ash.Changeset.for_update(record, :update_available_quantity, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update]
      :update -> [:update_available_quantity]
      :update_available_quantity -> []
      _ -> []
    end
  end
  ...
  defp step_skipped?(step, record) do
    case step do
      :create -> false
      :update -> false
      :update_available_quantity -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :update_available_quantity -> %{max_attempts: 1, backoff_ms: 0}
