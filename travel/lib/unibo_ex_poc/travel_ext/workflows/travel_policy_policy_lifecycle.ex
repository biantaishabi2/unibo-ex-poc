defmodule UniboExPoc.TravelExt.Workflows.TravelPolicy.PolicyLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.TravelExt.TravelPolicy

  def steps do
    [:s1_create, :s2_update, :s3_deactivate, :s4_activate]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","deactivate"],"next_step_ids":["s2_update","s3_deactivate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["deactivate"],"next_step_ids":["s3_deactivate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update","step_id":"s2_update"},{"idempotency_key":null,"next":["activate"],"next_step_ids":["s4_activate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"deactivate","step_id":"s3_deactivate"},{"idempotency_key":null,"next":["update","deactivate"],"next_step_ids":["s2_update","s3_deactivate"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate","step_id":"s4_activate"}],"workflow":"policy_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(TravelPolicy, :create, params), ash_opts)
      :s2_update ->
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_update, :s3_deactivate]
      :s2_update -> [:s3_deactivate]
      :s3_deactivate -> [:s4_activate]
      :s4_activate -> [:s2_update, :s3_deactivate]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s3_deactivate -> %{max_attempts: 1, backoff_ms: 0}
      :s4_activate -> %{max_attempts: 1, backoff_ms: 0}
