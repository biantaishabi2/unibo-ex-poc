...
#   approve_cancel --> [*]
# ```
defmodule UniboExPoc.TravelExt.TravelRefundOrder do
  use Ash.Resource,
    otp_app: :travel,
  ...
  postgres do
    table "travel_ext_travel_refund_orders"
    repo UniboExPoc.Repo
    identity_index_names unique_order_status: "idx_travel_ext_travel_refund_orders_unique_order_status"
  end

  graphql do
    type :travel_ext_travel_refund_order

    queries do
      get :get_travel_ext_travel_refund_order, :read
      list :list_travel_ext_travel_refund_orders, :read
    end

    mutations do
      create :create_travel_ext_travel_refund_order, :create
      update :approve_travel_ext_travel_refund_order, :approve
      update :reject_travel_ext_travel_refund_order, :reject
      update :refund_travel_ext_travel_refund_order, :refund
      update :refund_direct_travel_ext_travel_refund_order, :refund_direct
    end
  ...
  actions do
    defaults [:read]
    create :create do
      description "Create Travel Refund Order via Create. doc_url: graphql://contract/travel_ext/create_travel_ext_travel_refund_order"
      primary? true
    ...
    update :approve do
      description "Update Travel Refund Order via Approve. doc_url: graphql://contract/travel_ext/approve_travel_ext_travel_refund_order"
      primary? true
      accept []
    ...
    update :reject do
      description "Update Travel Refund Order via Reject. doc_url: graphql://contract/travel_ext/reject_travel_ext_travel_refund_order"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :status)
        if current == :pending do
    ...
    update :refund do
      description "Update Travel Refund Order via Refund. doc_url: graphql://contract/travel_ext/refund_travel_ext_travel_refund_order"
      accept []
      change fn changeset, _ctx ->
        current = Ash.Changeset.get_attribute(changeset, :status)
    ...
    update :refund_direct do
      description "Update Travel Refund Order via Refund Direct. doc_url: graphql://contract/travel_ext/refund_direct_travel_ext_travel_refund_order"
      accept []
      change fn changeset, _ctx ->
