defmodule UniboExPocWeb.Pages.Travel.ApprovalCategoryConfigLive do
  @moduledoc """
  ...
  @page_id "approval_category_config"
  ...
  @backend_embedded_page %{page_id: "approval_category_config", page_kind: "detail", api_map: %{get: "Travel.TravelPolicyCheck.get", list: "Travel.TravelPolicyCheck.list"}, backend: %{load: %{selection: "id"}}, route: %{path: "/pages/travel/approval_category_config", query: "", kind: "detail"}, state_schema: %{defaults: %{record: %{site_name: "", logo: "", site_description: "", notify_method: "", notify_frequency: "", session_timeout: "", password_policy: ""}}}, status_keys: []}
  ...
  defp __sync_entity_assign(socket, dto) when is_map(dto) do
    case dto do
      %{travel_policy_check: entity} when is_map(entity) and map_size(entity) > 0 -> assign(socket, :travel_policy_check, entity)
      %{record: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
          true -> assign(socket, :travel_policy_check, entity)
          false -> socket
        end
      _ ->
        source =
        ...
        if map_size(source) == 0 do
          ...
          entity = Map.get(socket.assigns, :travel_policy_check, %{})
          entity = if is_map(entity), do: Map.merge(entity, source), else: source
          assign(socket, :travel_policy_check, entity)
        end
    end
  end
  ...
  defp __sync_record_alias(socket) do
    case socket.assigns do
      %{travel_policy_check: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
  ...
  def render(assigns) do
    ~H"""
    <.page title="详情" id="approval_category_config">
