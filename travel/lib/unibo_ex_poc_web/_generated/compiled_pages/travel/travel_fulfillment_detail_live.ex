defmodule UniboExPocWeb.Pages.Travel.TravelFulfillmentDetailLive do
  @moduledoc """
  ...
  @backend_params_accept ["id", "travel_order_id", "supplier_booking_ref", "fulfillment_type", "confirmation_payload", "failure_reason", "voucher_or_ticket_ref", "ticket_refs"]
  ...
  @backend_api_map %{
    ...
    "complete_fulfillment" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.complete_fulfillment"},
    "confirm_booking" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.confirm_booking"},
    "create" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.create"},
    "destroy" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.destroy"},
    "fail_fulfillment" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.fail_fulfillment"},
    "get" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.get"},
    "issue_voucher_or_ticket" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.issue_voucher_or_ticket"},
  ...
  defp __sync_entity_assign(socket, dto) when is_map(dto) do
    case dto do
      %{travel_fulfillment: entity} when is_map(entity) and map_size(entity) > 0 -> assign(socket, :travel_fulfillment, entity)
      %{record: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
          true -> assign(socket, :travel_fulfillment, entity)
          false -> socket
        end
      _ ->
        source =
        ...
        if map_size(source) == 0 do
          ...
          entity = Map.get(socket.assigns, :travel_fulfillment, %{})
          entity = if is_map(entity), do: Map.merge(entity, source), else: source
          assign(socket, :travel_fulfillment, entity)
        end
    end
  end
  ...
  defp __sync_record_alias(socket) do
    case socket.assigns do
      %{travel_fulfillment: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
  ...
  defp __apply_transitions("action_cancel_fulfillment", params, assigns) do
  assigns = assigns
  effects = []
  ...
  defp __apply_transitions("action_complete_fulfillment", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
  ...
  defp __apply_transitions("action_confirm_booking", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_fulfillment", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end

  defp __apply_transitions("action_fail_fulfillment", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
  ...
  defp __apply_transitions("action_issue_voucher_or_ticket", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
  ...
  defp __apply_transitions("action_mark_in_use", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
