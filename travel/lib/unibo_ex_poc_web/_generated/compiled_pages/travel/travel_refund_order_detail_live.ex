defmodule UniboExPocWeb.Pages.Travel.TravelRefundOrderDetailLive do
  @moduledoc """
  ...
  # status.keys preview (first ~40): editing, record, record.status, travel_refund_order, travel_refund_order.approval_mode, travel_refund_order.refund_amount, travel_refund_order.refund_fee, travel_refund_order.refund_reason, travel_refund_order.status
  ...
  @backend_params_accept ["id", "original_order_id", "approval_mode", "refund_reason", "refund_fee", "refund_amount"]
  ...
  @backend_api_map %{
    ...
    "create" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRefundOrder.create"},
    "get" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRefundOrder.get"},
    "refund" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRefundOrder.refund"},
    "refund_direct" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRefundOrder.refund_direct"},
    "reject_refund" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelRefundOrder.reject_refund"},
  ...
  @impl true
  def handle_event("action_confirm_refund", params, socket) do
    # UI action event name: action_confirm_refund
    socket = dispatch_backend("action_confirm_refund", params, socket)
    {:noreply, socket}
  end
  ...
  @impl true
  def handle_event("action_refund", params, socket) do
    # UI action event name: action_refund
    socket = dispatch_backend("action_refund", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("action_refund_direct", params, socket) do
    # UI action event name: action_refund_direct
    socket = dispatch_backend("action_refund_direct", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("action_reject_refund", params, socket) do
    # UI action event name: action_reject_refund
    socket = dispatch_backend("action_reject_refund", params, socket)
    {:noreply, socket}
  end
    ...
    if has_id do
      ...
      id =
        Map.get(params, "id") ||
          ...
          get_in(socket.assigns, [:record, "id"]) ||
          get_in(socket.assigns, [:travel_refund_order, :id]) ||
          get_in(socket.assigns, [:travel_refund_order, "id"])
      case id do
  ...
  defp __sync_entity_assign(socket, dto) when is_map(dto) do
    case dto do
      %{travel_refund_order: entity} when is_map(entity) and map_size(entity) > 0 -> assign(socket, :travel_refund_order, entity)
      %{record: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
          true -> assign(socket, :travel_refund_order, entity)
          false -> socket
        end
      _ ->
        source =
        ...
        if map_size(source) == 0 do
          ...
          entity = Map.get(socket.assigns, :travel_refund_order, %{})
          entity = if is_map(entity), do: Map.merge(entity, source), else: source
          assign(socket, :travel_refund_order, entity)
        end
    end
  end
  ...
  defp __sync_record_alias(socket) do
    case socket.assigns do
      %{travel_refund_order: entity} when is_map(entity) ->
        case __assign_map_present?(entity) do
  ...
  defp __apply_transitions("action_confirm_refund", params, assigns) do
  assigns = assigns
  effects = []
  ...
  assigns = assigns
  effects = []
  effects = [%{type: "navigate", to: "/pages/travel/travel_refund_order"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end

  defp __apply_transitions("action_refund", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
  ...
  defp __apply_transitions("action_refund_direct", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
  ...
  defp __apply_transitions("action_reject_refund", params, assigns) do
  assigns = assigns
  effects = []
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
  ...
  def render(assigns) do
    ~H"""
    <.page title="详情" id="travel_refund_order_detail">
        ...
        <.flex gap={3} align="center" direction="row" id="title_group">
          <.stack direction="column" id="title_meta">
            ...
            <.text variant="h1" id="page_title">
              退票/退订单,记录针对已有 TravelOrder 的退票请求、手续费与退款状态;审批通过 overlay on Approvals 域
            </.text>
          </.stack>
          <%= case get_in(@travel_refund_order, [:status]) do %>
            <% "pending" -> %>
            ...
            <%= if !(@editing && @editing != []) do %>
              <.grid columns={2} gap={4} id="info_grid">
                <.grid columns={2} gap={3} id="detail_fields">
                  <.stack gap={0} direction="column" id="detail_fields_refund_reason">
                    <.text variant="caption" color="muted" id="detail_fields_refund_reason_label">
                      退票原因
                    </.text>
                    <.text id="detail_fields_refund_reason_value">
                      <%= get_in(@travel_refund_order, [:refund_reason]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_refund_fee">
                    <.text variant="caption" color="muted" id="detail_fields_refund_fee_label">
                      退票手续费
                    </.text>
                    <.text id="detail_fields_refund_fee_value">
                      <%= get_in(@travel_refund_order, [:refund_fee]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_refund_amount">
                    <.text variant="caption" color="muted" id="detail_fields_refund_amount_label">
                      实退金额
                    </.text>
                    <.text id="detail_fields_refund_amount_value">
                      <%= get_in(@travel_refund_order, [:refund_amount]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_status">
                    ...
                    <.text id="detail_fields_status_value">
                      <%= get_in(@travel_refund_order, [:status]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                  <.stack gap={0} direction="column" id="detail_fields_approval_mode">
                    ...
                    <.text id="detail_fields_approval_mode_value">
                      <%= get_in(@travel_refund_order, [:approval_mode]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                  </.stack>
                </.grid>
              </.grid>
            <% else %>
              <.form for={%{}} phx-submit="form_submit" phx-change="form_change" id="travel_refund_order_edit_form">
                <.grid columns={2} gap={4} id="form_grid">
                  <.stack direction="column" id="form_fields">
                    <.input size="md" name="travel_refund_order[refund_reason]" id="travel_refund_order_form_refund_reason" value={get_in(@travel_refund_order, [:refund_reason]) || ""} />
                    <.input size="md" name="travel_refund_order[refund_fee]" id="travel_refund_order_form_refund_fee" value={get_in(@travel_refund_order, [:refund_fee]) || ""} />
                    <.input size="md" name="travel_refund_order[refund_amount]" id="travel_refund_order_form_refund_amount" value={get_in(@travel_refund_order, [:refund_amount]) || ""} />
                    <.select size="md" name="travel_refund_order[approval_mode]" id="travel_refund_order_form_approval_mode" :let={builder} value={get_in(@travel_refund_order, [:approval_mode]) || ""}>
