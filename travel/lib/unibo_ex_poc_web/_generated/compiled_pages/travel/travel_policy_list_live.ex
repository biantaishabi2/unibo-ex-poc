defmodule UniboExPocWeb.Pages.Travel.TravelPolicyListLive do
  @moduledoc """
  ...
  # status.keys preview (first ~40): rows, rows[], rows[].approval_mode, rows[].cabin_class_limit, rows[].city_tier, rows[].employee_level, rows[].exceed_strategy, rows[].hotel_star_limit, rows[].id, rows[].is_active, rows[].max_amount, rows[].personal_pay_ratio, rows[].policy_name, rows[].product_type, rows[].season, rows_empty, travel_policy, travel_policy.page, travel_policy.total_pages
  ...
  @backend_params_accept ["product_type", "exceed_strategy", "approval_mode"]
  @backend_info_reload_messages []
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelPolicy.list"}
  }
  @backend_embedded_page %{page_id: "travel_policy_list", page_kind: "list", api_map: %{list: "Travel.TravelPolicy.list"}, backend: %{load: %{selection: "approval_mode: approvalMode cabin_class_limit: cabinClassLimit city_tier: cityTier employee_level: employeeLevel enterprise_id: enterpriseId exceed_strategy: exceedStrategy hotel_star_limit: hotelStarLimit id is_active: isActive max_amount: maxAmount personal_pay_ratio: personalPayRatio policy_name: policyName product_type: productType season"}}, route: %{path: "/pages/travel/travel_policy", query: "", kind: "list"}, state_schema: %{defaults: %{travel_policy: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", policy_name: "", product_type: "", employee_level: "", city_tier: "", season: "", max_amount: "", cabin_class_limit: "", hotel_star_limit: "", exceed_strategy: "", approval_mode: "", personal_pay_ratio: "", is_active: ""}, %{id: "row_02", policy_name: "", product_type: "", employee_level: "", city_tier: "", season: "", max_amount: "", cabin_class_limit: "", hotel_star_limit: "", exceed_strategy: "", approval_mode: "", personal_pay_ratio: "", is_active: ""}, %{id: "row_03", policy_name: "", product_type: "", employee_level: "", city_tier: "", season: "", max_amount: "", cabin_class_limit: "", hotel_star_limit: "", exceed_strategy: "", approval_mode: "", personal_pay_ratio: "", is_active: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_policy/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_policy_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              <.table_head id="th_policy_name">
                政策名称,如'总部差旅标准'
              </.table_head>
              <.table_head id="th_product_type">
                适用商品类型
              </.table_head>
              <.table_head id="th_employee_level">
                职级标识
              </.table_head>
              <.table_head id="th_city_tier">
                城市等级（tier_1/tier_2/tier_3）
              </.table_head>
              <.table_head id="th_season">
                淡旺季标识（可选）
              </.table_head>
              <.table_head id="th_max_amount">
                金额上限（单位分）
              </.table_head>
              <.table_head id="th_cabin_class_limit">
                舱位限制
              </.table_head>
              <.table_head id="th_hotel_star_limit">
                酒店星级限制
              </.table_head>
              <.table_head id="th_exceed_strategy">
                超标处理策略
              </.table_head>
              <.table_head id="th_approval_mode">
                审批模式;none 表示关闭审批,self/oa 表示进入对应审批流
              </.table_head>
              <.table_head id="th_personal_pay_ratio">
                个人支付比例 0-100
              </.table_head>
              <.table_head id="th_is_active">
                是否启用
              </.table_head>
            </.table_row>
          </.table_header>
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_policy_row">
                <.table_cell id="td_policy_name">
                  <%= get_in(row, [:policy_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                ...
                <.table_cell id="td_employee_level">
                  <%= get_in(row, [:employee_level]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_city_tier">
                  <%= get_in(row, [:city_tier]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_season">
                  <%= get_in(row, [:season]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_max_amount">
                  <%= get_in(row, [:max_amount]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_cabin_class_limit">
                  <%= get_in(row, [:cabin_class_limit]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_hotel_star_limit">
                  <%= get_in(row, [:hotel_star_limit]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_exceed_strategy">
                  <%= if (val = get_in(row, [:exceed_strategy])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_exceed_strategy">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                ...
                <.table_cell id="td_personal_pay_ratio">
                  <%= get_in(row, [:personal_pay_ratio]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_is_active">
                  <%= if (val = get_in(row, [:is_active])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_is_active">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_policy_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_policy, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_policy, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_policy_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
