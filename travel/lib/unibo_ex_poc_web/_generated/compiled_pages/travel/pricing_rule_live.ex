defmodule UniboExPocWeb.Pages.Travel.PricingRuleLive do
  @moduledoc """
  ...
  @page_id "pricing_rule"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): pricing_rules, pricing_rules[], pricing_rules[].markup_rate, pricing_rules[].product_type, pricing_rules[].status, pricing_rules[].supplier_code
  ...
  @backend_api_map %{}
  @backend_embedded_page %{page_id: "pricing_rule", page_kind: "custom", api_map: %{}, backend: %{load: %{selection: "id"}}, route: %{path: "/pages/travel/pricing_rule", query: "", kind: "custom"}, state_schema: %{defaults: %{pricing_rules: [%{product_type: "", supplier_code: "", markup_rate: "", status: ""}, %{product_type: "", supplier_code: "", markup_rate: "", status: ""}, %{product_type: "", supplier_code: "", markup_rate: "", status: ""}]}}, status_keys: []}
  ...
  def handle_event("create", params, socket) do
    # UI action event name: create
    socket = dispatch_backend("create", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("destroy", params, socket) do
    # UI action event name: destroy
    socket = dispatch_backend("destroy", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("open_dialog:create_dialog", params, socket) do
    # UI action event name: open_dialog:create_dialog
    socket = dispatch_backend("open_dialog:create_dialog", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("update", params, socket) do
    # UI action event name: update
    socket = dispatch_backend("update", params, socket)
    {:noreply, socket}
  end
