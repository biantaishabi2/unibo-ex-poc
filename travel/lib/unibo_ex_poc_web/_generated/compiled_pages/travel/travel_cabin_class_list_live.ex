defmodule UniboExPocWeb.Pages.Travel.TravelCabinClassListLive do
  @moduledoc """
  ...
  @page_id "travel_cabin_class_list"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): rows, rows[], rows[].cabin_class_code, rows[].cabin_class_name, rows[].cabin_rank, rows[].id, rows[].status, rows_empty, travel_cabin_class, travel_cabin_class.page, travel_cabin_class.total_pages
  ...
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelCabinClass.list"}
  }
  @backend_embedded_page %{page_id: "travel_cabin_class_list", page_kind: "list", api_map: %{list: "Travel.TravelCabinClass.list"}, backend: %{load: %{selection: "cabin_class_code: cabinClassCode cabin_class_name: cabinClassName cabin_rank: cabinRank id status"}}, route: %{path: "/pages/travel/travel_cabin_class", query: "", kind: "list"}, state_schema: %{defaults: %{travel_cabin_class: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", cabin_class_code: "", cabin_class_name: "", cabin_rank: "", status: ""}, %{id: "row_02", cabin_class_code: "", cabin_class_name: "", cabin_rank: "", status: ""}, %{id: "row_03", cabin_class_code: "", cabin_class_name: "", cabin_rank: "", status: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_cabin_class/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_cabin_class_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              <.table_head id="th_cabin_class_code">
                舱位规范编码
              </.table_head>
              <.table_head id="th_cabin_class_name">
                舱位名称
              </.table_head>
              <.table_head id="th_cabin_rank">
                舱位等级序
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_cabin_class_row">
                <.table_cell id="td_cabin_class_code">
                  <%= get_in(row, [:cabin_class_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_cabin_class_name">
                  <%= get_in(row, [:cabin_class_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_cabin_rank">
                  <%= get_in(row, [:cabin_rank]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_cabin_class_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_cabin_class, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_cabin_class, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_cabin_class_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
