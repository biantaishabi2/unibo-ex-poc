defmodule UniboExPocWeb.Pages.Travel.TravelPolicyCheckListLive do
  @moduledoc """
  ...
  # status.keys preview (first ~40): rows, rows[], rows[].actual_amount, rows[].approval_mode, rows[].check_result, rows[].exceed_amount, rows[].exceed_ratio, rows[].exceed_reason, rows[].exceed_strategy, rows[].id, rows[].personal_pay_amount, rows[].policy_amount, rows_empty, travel_policy_check, travel_policy_check.page, travel_policy_check.total_pages
  ...
  @backend_params_accept ["check_result", "approval_mode"]
  ...
  @backend_embedded_page %{page_id: "travel_policy_check_list", page_kind: "list", api_map: %{list: "Travel.TravelPolicyCheck.list"}, backend: %{load: %{selection: "actual_amount: actualAmount approval_mode: approvalMode approval_request_id: approvalRequestId check_result: checkResult exceed_amount: exceedAmount exceed_ratio: exceedRatio exceed_reason: exceedReason exceed_strategy: exceedStrategy id personal_pay_amount: personalPayAmount policy_amount: policyAmount"}}, route: %{path: "/pages/travel/travel_policy_check", query: "", kind: "list"}, state_schema: %{defaults: %{travel_policy_check: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", check_result: "", policy_amount: "", actual_amount: "", exceed_amount: "", exceed_ratio: "", exceed_strategy: "", exceed_reason: "", personal_pay_amount: "", approval_mode: ""}, %{id: "row_02", check_result: "", policy_amount: "", actual_amount: "", exceed_amount: "", exceed_ratio: "", exceed_strategy: "", exceed_reason: "", personal_pay_amount: "", approval_mode: ""}, %{id: "row_03", check_result: "", policy_amount: "", actual_amount: "", exceed_amount: "", exceed_ratio: "", exceed_strategy: "", exceed_reason: "", personal_pay_amount: "", approval_mode: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_policy_check/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_policy_check_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              <.table_head id="th_check_result">
                校验结果
              </.table_head>
              <.table_head id="th_policy_amount">
                差标金额（单位分）
              </.table_head>
              <.table_head id="th_actual_amount">
                实际金额（单位分）
              </.table_head>
              <.table_head id="th_exceed_amount">
                超标金额
              </.table_head>
              <.table_head id="th_exceed_ratio">
                超标比例（如'15%'）
              </.table_head>
              <.table_head id="th_exceed_strategy">
                命中的超标策略
              </.table_head>
              <.table_head id="th_exceed_reason">
                超标原因（员工填写）
              </.table_head>
              <.table_head id="th_personal_pay_amount">
                个人支付部分
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_policy_check_row">
                <.table_cell id="td_check_result">
                  <%= if (val = get_in(row, [:check_result])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_check_result">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                <.table_cell id="td_policy_amount">
                  <%= get_in(row, [:policy_amount]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_actual_amount">
                  <%= get_in(row, [:actual_amount]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_exceed_amount">
                  <%= get_in(row, [:exceed_amount]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_exceed_ratio">
                  <%= get_in(row, [:exceed_ratio]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_exceed_strategy">
                  <%= get_in(row, [:exceed_strategy]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_exceed_reason">
                  <%= get_in(row, [:exceed_reason]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_personal_pay_amount">
                  <%= get_in(row, [:personal_pay_amount]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_policy_check_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_policy_check, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_policy_check, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_policy_check_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
