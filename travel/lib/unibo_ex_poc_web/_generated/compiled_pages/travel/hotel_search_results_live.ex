defmodule UniboExPocWeb.Pages.Travel.HotelSearchResultsLive do
  @moduledoc """
  ...
  @backend_api_map %{
    "search" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.HotelOffer.list"}
  }
  @backend_embedded_page %{page_id: "hotel_search_results", page_kind: "search", api_map: %{search: "Travel.HotelOffer.list"}, backend: %{load: %{selection: "id"}}, route: %{path: "/pages/travel/hotel_search_results", query: "", kind: "search"}, state_schema: %{defaults: %{search: %{city_label: "", checkin_date: "", night_count_label: "", checkout_date: ""}, policy: %{exceeded: true}, hotels: %{total_count: true, items: [%{id: "hot_01", hotel_name: "", hotel_star: "", is_policy_compliant: true, address: "", district: "", room_type_name: "", breakfast_label: "", cancel_label: "", rating_label: "", listed_price: ""}, %{id: "hot_02", hotel_name: "", hotel_star: "", is_policy_compliant: true, address: "", district: "", room_type_name: "", breakfast_label: "", cancel_label: "", rating_label: "", listed_price: ""}, %{id: "hot_03", hotel_name: "", hotel_star: "", is_policy_compliant: true, address: "", district: "", room_type_name: "", breakfast_label: "", cancel_label: "", rating_label: "", listed_price: ""}]}}}, status_keys: []}
  ...
  def render(assigns) do
    ...
    <.page title="搜索结果" id="hotel_search_results">
        ...
        <.flex gap={3} paddingx={3} paddingy={2} align="center" background="surface" direction="row" id="hotel_date_bar">
          <.stack gap={0} align="center" direction="column" id="checkin_info">
            ...
            <.text variant="body" weight="bold" id="checkin_date">
              <%= get_in(@search, [:checkin_date]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
            </.text>
          </.stack>
          <.text variant="caption" color="muted" id="night_count">
            <%= get_in(@search, [:night_count_label]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.stack gap={0} align="center" direction="column" id="checkout_info">
            ...
            <.text variant="body" weight="bold" id="checkout_date">
              <%= get_in(@search, [:checkout_date]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
            </.text>
          </.stack>
        </.flex>
      </.section>
      ...
      <.section layout="none" id="results_section">
        <.stack gap={3} padding={3} direction="column" id="hotel_results_list">
          <%= for item <- (get_in(@hotels, [:items]) || []) do %>
            <.card phx-click="navigate" params={"/travel/hotel/#{get_in(item, [:id])}"} variant="default" id="hotel_card">
              <.card_header id="hotel_card_header">
                <.flex gap={2} align="center" direction="row" id="hotel_name_row">
                  <.text variant="body" weight="bold" id="hotel_name">
                    <%= get_in(item, [:hotel_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                  </.text>
                  <%= if (val = get_in(item, [:hotel_star])) && to_string(val) != "" do %>
                    <.badge variant="outline" size="sm" id="hotel_star_badge">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                  <%= if get_in(item, [:is_policy_compliant]) == false do %>
                    <.badge variant="destructive" size="sm" id="hotel_exceed_badge">
                      超标
                    </.badge>
                  <% end %>
                </.flex>
              </.card_header>
              <.card_content id="hotel_card_content">
                <.stack gap={1} direction="column" id="hotel_detail_stack">
                  <.text variant="caption" color="muted" id="hotel_address">
                    <%= get_in(item, [:address]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                  </.text>
                  <.text variant="caption" color="muted" id="hotel_district">
                    <%= get_in(item, [:district]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                  </.text>
                  <.flex gap={1} wrap={true} direction="row" id="hotel_tags_row">
                    <%= if (val = get_in(item, [:room_type_name])) && to_string(val) != "" do %>
                      <.badge variant="outline" size="sm" id="hotel_room_type">
                        <%= to_string(val) %>
                      </.badge>
                    <% end %>
                    <%= if (val = get_in(item, [:breakfast_label])) && to_string(val) != "" do %>
                      <.badge variant="outline" size="sm" id="hotel_breakfast">
                        <%= to_string(val) %>
                      </.badge>
                    <% end %>
                    <%= if (val = get_in(item, [:cancel_label])) && to_string(val) != "" do %>
                      <.badge variant="outline" size="sm" id="hotel_cancel_policy">
                        <%= to_string(val) %>
                      </.badge>
                    <% end %>
                  </.flex>
              ...
              <.card_footer id="hotel_card_footer">
                <.flex justify="between" align="baseline" direction="row" id="hotel_price_row">
                  <.text variant="caption" color="muted" id="hotel_rating">
                    <%= get_in(item, [:rating_label]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                  </.text>
                  <.flex gap={1} align="baseline" direction="row" id="hotel_price">
                    <.text variant="caption" id="hotel_currency">
                      ¥
                    </.text>
                    <.text variant="h3" color="primary" id="hotel_listed_price">
                      <%= get_in(item, [:listed_price]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                    </.text>
                    <.text variant="caption" color="muted" id="hotel_price_unit">
                      /晚
                    </.text>
                  </.flex>
                </.flex>
      ...
      <.section layout="none" id="empty_section">
        <%= if get_in(@hotels, [:total_count]) == 0 do %>
