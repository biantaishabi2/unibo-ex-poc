defmodule UniboExPocWeb.Pages.Travel.TravelFulfillmentListLive do
  @moduledoc """
  ...
  # status.keys preview (first ~40): rows, rows[], rows[].boarding_status, rows[].change_result, rows[].confirmation_payload, rows[].failure_reason, rows[].fulfillment_type, rows[].id, rows[].status, rows[].supplier_booking_ref, rows[].ticket_refs, rows[].used_at, rows[].voucher_or_ticket_ref, rows[].waitlist_result, rows_empty, travel_fulfillment, travel_fulfillment.page, travel_fulfillment.total_pages
  ...
  @backend_params_accept ["fulfillment_type", "status", "waitlist_result", "change_result", "boarding_status", "used_at"]
  @backend_info_reload_messages []
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.TravelFulfillment.list"}
  }
  @backend_embedded_page %{page_id: "travel_fulfillment_list", page_kind: "list", api_map: %{list: "Travel.TravelFulfillment.list"}, backend: %{load: %{selection: "boarding_status: boardingStatus change_result: changeResult confirmation_payload: confirmationPayload failure_reason: failureReason fulfillment_type: fulfillmentType id status supplier_booking_ref: supplierBookingRef tenant_id: tenantId ticket_refs: ticketRefs used_at: usedAt voucher_or_ticket_ref: voucherOrTicketRef waitlist_result: waitlistResult"}}, route: %{path: "/pages/travel/travel_fulfillment", query: "", kind: "list"}, state_schema: %{defaults: %{travel_fulfillment: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", fulfillment_type: "", status: "", supplier_booking_ref: "", voucher_or_ticket_ref: "", ticket_refs: "", waitlist_result: "", change_result: "", boarding_status: "", confirmation_payload: "", failure_reason: "", used_at: ""}, %{id: "row_02", fulfillment_type: "", status: "", supplier_booking_ref: "", voucher_or_ticket_ref: "", ticket_refs: "", waitlist_result: "", change_result: "", boarding_status: "", confirmation_payload: "", failure_reason: "", used_at: ""}, %{id: "row_03", fulfillment_type: "", status: "", supplier_booking_ref: "", voucher_or_ticket_ref: "", ticket_refs: "", waitlist_result: "", change_result: "", boarding_status: "", confirmation_payload: "", failure_reason: "", used_at: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/travel_fulfillment/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="travel_fulfillment_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              <.table_head id="th_fulfillment_type">
                fulfillment_type
              </.table_head>
              <.table_head id="th_status">
                status
              </.table_head>
              <.table_head id="th_supplier_booking_ref">
                供应商预订号
              </.table_head>
              <.table_head id="th_voucher_or_ticket_ref">
                凭证号或票号
              </.table_head>
              <.table_head id="th_ticket_refs">
                多张票号、座席和乘客映射结果
              </.table_head>
              <.table_head id="th_waitlist_result">
                train 候补兑现结果
              </.table_head>
              <.table_head id="th_change_result">
                train 改签结果
              </.table_head>
              <.table_head id="th_boarding_status">
                train 乘车状态
              </.table_head>
              <.table_head id="th_confirmation_payload">
                确认结果快照
              </.table_head>
              <.table_head id="th_failure_reason">
                失败原因
              </.table_head>
              <.table_head id="th_used_at">
                实际使用时间
              </.table_head>
            </.table_row>
          </.table_header>
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="travel_fulfillment_row">
                <.table_cell id="td_fulfillment_type">
                  <%= if (val = get_in(row, [:fulfillment_type])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_fulfillment_type">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                ...
                <.table_cell id="td_supplier_booking_ref">
                  <%= get_in(row, [:supplier_booking_ref]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_voucher_or_ticket_ref">
                  <%= get_in(row, [:voucher_or_ticket_ref]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_ticket_refs">
                  <%= get_in(row, [:ticket_refs]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_waitlist_result">
                  <%= if (val = get_in(row, [:waitlist_result])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_waitlist_result">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                <.table_cell id="td_change_result">
                  <%= if (val = get_in(row, [:change_result])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_change_result">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                <.table_cell id="td_boarding_status">
                  <%= if (val = get_in(row, [:boarding_status])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_boarding_status">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                <.table_cell id="td_confirmation_payload">
                  <%= get_in(row, [:confirmation_payload]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_failure_reason">
                  <%= get_in(row, [:failure_reason]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_used_at">
                  <%= get_in(row, [:used_at]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
              </.table_row>
            <% end %>
          </.table_body>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="travel_fulfillment_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@travel_fulfillment, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@travel_fulfillment, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="travel_fulfillment_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
