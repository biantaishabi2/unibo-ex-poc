defmodule UniboExPocWeb.Pages.Travel.VacationOfferListLive do
  @moduledoc """
  ...
  # status.keys preview (first ~40): rows, rows[], rows[].booking_rules, rows[].cancellation_policy, rows[].currency, rows[].departure_city_code, rows[].destination_code, rows[].end_date, rows[].id, rows[].inventory_count, rows[].listed_price, rows[].package_code, rows[].package_name, rows[].package_type, rows[].sale_status, rows[].settlement_price, rows[].start_date, rows[].supplier_code, rows_empty, vacation_offer, vacation_offer.page, vacation_offer.total_pages
  ...
  @backend_params_accept ["package_type", "start_date", "end_date", "sale_status"]
  @backend_info_reload_messages []
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.VacationOffer.list"}
  }
  @backend_embedded_page %{page_id: "vacation_offer_list", page_kind: "list", api_map: %{list: "Travel.VacationOffer.list"}, backend: %{load: %{selection: "booking_rules: bookingRules cancellation_policy: cancellationPolicy currency departure_city_code: departureCityCode destination_code: destinationCode end_date: endDate host_shop_id: hostShopId id inventory_count: inventoryCount listed_price: listedPrice package_code: packageCode package_name: packageName package_type: packageType sale_status: saleStatus settlement_price: settlementPrice start_date: startDate supplier_code: supplierCode tenant_id: tenantId"}}, route: %{path: "/pages/travel/vacation_offer", query: "", kind: "list"}, state_schema: %{defaults: %{vacation_offer: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", supplier_code: "", package_code: "", package_name: "", package_type: "", departure_city_code: "", destination_code: "", start_date: "", end_date: "", listed_price: "", settlement_price: "", currency: "", inventory_count: "", booking_rules: "", cancellation_policy: "", sale_status: ""}, %{id: "row_02", supplier_code: "", package_code: "", package_name: "", package_type: "", departure_city_code: "", destination_code: "", start_date: "", end_date: "", listed_price: "", settlement_price: "", currency: "", inventory_count: "", booking_rules: "", cancellation_policy: "", sale_status: ""}, %{id: "row_03", supplier_code: "", package_code: "", package_name: "", package_type: "", departure_city_code: "", destination_code: "", start_date: "", end_date: "", listed_price: "", settlement_price: "", currency: "", inventory_count: "", booking_rules: "", cancellation_policy: "", sale_status: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/vacation_offer/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="vacation_offer_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              ...
              <.table_head id="th_package_code">
                套餐编码
              </.table_head>
              <.table_head id="th_package_name">
                套餐名称
              </.table_head>
              <.table_head id="th_package_type">
                套餐类型
              </.table_head>
              <.table_head id="th_departure_city_code">
                出发城市编码
              </.table_head>
              <.table_head id="th_destination_code">
                目的地编码
              </.table_head>
              ...
              <.table_head id="th_inventory_count">
                可售库存快照
              </.table_head>
              <.table_head id="th_booking_rules">
                预订规则快照
              </.table_head>
              <.table_head id="th_cancellation_policy">
                取消规则快照
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="vacation_offer_row">
                <.table_cell id="td_supplier_code">
                  <%= get_in(row, [:supplier_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_package_code">
                  <%= get_in(row, [:package_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_package_name">
                  <%= get_in(row, [:package_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_package_type">
                  <%= if (val = get_in(row, [:package_type])) && to_string(val) != "" do %>
                    <.badge variant="default" size="sm" id="badge_package_type">
                      <%= to_string(val) %>
                    </.badge>
                  <% end %>
                </.table_cell>
                <.table_cell id="td_departure_city_code">
                  <%= get_in(row, [:departure_city_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_destination_code">
                  <%= get_in(row, [:destination_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_start_date">
                  <%= get_in(row, [:start_date]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_end_date">
                  <%= get_in(row, [:end_date]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                ...
                <.table_cell id="td_inventory_count">
                  <%= get_in(row, [:inventory_count]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_booking_rules">
                  <%= get_in(row, [:booking_rules]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_cancellation_policy">
                  <%= get_in(row, [:cancellation_policy]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="vacation_offer_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@vacation_offer, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@vacation_offer, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="vacation_offer_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
