defmodule UniboExPocWeb.Pages.Travel.FlightOfferListLive do
  @moduledoc """
  ...
  # status.keys preview (first ~40): flight_offer, flight_offer.page, flight_offer.total_pages, rows, rows[], rows[].arrival_airport_code, rows[].arrival_at, rows[].baggage_policy, rows[].cabin_class, rows[].currency, rows[].departure_airport_code, rows[].departure_at, rows[].fare_family, rows[].flight_no, rows[].id, rows[].itinerary_code, rows[].listed_price, rows[].refund_change_policy, rows[].sale_status, rows[].seats_available, rows[].settlement_price, rows[].supplier_code, rows_empty
  ...
  @backend_params_accept ["departure_at", "arrival_at", "sale_status"]
  @backend_info_reload_messages []
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.FlightOffer.list"}
  }
  @backend_embedded_page %{page_id: "flight_offer_list", page_kind: "list", api_map: %{list: "Travel.FlightOffer.list"}, backend: %{load: %{selection: "arrival_airport_code: arrivalAirportCode arrival_at: arrivalAt baggage_policy: baggagePolicy cabin_class: cabinClass currency departure_airport_code: departureAirportCode departure_at: departureAt fare_family: fareFamily flight_no: flightNo host_shop_id: hostShopId id itinerary_code: itineraryCode listed_price: listedPrice refund_change_policy: refundChangePolicy sale_status: saleStatus seats_available: seatsAvailable settlement_price: settlementPrice supplier_code: supplierCode tenant_id: tenantId"}}, route: %{path: "/pages/travel/flight_offer", query: "", kind: "list"}, state_schema: %{defaults: %{flight_offer: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", supplier_code: "", itinerary_code: "", flight_no: "", departure_airport_code: "", arrival_airport_code: "", departure_at: "", arrival_at: "", cabin_class: "", fare_family: "", listed_price: "", settlement_price: "", currency: "", seats_available: "", baggage_policy: "", refund_change_policy: "", sale_status: ""}, %{id: "row_02", supplier_code: "", itinerary_code: "", flight_no: "", departure_airport_code: "", arrival_airport_code: "", departure_at: "", arrival_at: "", cabin_class: "", fare_family: "", listed_price: "", settlement_price: "", currency: "", seats_available: "", baggage_policy: "", refund_change_policy: "", sale_status: ""}, %{id: "row_03", supplier_code: "", itinerary_code: "", flight_no: "", departure_airport_code: "", arrival_airport_code: "", departure_at: "", arrival_at: "", cabin_class: "", fare_family: "", listed_price: "", settlement_price: "", currency: "", seats_available: "", baggage_policy: "", refund_change_policy: "", sale_status: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/flight_offer/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="flight_offer_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              ...
              <.table_head id="th_itinerary_code">
                行程编码
              </.table_head>
              <.table_head id="th_flight_no">
                航班号
              </.table_head>
              <.table_head id="th_departure_airport_code">
                出发机场编码
              </.table_head>
              <.table_head id="th_arrival_airport_code">
                到达机场编码
              </.table_head>
              ...
              <.table_head id="th_cabin_class">
                舱等
              </.table_head>
              <.table_head id="th_fare_family">
                运价族
              </.table_head>
              ...
              <.table_head id="th_seats_available">
                可售座位快照
              </.table_head>
              <.table_head id="th_baggage_policy">
                行李规则快照
              </.table_head>
              <.table_head id="th_refund_change_policy">
                退改规则快照
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="flight_offer_row">
                <.table_cell id="td_supplier_code">
                  <%= get_in(row, [:supplier_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_itinerary_code">
                  <%= get_in(row, [:itinerary_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_flight_no">
                  <%= get_in(row, [:flight_no]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_departure_airport_code">
                  <%= get_in(row, [:departure_airport_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_arrival_airport_code">
                  <%= get_in(row, [:arrival_airport_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                ...
                <.table_cell id="td_cabin_class">
                  <%= get_in(row, [:cabin_class]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_fare_family">
                  <%= get_in(row, [:fare_family]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                ...
                <.table_cell id="td_seats_available">
                  <%= get_in(row, [:seats_available]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_baggage_policy">
                  <%= get_in(row, [:baggage_policy]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_refund_change_policy">
                  <%= get_in(row, [:refund_change_policy]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="flight_offer_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@flight_offer, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@flight_offer, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="flight_offer_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
