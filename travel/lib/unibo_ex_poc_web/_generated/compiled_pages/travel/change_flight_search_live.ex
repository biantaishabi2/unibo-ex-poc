defmodule UniboExPocWeb.Pages.Travel.ChangeFlightSearchLive do
  @moduledoc """
  ...
  @backend_embedded_page %{page_id: "change_flight_search", page_kind: "search", api_map: %{search: "Travel.FlightOffer.list"}, backend: %{load: %{selection: "id"}}, route: %{path: "/pages/travel/change_flight_search", query: "", kind: "search"}, state_schema: %{defaults: %{policy: %{exceeded: true}, results: %{total_count: true}, search: %{date_range: [%{is_selected: "", date_label: ""}, %{is_selected: "", date_label: ""}, %{is_selected: "", date_label: ""}]}}}, status_keys: []}
  ...
  def render(assigns) do
    ~H"""
    <.page title="搜索结果" id="change_flight_search">
        ...
        <.flex gap={2} paddingx={3} paddingy={2} overflowx="scroll" background="surface" direction="row" id="date_slider">
          <%= for date_item <- (get_in(@search, [:date_range]) || []) do %>
            <%= if (val = get_in(date_item, [:date_label])) && to_string(val) != "" do %>
              <.badge variant={get_in(date_item, [:is_selected])} phx-click="select_date" size="sm" id="date_badge">
                <%= to_string(val) %>
              </.badge>
            <% end %>
          <% end %>
      ...
      <.section layout="none" id="empty_section">
        <%= if get_in(@results, [:total_count]) == 0 do %>
