defmodule UniboExPocWeb.Pages.Travel.HotelOfferListLive do
  @moduledoc """
  ...
  @backend_params_accept ["checkin_date", "checkout_date", "sale_status"]
  @backend_info_reload_messages []
  @backend_api_map %{
    "list" => %{module: UniboExPocWeb.Graphql.StitchBackend, fun: :dispatch, api: "Travel.HotelOffer.list"}
  }
  @backend_embedded_page %{page_id: "hotel_offer_list", page_kind: "list", api_map: %{list: "Travel.HotelOffer.list"}, backend: %{load: %{selection: "cancellation_policy: cancellationPolicy checkin_date: checkinDate checkout_date: checkoutDate city_code: cityCode currency guarantee_policy: guaranteePolicy host_shop_id: hostShopId hotel_code: hotelCode hotel_name: hotelName id inventory_count: inventoryCount listed_price: listedPrice rate_plan_code: ratePlanCode room_type_code: roomTypeCode sale_status: saleStatus settlement_price: settlementPrice supplier_code: supplierCode tenant_id: tenantId"}}, route: %{path: "/pages/travel/hotel_offer", query: "", kind: "list"}, state_schema: %{defaults: %{hotel_offer: %{page: "", total_pages: ""}, rows_empty: true, rows: [%{id: "row_01", supplier_code: "", hotel_code: "", hotel_name: "", city_code: "", room_type_code: "", rate_plan_code: "", checkin_date: "", checkout_date: "", listed_price: "", settlement_price: "", currency: "", inventory_count: "", cancellation_policy: "", guarantee_policy: "", sale_status: ""}, %{id: "row_02", supplier_code: "", hotel_code: "", hotel_name: "", city_code: "", room_type_code: "", rate_plan_code: "", checkin_date: "", checkout_date: "", listed_price: "", settlement_price: "", currency: "", inventory_count: "", cancellation_policy: "", guarantee_policy: "", sale_status: ""}, %{id: "row_03", supplier_code: "", hotel_code: "", hotel_name: "", city_code: "", room_type_code: "", rate_plan_code: "", checkin_date: "", checkout_date: "", listed_price: "", settlement_price: "", currency: "", inventory_count: "", cancellation_policy: "", guarantee_policy: "", sale_status: ""}]}}, status_keys: ["rows", "rows_empty", "filter", "loading"]}
  ...
  assigns = assigns
  effects = []
  effects = [%{to: "/pages/travel/hotel_offer/new", type: "navigate"} | effects]
  effects = __interpolate_effects(effects, params)
  %{assigns: assigns, effects: Enum.reverse(effects)}
end
  ...
  def render(assigns) do
    ~H"""
    <.page title="列表" id="hotel_offer_list">
        ...
        <.table id="data_table">
          <.table_header id="data_header">
            <.table_row id="header_row">
              ...
              <.table_head id="th_hotel_code">
                酒店编码
              </.table_head>
              <.table_head id="th_hotel_name">
                酒店名称
              </.table_head>
              <.table_head id="th_city_code">
                城市编码
              </.table_head>
              <.table_head id="th_room_type_code">
                房型编码
              </.table_head>
              <.table_head id="th_rate_plan_code">
                价计划编码
              </.table_head>
              <.table_head id="th_checkin_date">
                入住日期
              </.table_head>
              <.table_head id="th_checkout_date">
                离店日期
              </.table_head>
              ...
              <.table_head id="th_guarantee_policy">
                担保规则快照
              </.table_head>
          ...
          <.table_body id="data_body">
            <%= for {row, idx} <- Enum.with_index((@rows || [])) do %>
              <.table_row phx-click="row_click" phx-value-id={row.id} id="hotel_offer_row">
                <.table_cell id="td_supplier_code">
                  <%= get_in(row, [:supplier_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_hotel_code">
                  <%= get_in(row, [:hotel_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_hotel_name">
                  <%= get_in(row, [:hotel_name]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_city_code">
                  <%= get_in(row, [:city_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_room_type_code">
                  <%= get_in(row, [:room_type_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_rate_plan_code">
                  <%= get_in(row, [:rate_plan_code]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_checkin_date">
                  <%= get_in(row, [:checkin_date]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                <.table_cell id="td_checkout_date">
                  <%= get_in(row, [:checkout_date]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
                ...
                <.table_cell id="td_guarantee_policy">
                  <%= get_in(row, [:guarantee_policy]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
                </.table_cell>
      ...
      <.section layout="none" id="footer">
        <.flex gap={2} direction="row" justify="center" id="pagination_bar">
          <.button phx-click="hotel_offer_page_prev" variant="primary" size="md" id="page_prev">
            上一页
          </.button>
          <.text id="page_info">
            <%= get_in(@hotel_offer, [:page]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %> / <%= get_in(@hotel_offer, [:total_pages]) |> then(fn v when is_binary(v) and v != "" -> v; _ -> "" end) %>
          </.text>
          <.button phx-click="hotel_offer_page_next" variant="primary" size="md" id="page_next">
            下一页
          </.button>
        </.flex>
      </.section>
