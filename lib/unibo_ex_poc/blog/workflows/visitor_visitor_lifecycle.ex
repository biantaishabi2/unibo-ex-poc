defmodule UniboExPoc.Blog.Workflows.Visitor.VisitorLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Blog.Visitor
...
{"steps":[{"idempotency_key":null,"next":["track_visit"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"upsert"},{"idempotency_key":null,"next":["track_visit","merge","cleanup"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"track_visit"},{"idempotency_key":null,"next":["cleanup"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"merge"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cleanup"}],"workflow":"visitor_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :upsert ->
        Ash.create(Ash.Changeset.for_create(Visitor, :upsert, params), ash_opts)
      :track_visit ->
        Ash.update(Ash.Changeset.for_update(record, :track_visit, params), ash_opts)
      :merge ->
        Ash.update(Ash.Changeset.for_update(record, :merge, params), ash_opts)
      :cleanup ->
        Ash.destroy(Ash.Changeset.for_destroy(record, :cleanup, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :upsert -> [:track_visit]
      :track_visit -> [:track_visit, :merge, :cleanup]
      :merge -> [:cleanup]
      :cleanup -> []
      _ -> []
    end
  ...
  defp retry_policy(step) do
    case step do
      :upsert -> %{max_attempts: 1, backoff_ms: 0}
      :track_visit -> %{max_attempts: 1, backoff_ms: 0}
      :merge -> %{max_attempts: 1, backoff_ms: 0}
      :cleanup -> %{max_attempts: 1, backoff_ms: 0}
