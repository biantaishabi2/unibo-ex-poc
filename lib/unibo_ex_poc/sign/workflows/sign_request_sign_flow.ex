defmodule UniboExPoc.Sign.Workflows.SignRequest.SignFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Sign.SignRequest

  def steps do
    [:create, :send, :complete, :cancel, :recall, :expire]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["send","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["complete","cancel","recall","expire"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"send"},{"idempotency_key":null,"next":["cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"complete"},{"idempotency_key":null,"next":["recall"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"},{"idempotency_key":null,"next":["send","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"recall"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"expire"}],"workflow":"sign_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SignRequest, :create, params), ash_opts)
      :send ->
      ...
      :recall ->
        Ash.update(Ash.Changeset.for_update(record, :recall, params), ash_opts)
      :expire ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:send, :cancel]
      :send -> [:complete, :cancel, :recall, :expire]
      :complete -> [:cancel]
      :cancel -> [:recall]
      :recall -> [:send, :cancel]
      :expire -> []
