defmodule UniboExPoc.Inventory.Workflows.StockPicking.StockPickingLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Inventory.StockPicking

  def steps do
    [:create, :action_confirm, :action_assign, :button_validate]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["action_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_assign"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_confirm"},{"idempotency_key":null,"next":["button_validate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_assign"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"button_validate"}],"workflow":"stock_picking_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(StockPicking, :create, params), ash_opts)
      :action_confirm ->
        Ash.update(Ash.Changeset.for_update(record, :action_confirm, params), ash_opts)
      :action_assign ->
        Ash.update(Ash.Changeset.for_update(record, :action_assign, params), ash_opts)
      :button_validate ->
        Ash.update(Ash.Changeset.for_update(record, :button_validate, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_assign -> %{max_attempts: 1, backoff_ms: 0}
      :button_validate -> %{max_attempts: 1, backoff_ms: 0}
