...
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(StockMove, :create, params), ash_opts)
      :action_confirm ->
      ...
      :trigger_assign ->
        Ash.update(Ash.Changeset.for_update(record, :trigger_assign, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      ...
      :action_assign -> [:action_done]
      :action_done -> [:trigger_assign]
      :trigger_assign -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :trigger_assign -> %{max_attempts: 1, backoff_ms: 0}
