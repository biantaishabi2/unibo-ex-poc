...
{"steps":[{"idempotency_key":null,"next":["update","rate_shipment","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","rate_shipment","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","rate_shipment"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"rate_shipment"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"delivery_carrier_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(DeliveryCarrier, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :rate_shipment ->
        Ash.update(Ash.Changeset.for_update(record, :rate_shipment, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :rate_shipment, :destroy]
      :update -> [:update, :rate_shipment, :destroy]
      :rate_shipment -> [:update, :rate_shipment]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :rate_shipment -> %{max_attempts: 1, backoff_ms: 0}
