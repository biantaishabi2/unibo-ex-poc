...
{"steps":[{"idempotency_key":null,"next":["validate","update","close","flag","accept_answer","toggle_favourite","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","close","flag","accept_answer","toggle_favourite"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"validate"},{"idempotency_key":null,"next":["update","close","flag","accept_answer","toggle_favourite","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["reopen"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"close"},{"idempotency_key":null,"next":["update","close","flag"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reopen"},{"idempotency_key":null,"next":["mark_offensive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"flag"},{"idempotency_key":null,"next":["accept_answer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_offensive"},{"idempotency_key":null,"next":["unaccept_answer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"accept_answer"},{"idempotency_key":null,"next":["accept_answer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unaccept_answer"},{"idempotency_key":null,"next":["toggle_favourite","update","close"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"toggle_favourite"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"post_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Post, :create, params), ash_opts)
      :validate ->
        Ash.update(Ash.Changeset.for_update(record, :validate, params), ash_opts)
      :update ->
      ...
      :flag ->
        Ash.update(Ash.Changeset.for_update(record, :flag, params), ash_opts)
      :mark_offensive ->
        Ash.update(Ash.Changeset.for_update(record, :mark_offensive, params), ash_opts)
      :accept_answer ->
        Ash.update(Ash.Changeset.for_update(record, :accept_answer, params), ash_opts)
      :unaccept_answer ->
        Ash.update(Ash.Changeset.for_update(record, :unaccept_answer, params), ash_opts)
      :toggle_favourite ->
        Ash.update(Ash.Changeset.for_update(record, :toggle_favourite, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:validate, :update, :close, :flag, :accept_answer, :toggle_favourite, :destroy]
      :validate -> [:update, :close, :flag, :accept_answer, :toggle_favourite]
      :update -> [:update, :close, :flag, :accept_answer, :toggle_favourite, :destroy]
      :close -> [:reopen]
      :reopen -> [:update, :close, :flag]
      :flag -> [:mark_offensive]
      :mark_offensive -> [:accept_answer]
      :accept_answer -> [:unaccept_answer]
      :unaccept_answer -> [:accept_answer]
      :toggle_favourite -> [:toggle_favourite, :update, :close]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp step_skipped?(step, record) do
    ...
    case step do
      ...
      :reopen -> false
      :flag -> false
      :mark_offensive -> false
      :accept_answer -> false
      :unaccept_answer -> false
  ...
  defp retry_policy(step) do
    case step do
      ...
      :validate -> %{max_attempts: 1, backoff_ms: 0}
      :update -> %{max_attempts: 1, backoff_ms: 0}
      :close -> %{max_attempts: 1, backoff_ms: 0}
      :reopen -> %{max_attempts: 1, backoff_ms: 0}
      :flag -> %{max_attempts: 1, backoff_ms: 0}
      :mark_offensive -> %{max_attempts: 1, backoff_ms: 0}
      :accept_answer -> %{max_attempts: 1, backoff_ms: 0}
      :unaccept_answer -> %{max_attempts: 1, backoff_ms: 0}
      :toggle_favourite -> %{max_attempts: 1, backoff_ms: 0}
