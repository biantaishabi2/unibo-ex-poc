...
{"steps":[{"idempotency_key":null,"next":["register_box"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["heartbeat"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"register_box"},{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"heartbeat"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"}],"workflow":"iot_box_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(IoTBox, :create, params), ash_opts)
      :register_box ->
        Ash.update(Ash.Changeset.for_update(record, :register_box, params), ash_opts)
      :heartbeat ->
        Ash.update(Ash.Changeset.for_update(record, :heartbeat, params), ash_opts)
      :update ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :register_box -> %{max_attempts: 1, backoff_ms: 0}
      :heartbeat -> %{max_attempts: 1, backoff_ms: 0}
