defmodule UniboExPoc.IoT.Workflows.IncomingNumber.IncomingNumberLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.IncomingNumber

  def steps do
    [:create, :bind_destination, :update, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["bind_destination"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"bind_destination"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"incoming_number_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(IncomingNumber, :create, params), ash_opts)
      :bind_destination ->
        Ash.update(Ash.Changeset.for_update(record, :bind_destination, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:bind_destination]
      :bind_destination -> [:update]
      :update -> [:destroy]
      :destroy -> []
      _ -> []
    end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :bind_destination -> %{max_attempts: 1, backoff_ms: 0}
