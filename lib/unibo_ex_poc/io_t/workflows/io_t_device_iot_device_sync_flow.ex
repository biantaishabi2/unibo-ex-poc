defmodule UniboExPoc.IoT.Workflows.IoTDevice.IotDeviceSyncFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.IoTDevice
...
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"upsert"},{"idempotency_key":null,"next":["update_value"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update_value"}],"workflow":"iot_device_sync_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :upsert ->
        Ash.create(Ash.Changeset.for_create(IoTDevice, :upsert, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :update_value ->
        Ash.update(Ash.Changeset.for_update(record, :update_value, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :update_value -> %{max_attempts: 1, backoff_ms: 0}
