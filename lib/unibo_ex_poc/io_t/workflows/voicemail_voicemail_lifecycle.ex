defmodule UniboExPoc.IoT.Workflows.Voicemail.VoicemailLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.IoT.Voicemail

  def steps do
    [:create, :mark_read, :transcribe, :archive, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["mark_read"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["transcribe"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_read"},{"idempotency_key":null,"next":["archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"transcribe"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"voicemail_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Voicemail, :create, params), ash_opts)
      :mark_read ->
        Ash.update(Ash.Changeset.for_update(record, :mark_read, params), ash_opts)
      :transcribe ->
        Ash.update(Ash.Changeset.for_update(record, :transcribe, params), ash_opts)
      :archive ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :mark_read -> %{max_attempts: 1, backoff_ms: 0}
      :transcribe -> %{max_attempts: 1, backoff_ms: 0}
