defmodule UniboV4.HR.Workflows.JobApplication.ApplicationReopenFlowWorkflow do
  ...
  def steps do
    [:reject, :reset, :advance]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["reset"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reject"},{"idempotency_key":null,"next":["advance"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reset"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"advance"}],"workflow":"application_reopen_flow"}
"""
