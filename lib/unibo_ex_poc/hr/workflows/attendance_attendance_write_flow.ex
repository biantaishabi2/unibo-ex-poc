...
{"steps":[{"idempotency_key":null,"next":["check_out"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"check_out"}],"workflow":"attendance_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Attendance, :create, params), ash_opts)
      :check_out ->
        Ash.update(Ash.Changeset.for_update(record, :check_out, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :check_out -> %{max_attempts: 1, backoff_ms: 0}
