defmodule UniboExPoc.Subscriptions.Workflows.SubscriptionOrder.CronPaymentRetryWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
...
{"steps":[{"idempotency_key":null,"next":["retry_payment"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"evaluate_retry_due"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"retry_payment"}],"workflow":"cron_payment_retry"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :evaluate_retry_due ->
        Ash.update(Ash.Changeset.for_update(record, :evaluate_retry_due, params), ash_opts)
      :retry_payment ->
        Ash.update(Ash.Changeset.for_update(record, :retry_payment, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :evaluate_retry_due -> %{max_attempts: 1, backoff_ms: 0}
      :retry_payment -> %{max_attempts: 1, backoff_ms: 0}
