defmodule UniboExPoc.Subscriptions.Workflows.SubscriptionOrder.CronInvoicingWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
...
{"steps":[{"idempotency_key":null,"next":["generate_invoice"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"evaluate_invoicing_due"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"generate_invoice"}],"workflow":"cron_invoicing"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :evaluate_invoicing_due ->
        Ash.update(Ash.Changeset.for_update(record, :evaluate_invoicing_due, params), ash_opts)
      :generate_invoice ->
        Ash.update(Ash.Changeset.for_update(record, :generate_invoice, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp branch_next(step, record) do
    _ = record
    case step do
      :evaluate_invoicing_due -> nil
      :generate_invoice -> nil
      _ -> nil
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :evaluate_invoicing_due -> %{max_attempts: 1, backoff_ms: 0}
      :generate_invoice -> %{max_attempts: 1, backoff_ms: 0}
