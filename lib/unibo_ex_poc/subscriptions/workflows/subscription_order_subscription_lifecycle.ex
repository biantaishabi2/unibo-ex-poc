defmodule UniboExPoc.Subscriptions.Workflows.SubscriptionOrder.SubscriptionLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Subscriptions.SubscriptionOrder
...
{"steps":[{"idempotency_key":null,"next":["action_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_pause","action_close","action_upsell"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_confirm"},{"idempotency_key":null,"next":["action_resume"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_pause"},{"idempotency_key":null,"next":["action_pause","action_close","action_upsell"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_resume"},{"idempotency_key":null,"next":["action_renew"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_close"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_renew"}],"workflow":"subscription_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SubscriptionOrder, :create, params), ash_opts)
      :action_confirm ->
        Ash.update(Ash.Changeset.for_update(record, :action_confirm, params), ash_opts)
      :action_pause ->
        Ash.update(Ash.Changeset.for_update(record, :action_pause, params), ash_opts)
      :action_resume ->
        Ash.update(Ash.Changeset.for_update(record, :action_resume, params), ash_opts)
      :action_close ->
        Ash.update(Ash.Changeset.for_update(record, :action_close, params), ash_opts)
      :action_renew ->
        Ash.create(Ash.Changeset.for_create(SubscriptionOrder, :action_renew, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      ...
      :action_confirm -> [:action_pause, :action_close, :action_upsell]
      :action_pause -> [:action_resume]
      :action_resume -> [:action_pause, :action_close, :action_upsell]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_pause -> %{max_attempts: 1, backoff_ms: 0}
      :action_resume -> %{max_attempts: 1, backoff_ms: 0}
      :action_close -> %{max_attempts: 1, backoff_ms: 0}
      :action_renew -> %{max_attempts: 1, backoff_ms: 0}
