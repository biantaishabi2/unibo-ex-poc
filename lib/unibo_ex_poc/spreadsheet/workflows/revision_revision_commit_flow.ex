defmodule UniboExPoc.Spreadsheet.Workflows.Revision.RevisionCommitFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Spreadsheet.Revision

  def steps do
    [:create_snapshot, :apply_revision]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["apply_revision"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create_snapshot"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"apply_revision"}],"workflow":"revision_commit_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create_snapshot ->
        Ash.create(Ash.Changeset.for_create(Revision, :create_snapshot, params), ash_opts)
      :apply_revision ->
        Ash.create(Ash.Changeset.for_create(Revision, :apply_revision, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :create_snapshot -> %{max_attempts: 1, backoff_ms: 0}
      :apply_revision -> %{max_attempts: 1, backoff_ms: 0}
