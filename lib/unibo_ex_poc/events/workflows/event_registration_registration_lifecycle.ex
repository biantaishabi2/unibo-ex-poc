...
{"steps":[{"idempotency_key":null,"next":["confirm","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"register"},{"idempotency_key":null,"next":["check_in","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"confirm"},{"idempotency_key":null,"next":["cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"check_in"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"}],"workflow":"registration_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :register ->
        Ash.create(Ash.Changeset.for_create(EventRegistration, :register, params), ash_opts)
      :confirm ->
        Ash.update(Ash.Changeset.for_update(record, :confirm, params), ash_opts)
      :check_in ->
        Ash.update(Ash.Changeset.for_update(record, :check_in, params), ash_opts)
      :cancel ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :check_in -> %{max_attempts: 1, backoff_ms: 0}
