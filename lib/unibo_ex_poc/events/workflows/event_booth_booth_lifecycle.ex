defmodule UniboExPoc.Events.Workflows.EventBooth.BoothLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Events.EventBooth
...
{"steps":[{"idempotency_key":null,"next":["update","reserve","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["release"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reserve"},{"idempotency_key":null,"next":["reserve","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"release"}],"workflow":"booth_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(EventBooth, :create, params), ash_opts)
      :reserve ->
        Ash.update(Ash.Changeset.for_update(record, :reserve, params), ash_opts)
      :release ->
        Ash.update(Ash.Changeset.for_update(record, :release, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :reserve, :destroy]
      :reserve -> [:release]
      :release -> [:reserve, :destroy]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :reserve -> %{max_attempts: 1, backoff_ms: 0}
      :release -> %{max_attempts: 1, backoff_ms: 0}
