defmodule UniboExPoc.Project.Workflows.Milestone.MilestoneWriteFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Project.Milestone
...
{"steps":[{"idempotency_key":null,"next":["reach"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reach"}],"workflow":"milestone_write_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Milestone, :create, params), ash_opts)
      :reach ->
        Ash.update(Ash.Changeset.for_update(record, :reach, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :reach -> %{max_attempts: 1, backoff_ms: 0}
