...
{"steps":[{"idempotency_key":null,"next":["join"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["leave"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"join"},{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"leave"},{"idempotency_key":null,"next":["destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"mail_group_membership_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(MailGroup, :create, params), ash_opts)
      :join ->
        Ash.update(Ash.Changeset.for_update(record, :join, params), ash_opts)
      :leave ->
        Ash.update(Ash.Changeset.for_update(record, :leave, params), ash_opts)
      :update ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :leave -> %{max_attempts: 1, backoff_ms: 0}
