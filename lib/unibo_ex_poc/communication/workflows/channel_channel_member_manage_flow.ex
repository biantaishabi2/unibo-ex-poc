defmodule UniboExPoc.Communication.Workflows.Channel.ChannelMemberManageFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Communication.Channel

  def steps do
    [:create, :update, :add_members, :archive]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["add_members"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"add_members"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"}],"workflow":"channel_member_manage_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Channel, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :add_members ->
        Ash.update(Ash.Changeset.for_update(record, :add_members, params), ash_opts)
      :archive ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :add_members -> %{max_attempts: 1, backoff_ms: 0}
