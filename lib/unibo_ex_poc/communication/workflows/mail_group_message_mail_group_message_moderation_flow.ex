...
{"steps":[{"idempotency_key":null,"next":["moderate_accept"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["moderate_reject"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"moderate_accept"},{"idempotency_key":null,"next":["moderate_allow"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"moderate_reject"},{"idempotency_key":null,"next":["moderate_ban"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"moderate_allow"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"moderate_ban"}],"workflow":"mail_group_message_moderation_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(MailGroupMessage, :create, params), ash_opts)
      :moderate_accept ->
        Ash.update(Ash.Changeset.for_update(record, :moderate_accept, params), ash_opts)
      :moderate_reject ->
        Ash.update(Ash.Changeset.for_update(record, :moderate_reject, params), ash_opts)
      :moderate_allow ->
        Ash.update(Ash.Changeset.for_update(record, :moderate_allow, params), ash_opts)
      :moderate_ban ->
        Ash.update(Ash.Changeset.for_update(record, :moderate_ban, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:moderate_accept]
      :moderate_accept -> [:moderate_reject]
      :moderate_reject -> [:moderate_allow]
      :moderate_allow -> [:moderate_ban]
      :moderate_ban -> []
      _ -> []
    end
  end
  ...
  defp step_skipped?(step, record) do
    ...
    case step do
      ...
      :moderate_reject -> false
      :moderate_allow -> false
      :moderate_ban -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :moderate_accept -> %{max_attempts: 1, backoff_ms: 0}
      :moderate_reject -> %{max_attempts: 1, backoff_ms: 0}
      :moderate_allow -> %{max_attempts: 1, backoff_ms: 0}
      :moderate_ban -> %{max_attempts: 1, backoff_ms: 0}
