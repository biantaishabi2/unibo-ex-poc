...
{"steps":[{"idempotency_key":null,"next":["update","join"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["join"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["quit"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"join"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"quit"}],"workflow":"livechat_channel_maintain_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LiveChatChannel, :create, params), ash_opts)
      :update ->
      ...
      :quit ->
        Ash.update(Ash.Changeset.for_update(record, :quit, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :quit -> %{max_attempts: 1, backoff_ms: 0}
