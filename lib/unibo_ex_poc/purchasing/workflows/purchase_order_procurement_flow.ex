...
{"steps":[{"idempotency_key":null,"next":["print_quotation","send_rfq","button_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["button_confirm","button_cancel","button_draft"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"print_quotation"},{"idempotency_key":null,"next":["button_confirm","button_cancel","button_draft"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"send_rfq"},{"idempotency_key":null,"next":["button_approve","button_done","button_cancel","action_create_invoice"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"button_confirm"},{"idempotency_key":null,"next":["button_done","button_cancel","action_create_invoice"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"button_approve"},{"idempotency_key":null,"next":["button_unlock"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"button_done"},{"idempotency_key":null,"next":["button_done","button_cancel","action_create_invoice"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"button_unlock"},{"idempotency_key":null,"next":["button_draft"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"button_cancel"},{"idempotency_key":null,"next":["print_quotation","send_rfq","button_confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"button_draft"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_create_invoice"}],"workflow":"procurement_flow"}
"""
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:print_quotation, :send_rfq, :button_confirm]
      :print_quotation -> [:button_confirm, :button_cancel, :button_draft]
      :send_rfq -> [:button_confirm, :button_cancel, :button_draft]
      :button_confirm -> [:button_approve, :button_done, :button_cancel, :action_create_invoice]
      :button_approve -> [:button_done, :button_cancel, :action_create_invoice]
      :button_done -> [:button_unlock]
      :button_unlock -> [:button_done, :button_cancel, :action_create_invoice]
      :button_cancel -> [:button_draft]
      :button_draft -> [:print_quotation, :send_rfq, :button_confirm]
      :action_create_invoice -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :print_quotation -> %{max_attempts: 1, backoff_ms: 0}
      :send_rfq -> %{max_attempts: 1, backoff_ms: 0}
      :button_confirm -> %{max_attempts: 1, backoff_ms: 0}
      :button_approve -> %{max_attempts: 1, backoff_ms: 0}
      :button_done -> %{max_attempts: 1, backoff_ms: 0}
      :button_unlock -> %{max_attempts: 1, backoff_ms: 0}
      :button_cancel -> %{max_attempts: 1, backoff_ms: 0}
      :button_draft -> %{max_attempts: 1, backoff_ms: 0}
      :action_create_invoice -> %{max_attempts: 1, backoff_ms: 0}
