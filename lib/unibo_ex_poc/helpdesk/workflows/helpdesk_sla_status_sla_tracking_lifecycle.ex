...
{"steps":[{"idempotency_key":null,"next":["check_and_update","accumulate_excluded_time"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["mark_reached","mark_failed","accumulate_excluded_time"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"check_and_update"},{"idempotency_key":null,"next":["mark_failed"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_reached"},{"idempotency_key":null,"next":["accumulate_excluded_time"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_failed"},{"idempotency_key":null,"next":["check_and_update","accumulate_excluded_time"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"accumulate_excluded_time"}],"workflow":"sla_tracking_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(HelpdeskSLAStatus, :create, params), ash_opts)
      :check_and_update ->
        Ash.update(Ash.Changeset.for_update(record, :check_and_update, params), ash_opts)
      :mark_reached ->
        Ash.update(Ash.Changeset.for_update(record, :mark_reached, params), ash_opts)
      :mark_failed ->
        Ash.update(Ash.Changeset.for_update(record, :mark_failed, params), ash_opts)
      :accumulate_excluded_time ->
        Ash.update(Ash.Changeset.for_update(record, :accumulate_excluded_time, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:check_and_update, :accumulate_excluded_time]
      :check_and_update -> [:mark_reached, :mark_failed, :accumulate_excluded_time]
      :mark_reached -> [:mark_failed]
      :mark_failed -> [:accumulate_excluded_time]
      :accumulate_excluded_time -> [:check_and_update, :accumulate_excluded_time]
      _ -> []
    end
  end
  ...
  defp step_skipped?(step, record) do
    case step do
      ...
      :mark_reached -> false
      :mark_failed -> false
      :accumulate_excluded_time -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :check_and_update -> %{max_attempts: 1, backoff_ms: 0}
      :mark_reached -> %{max_attempts: 1, backoff_ms: 0}
      :mark_failed -> %{max_attempts: 1, backoff_ms: 0}
      :accumulate_excluded_time -> %{max_attempts: 1, backoff_ms: 0}
