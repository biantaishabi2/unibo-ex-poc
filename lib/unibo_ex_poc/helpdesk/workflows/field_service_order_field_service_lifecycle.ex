defmodule UniboExPoc.Helpdesk.Workflows.FieldServiceOrder.FieldServiceLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Helpdesk.FieldServiceOrder

  def steps do
    [:create, :schedule, :start_timer, :stop_timer, :add_material, :fill_worksheet, :sign, :mark_done, :cancel]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["schedule","start_timer","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["start_timer","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"schedule"},{"idempotency_key":null,"next":["stop_timer","add_material","fill_worksheet","sign","mark_done","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start_timer"},{"idempotency_key":null,"next":["start_timer","add_material","fill_worksheet","sign","mark_done","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"stop_timer"},{"idempotency_key":null,"next":["add_material","fill_worksheet","sign","stop_timer","mark_done","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"add_material"},{"idempotency_key":null,"next":["sign","add_material","stop_timer","mark_done","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"fill_worksheet"},{"idempotency_key":null,"next":["mark_done","add_material","stop_timer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"sign"},{"idempotency_key":null,"next":["cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_done"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"}],"workflow":"field_service_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(FieldServiceOrder, :create, params), ash_opts)
      :schedule ->
        Ash.update(Ash.Changeset.for_update(record, :schedule, params), ash_opts)
      :start_timer ->
        Ash.update(Ash.Changeset.for_update(record, :start_timer, params), ash_opts)
      :stop_timer ->
        Ash.update(Ash.Changeset.for_update(record, :stop_timer, params), ash_opts)
      :add_material ->
        Ash.update(Ash.Changeset.for_update(record, :add_material, params), ash_opts)
      :fill_worksheet ->
        Ash.update(Ash.Changeset.for_update(record, :fill_worksheet, params), ash_opts)
      :sign ->
        Ash.update(Ash.Changeset.for_update(record, :sign, params), ash_opts)
      :mark_done ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:schedule, :start_timer, :cancel]
      :schedule -> [:start_timer, :cancel]
      :start_timer -> [:stop_timer, :add_material, :fill_worksheet, :sign, :mark_done, :cancel]
      :stop_timer -> [:start_timer, :add_material, :fill_worksheet, :sign, :mark_done, :cancel]
      :add_material -> [:add_material, :fill_worksheet, :sign, :stop_timer, :mark_done, :cancel]
      :fill_worksheet -> [:sign, :add_material, :stop_timer, :mark_done, :cancel]
      :sign -> [:mark_done, :add_material, :stop_timer]
      :mark_done -> [:cancel]
      :cancel -> []
      _ -> []
    end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :schedule -> %{max_attempts: 1, backoff_ms: 0}
      :start_timer -> %{max_attempts: 1, backoff_ms: 0}
      :stop_timer -> %{max_attempts: 1, backoff_ms: 0}
      :add_material -> %{max_attempts: 1, backoff_ms: 0}
      :fill_worksheet -> %{max_attempts: 1, backoff_ms: 0}
      :sign -> %{max_attempts: 1, backoff_ms: 0}
