...
{"steps":[{"idempotency_key":null,"next":["join","recompute_completion","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["recompute_completion"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"join"},{"idempotency_key":null,"next":["recompute_completion"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"recompute_completion"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"enrollment_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Enrollment, :create, params), ash_opts)
      :join ->
        Ash.update(Ash.Changeset.for_update(record, :join, params), ash_opts)
      :recompute_completion ->
        Ash.update(Ash.Changeset.for_update(record, :recompute_completion, params), ash_opts)
      :destroy ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :join -> %{max_attempts: 1, backoff_ms: 0}
      :recompute_completion -> %{max_attempts: 1, backoff_ms: 0}
