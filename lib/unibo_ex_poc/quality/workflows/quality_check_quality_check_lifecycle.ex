defmodule UniboExPoc.Quality.Workflows.QualityCheck.QualityCheckLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Quality.QualityCheck
...
{"steps":[{"idempotency_key":null,"next":["do_pass","do_fail"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["do_fail"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"do_pass"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"do_fail"}],"workflow":"quality_check_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(QualityCheck, :create, params), ash_opts)
      :do_pass ->
        Ash.update(Ash.Changeset.for_update(record, :do_pass, params), ash_opts)
      :do_fail ->
        Ash.update(Ash.Changeset.for_update(record, :do_fail, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :do_pass -> %{max_attempts: 1, backoff_ms: 0}
      :do_fail -> %{max_attempts: 1, backoff_ms: 0}
