defmodule UniboExPoc.Quality.Workflows.QualityPoint.QualityPointLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Quality.QualityPoint

  def steps do
    [:create, :update, :deactivate, :activate, :trigger_checks]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","activate","deactivate","trigger_checks"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["deactivate","trigger_checks"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["activate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"deactivate"},{"idempotency_key":null,"next":["update","deactivate","trigger_checks"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"trigger_checks"}],"workflow":"quality_point_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(QualityPoint, :create, params), ash_opts)
      :update ->
      ...
      :trigger_checks ->
        Ash.update(Ash.Changeset.for_update(record, :trigger_checks, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :activate, :deactivate, :trigger_checks]
      :update -> [:deactivate, :trigger_checks]
      :deactivate -> [:activate]
      :activate -> [:update, :deactivate, :trigger_checks]
      :trigger_checks -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :trigger_checks -> %{max_attempts: 1, backoff_ms: 0}
