defmodule UniboExPoc.Quality.Workflows.QualityAlert.AlertLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Quality.QualityAlert
...
{"steps":[{"idempotency_key":null,"next":["confirm"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["start_progress"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"confirm"},{"idempotency_key":null,"next":["done","create_maintenance_request"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start_progress"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"done"}],"workflow":"alert_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(QualityAlert, :create, params), ash_opts)
      :confirm ->
        Ash.update(Ash.Changeset.for_update(record, :confirm, params), ash_opts)
      :start_progress ->
        Ash.update(Ash.Changeset.for_update(record, :start_progress, params), ash_opts)
      :done ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :start_progress -> [:done, :create_maintenance_request]
      :done -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :start_progress -> %{max_attempts: 1, backoff_ms: 0}
