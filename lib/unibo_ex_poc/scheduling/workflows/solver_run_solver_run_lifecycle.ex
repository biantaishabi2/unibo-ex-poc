defmodule UniboExPoc.Scheduling.Workflows.SolverRun.SolverRunLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Scheduling.SolverRun

  def steps do
    [:s1_create, :s2_start_run, :s3_complete_feasible, :s4_mark_timeout]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["start_run"],"next_step_ids":["s2_start_run"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create","step_id":"s1_create"},{"idempotency_key":null,"next":["complete_feasible","complete_infeasible","mark_timeout","mark_error"],"next_step_ids":["s3_complete_feasible","complete_infeasible","s4_mark_timeout","mark_error"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start_run","step_id":"s2_start_run"},{"idempotency_key":null,"next":["mark_completed"],"next_step_ids":["mark_completed"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"complete_feasible","step_id":"s3_complete_feasible"},{"idempotency_key":null,"next":["mark_completed"],"next_step_ids":["mark_completed"],"on_error":[],"on_error_step_ids":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_timeout","step_id":"s4_mark_timeout"}],"workflow":"solver_run_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :s1_create ->
        Ash.create(Ash.Changeset.for_create(SolverRun, :create, params), ash_opts)
      :s2_start_run ->
        Ash.update(Ash.Changeset.for_update(record, :start_run, params), ash_opts)
      :s3_complete_feasible ->
        Ash.update(Ash.Changeset.for_update(record, :complete_feasible, params), ash_opts)
      :s4_mark_timeout ->
        Ash.update(Ash.Changeset.for_update(record, :mark_timeout, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_start_run]
      :s2_start_run -> [:s3_complete_feasible, :complete_infeasible, :s4_mark_timeout, :mark_error]
      :s3_complete_feasible -> [:mark_completed]
      :s4_mark_timeout -> [:mark_completed]
      _ -> []
    end
  end
  ...
  defp step_skipped?(step, _record) do
    case step do
      :s1_create -> false
      :s2_start_run -> false
      :s3_complete_feasible -> false
      :s4_mark_timeout -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s2_start_run -> %{max_attempts: 1, backoff_ms: 0}
      :s3_complete_feasible -> %{max_attempts: 1, backoff_ms: 0}
      :s4_mark_timeout -> %{max_attempts: 1, backoff_ms: 0}
