defmodule UniboExPoc.Ecommerce.Workflows.ShoppingCart.ShoppingCartLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Ecommerce.ShoppingCart

  def steps do
    [:create, :update, :cart_update, :update_pricelist, :select_carrier, :convert, :recover]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","cart_update","update_pricelist","select_carrier"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","cart_update","update_pricelist","select_carrier","convert"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","cart_update","update_pricelist","select_carrier","convert"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cart_update"},{"idempotency_key":null,"next":["update","cart_update","select_carrier","convert"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update_pricelist"},{"idempotency_key":null,"next":["update","cart_update","update_pricelist","convert"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"select_carrier"},{"idempotency_key":null,"next":["recover"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"convert"},{"idempotency_key":null,"next":["update","cart_update","update_pricelist","select_carrier","convert"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"recover"}],"workflow":"shopping_cart_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(ShoppingCart, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :cart_update ->
        Ash.update(Ash.Changeset.for_update(record, :cart_update, params), ash_opts)
      :update_pricelist ->
        Ash.update(Ash.Changeset.for_update(record, :update_pricelist, params), ash_opts)
      :select_carrier ->
        Ash.update(Ash.Changeset.for_update(record, :select_carrier, params), ash_opts)
      :convert ->
        Ash.update(Ash.Changeset.for_update(record, :convert, params), ash_opts)
      :recover ->
        Ash.update(Ash.Changeset.for_update(record, :recover, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :cart_update, :update_pricelist, :select_carrier]
      :update -> [:update, :cart_update, :update_pricelist, :select_carrier, :convert]
      :cart_update -> [:update, :cart_update, :update_pricelist, :select_carrier, :convert]
      :update_pricelist -> [:update, :cart_update, :select_carrier, :convert]
      :select_carrier -> [:update, :cart_update, :update_pricelist, :convert]
      :convert -> [:recover]
      :recover -> [:update, :cart_update, :update_pricelist, :select_carrier, :convert]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :cart_update -> %{max_attempts: 1, backoff_ms: 0}
      :update_pricelist -> %{max_attempts: 1, backoff_ms: 0}
      :select_carrier -> %{max_attempts: 1, backoff_ms: 0}
      :convert -> %{max_attempts: 1, backoff_ms: 0}
      :recover -> %{max_attempts: 1, backoff_ms: 0}
