defmodule UniboExPoc.Lunch.Workflows.LunchSupplier.SupplierLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Lunch.LunchSupplier
...
{"steps":[{"idempotency_key":null,"next":["update","send_orders","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","send_orders","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["confirm_orders","send_orders"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"send_orders"},{"idempotency_key":null,"next":["send_orders","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"confirm_orders"},{"idempotency_key":null,"next":["unarchive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"},{"idempotency_key":null,"next":["update","send_orders","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"unarchive"}],"workflow":"supplier_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LunchSupplier, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :send_orders ->
        Ash.update(Ash.Changeset.for_update(record, :send_orders, params), ash_opts)
      :confirm_orders ->
        Ash.update(Ash.Changeset.for_update(record, :confirm_orders, params), ash_opts)
      :archive ->
        Ash.update(Ash.Changeset.for_update(record, :archive, params), ash_opts)
      :unarchive ->
        Ash.update(Ash.Changeset.for_update(record, :unarchive, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :send_orders, :archive]
      :update -> [:update, :send_orders, :archive]
      :send_orders -> [:confirm_orders, :send_orders]
      :confirm_orders -> [:send_orders, :archive]
      :archive -> [:unarchive]
      :unarchive -> [:update, :send_orders, :archive]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :send_orders -> %{max_attempts: 1, backoff_ms: 0}
      :confirm_orders -> %{max_attempts: 1, backoff_ms: 0}
      :archive -> %{max_attempts: 1, backoff_ms: 0}
      :unarchive -> %{max_attempts: 1, backoff_ms: 0}
