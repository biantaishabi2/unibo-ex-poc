...
{"steps":[{"idempotency_key":null,"next":["action_order"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["action_send","action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_order"},{"idempotency_key":null,"next":["action_confirm","action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_send"},{"idempotency_key":null,"next":["action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_confirm"},{"idempotency_key":null,"next":["action_reset"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_cancel"},{"idempotency_key":null,"next":["action_send","action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_reset"}],"workflow":"order_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LunchOrder, :create, params), ash_opts)
      :action_order ->
        Ash.update(Ash.Changeset.for_update(record, :action_order, params), ash_opts)
      :action_send ->
        Ash.update(Ash.Changeset.for_update(record, :action_send, params), ash_opts)
      :action_confirm ->
      ...
      :action_reset ->
        Ash.update(Ash.Changeset.for_update(record, :action_reset, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:action_order]
      :action_order -> [:action_send, :action_cancel]
      :action_send -> [:action_confirm, :action_cancel]
      :action_confirm -> [:action_cancel]
      :action_cancel -> [:action_reset]
      :action_reset -> [:action_send, :action_cancel]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_order -> %{max_attempts: 1, backoff_ms: 0}
      :action_send -> %{max_attempts: 1, backoff_ms: 0}
      :action_confirm -> %{max_attempts: 1, backoff_ms: 0}
      :action_cancel -> %{max_attempts: 1, backoff_ms: 0}
      :action_reset -> %{max_attempts: 1, backoff_ms: 0}
