defmodule UniboExPoc.Loyalty.Workflows.LoyaltyProgram.ProgramLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Loyalty.LoyaltyProgram
...
{"steps":[{"idempotency_key":null,"next":["update","activate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","activate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["pause","expire"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":["activate","expire"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"pause"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"expire"}],"workflow":"program_lifecycle_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LoyaltyProgram, :create, params), ash_opts)
      :update ->
