...
{"steps":[{"idempotency_key":null,"next":["cancel_transaction"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel_transaction"}],"workflow":"transaction_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(LoyaltyTransaction, :create, params), ash_opts)
      :cancel_transaction ->
        Ash.update(Ash.Changeset.for_update(record, :cancel_transaction, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :cancel_transaction -> %{max_attempts: 1, backoff_ms: 0}
