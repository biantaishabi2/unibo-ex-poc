...
{"steps":[{"idempotency_key":null,"next":["sync_procurement_stock_impact"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"detect_component_delta"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"sync_procurement_stock_impact"}],"workflow":"bom_component_delta_impact"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :detect_component_delta ->
        Ash.update(Ash.Changeset.for_update(record, :detect_component_delta, params), ash_opts)
      :sync_procurement_stock_impact ->
        Ash.update(Ash.Changeset.for_update(record, :sync_procurement_stock_impact, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :detect_component_delta -> %{max_attempts: 1, backoff_ms: 0}
      :sync_procurement_stock_impact -> %{max_attempts: 1, backoff_ms: 0}
