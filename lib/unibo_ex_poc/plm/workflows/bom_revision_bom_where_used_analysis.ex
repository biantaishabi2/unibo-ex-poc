defmodule UniboExPoc.PLM.Workflows.BomRevision.BomWhereUsedAnalysisWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
...
{"steps":[{"idempotency_key":null,"next":["build_recursive_bom_tree"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"collect_component_usages"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"build_recursive_bom_tree"}],"workflow":"bom_where_used_analysis"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :collect_component_usages ->
        Ash.update(Ash.Changeset.for_update(record, :collect_component_usages, params), ash_opts)
      :build_recursive_bom_tree ->
        Ash.update(Ash.Changeset.for_update(record, :build_recursive_bom_tree, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp branch_next(step, record) do
    _ = record
    case step do
      :collect_component_usages -> nil
      :build_recursive_bom_tree -> nil
      _ -> nil
    end
  end
  ...
  defp step_skipped?(step, record) do
    _ = record
    case step do
      :collect_component_usages -> false
      :build_recursive_bom_tree -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :collect_component_usages -> %{max_attempts: 1, backoff_ms: 0}
      :build_recursive_bom_tree -> %{max_attempts: 1, backoff_ms: 0}
