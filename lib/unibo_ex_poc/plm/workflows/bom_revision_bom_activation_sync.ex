...
{"steps":[{"idempotency_key":null,"next":["sync_draft_mo"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"evaluate_activation"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"sync_draft_mo"}],"workflow":"bom_activation_sync"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :evaluate_activation ->
        Ash.update(Ash.Changeset.for_update(record, :evaluate_activation, params), ash_opts)
      :sync_draft_mo ->
        Ash.update(Ash.Changeset.for_update(record, :sync_draft_mo, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :evaluate_activation -> %{max_attempts: 1, backoff_ms: 0}
      :sync_draft_mo -> %{max_attempts: 1, backoff_ms: 0}
