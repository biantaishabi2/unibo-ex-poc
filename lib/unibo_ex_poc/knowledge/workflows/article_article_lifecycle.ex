...
{"steps":[{"idempotency_key":null,"next":["update","action_publish","action_trash","action_move","action_copy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","action_publish","action_trash","action_move","action_copy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["action_unpublish","action_archive","action_lock"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_publish"},{"idempotency_key":null,"next":["action_publish","action_move","action_copy","action_trash"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_unpublish"},{"idempotency_key":null,"next":["action_restore_state"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_archive"},{"idempotency_key":null,"next":["action_publish","action_move","action_trash"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_restore_state"},{"idempotency_key":null,"next":["action_restore_trash"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_trash"},{"idempotency_key":null,"next":["action_move","action_publish","action_archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_restore_trash"},{"idempotency_key":null,"next":["action_unlock"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_lock"},{"idempotency_key":null,"next":["update","action_publish","action_archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_unlock"},{"idempotency_key":null,"next":["update","action_publish","action_archive","action_trash"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_move"},{"idempotency_key":null,"next":["action_publish","action_move"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_copy"}],"workflow":"article_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Article, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :action_publish ->
        Ash.update(Ash.Changeset.for_update(record, :action_publish, params), ash_opts)
      :action_unpublish ->
        Ash.update(Ash.Changeset.for_update(record, :action_unpublish, params), ash_opts)
      :action_archive ->
        Ash.update(Ash.Changeset.for_update(record, :action_archive, params), ash_opts)
      :action_restore_state ->
        Ash.update(Ash.Changeset.for_update(record, :action_restore_state, params), ash_opts)
      :action_trash ->
        Ash.update(Ash.Changeset.for_update(record, :action_trash, params), ash_opts)
      :action_restore_trash ->
        Ash.update(Ash.Changeset.for_update(record, :action_restore_trash, params), ash_opts)
      :action_lock ->
        Ash.update(Ash.Changeset.for_update(record, :action_lock, params), ash_opts)
      :action_unlock ->
        Ash.update(Ash.Changeset.for_update(record, :action_unlock, params), ash_opts)
      :action_move ->
        Ash.update(Ash.Changeset.for_update(record, :action_move, params), ash_opts)
      :action_copy ->
        Ash.create(Ash.Changeset.for_create(Article, :action_copy, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :action_publish, :action_trash, :action_move, :action_copy]
      :update -> [:update, :action_publish, :action_trash, :action_move, :action_copy]
      :action_publish -> [:action_unpublish, :action_archive, :action_lock]
      :action_unpublish -> [:action_publish, :action_move, :action_copy, :action_trash]
      :action_archive -> [:action_restore_state]
      :action_restore_state -> [:action_publish, :action_move, :action_trash]
      :action_trash -> [:action_restore_trash]
      :action_restore_trash -> [:action_move, :action_publish, :action_archive]
      :action_lock -> [:action_unlock]
      :action_unlock -> [:update, :action_publish, :action_archive]
      :action_move -> [:update, :action_publish, :action_archive, :action_trash]
      :action_copy -> [:action_publish, :action_move]
      _ -> []
    end
  end
  ...
  defp branch_next(step, record) do
    ...
    case step do
      ...
      :update -> nil
      :action_publish -> nil
      :action_unpublish -> nil
      :action_archive -> nil
      :action_restore_state -> nil
      :action_trash -> nil
      :action_restore_trash -> nil
      :action_lock -> nil
      :action_unlock -> nil
      :action_move -> nil
      :action_copy -> nil
  ...
  defp step_skipped?(step, record) do
    ...
    case step do
      ...
      :action_unpublish -> false
      :action_archive -> false
      :action_restore_state -> false
      :action_trash -> false
      :action_restore_trash -> false
      :action_lock -> false
      :action_unlock -> false
      :action_move -> false
      :action_copy -> false
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_publish -> %{max_attempts: 1, backoff_ms: 0}
      :action_unpublish -> %{max_attempts: 1, backoff_ms: 0}
      :action_archive -> %{max_attempts: 1, backoff_ms: 0}
      :action_restore_state -> %{max_attempts: 1, backoff_ms: 0}
      :action_trash -> %{max_attempts: 1, backoff_ms: 0}
      :action_restore_trash -> %{max_attempts: 1, backoff_ms: 0}
      :action_lock -> %{max_attempts: 1, backoff_ms: 0}
      :action_unlock -> %{max_attempts: 1, backoff_ms: 0}
      :action_move -> %{max_attempts: 1, backoff_ms: 0}
      :action_copy -> %{max_attempts: 1, backoff_ms: 0}
