defmodule UniboExPoc.Calendar.Workflows.WeekException.WeekExceptionManageFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Calendar.WeekException
...
{"steps":[{"idempotency_key":null,"next":["remove"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"add"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"remove"}],"workflow":"week_exception_manage_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :add ->
        Ash.create(Ash.Changeset.for_create(WeekException, :add, params), ash_opts)
      :remove ->
  ...
  defp retry_policy(step) do
    case step do
      :add -> %{max_attempts: 1, backoff_ms: 0}
