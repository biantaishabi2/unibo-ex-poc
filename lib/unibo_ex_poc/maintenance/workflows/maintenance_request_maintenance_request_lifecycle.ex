defmodule UniboExPoc.Maintenance.Workflows.MaintenanceRequest.MaintenanceRequestLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Maintenance.MaintenanceRequest

  def steps do
    [:create, :update, :change_stage, :cancel]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","change_stage","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["change_stage","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["change_stage","cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"change_stage"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"}],"workflow":"maintenance_request_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(MaintenanceRequest, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :change_stage, :cancel]
      :update -> [:change_stage, :cancel]
      :change_stage -> [:change_stage, :cancel]
      :cancel -> []
      _ -> []
    end
  end
