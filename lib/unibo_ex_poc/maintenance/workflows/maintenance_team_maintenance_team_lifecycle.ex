defmodule UniboExPoc.Maintenance.Workflows.MaintenanceTeam.MaintenanceTeamLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Maintenance.MaintenanceTeam

  def steps do
    [:create, :update, :add_member, :remove_member]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","add_member"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["add_member"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["remove_member","add_member"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"add_member"},{"idempotency_key":null,"next":["add_member"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"remove_member"}],"workflow":"maintenance_team_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(MaintenanceTeam, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :add_member ->
        Ash.update(Ash.Changeset.for_update(record, :add_member, params), ash_opts)
      :remove_member ->
        Ash.update(Ash.Changeset.for_update(record, :remove_member, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :add_member]
      :update -> [:add_member]
      :add_member -> [:remove_member, :add_member]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :add_member -> %{max_attempts: 1, backoff_ms: 0}
      :remove_member -> %{max_attempts: 1, backoff_ms: 0}
