...
{"steps":[{"idempotency_key":null,"next":["update","change_driver","set_future_driver","change_state","deactivate","set_odometer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["change_driver","set_future_driver","change_state","deactivate","set_odometer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","set_future_driver","change_state","deactivate","set_odometer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"change_driver"},{"idempotency_key":null,"next":["change_driver"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"set_future_driver"},{"idempotency_key":null,"next":["update","deactivate"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"change_state"},{"idempotency_key":null,"next":["set_odometer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"deactivate"},{"idempotency_key":null,"next":["set_odometer"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"set_odometer"}],"workflow":"vehicle_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Vehicle, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :change_driver ->
        Ash.update(Ash.Changeset.for_update(record, :change_driver, params), ash_opts)
      :set_future_driver ->
        Ash.update(Ash.Changeset.for_update(record, :set_future_driver, params), ash_opts)
      :change_state ->
        Ash.update(Ash.Changeset.for_update(record, :change_state, params), ash_opts)
      :deactivate ->
        Ash.update(Ash.Changeset.for_update(record, :deactivate, params), ash_opts)
      :set_odometer ->
        Ash.update(Ash.Changeset.for_update(record, :set_odometer, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :change_driver, :set_future_driver, :change_state, :deactivate, :set_odometer]
      :update -> [:change_driver, :set_future_driver, :change_state, :deactivate, :set_odometer]
      :change_driver -> [:update, :set_future_driver, :change_state, :deactivate, :set_odometer]
      :set_future_driver -> [:change_driver]
      :change_state -> [:update, :deactivate]
      :deactivate -> [:set_odometer]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :change_driver -> %{max_attempts: 1, backoff_ms: 0}
      :set_future_driver -> %{max_attempts: 1, backoff_ms: 0}
      :change_state -> %{max_attempts: 1, backoff_ms: 0}
      :deactivate -> %{max_attempts: 1, backoff_ms: 0}
      :set_odometer -> %{max_attempts: 1, backoff_ms: 0}
