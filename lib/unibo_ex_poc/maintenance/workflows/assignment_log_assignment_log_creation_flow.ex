defmodule UniboExPoc.Maintenance.Workflows.AssignmentLog.AssignmentLogCreationFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Maintenance.AssignmentLog
...
{"steps":[{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"}],"workflow":"assignment_log_creation_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(AssignmentLog, :create, params), ash_opts)
      _ -> {:ok, record}
    end
  end
