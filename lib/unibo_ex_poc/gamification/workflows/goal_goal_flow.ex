defmodule UniboExPoc.Gamification.Workflows.Goal.GoalFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Gamification.Goal
...
{"steps":[{"idempotency_key":null,"next":["update","action_start"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["update","action_start","action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["action_reach","action_fail","action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_start"},{"idempotency_key":null,"next":["action_fail"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_reach"},{"idempotency_key":null,"next":["action_cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_fail"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"action_cancel"}],"workflow":"goal_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Goal, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :action_start ->
        Ash.update(Ash.Changeset.for_update(record, :action_start, params), ash_opts)
      :action_reach ->
        Ash.update(Ash.Changeset.for_update(record, :action_reach, params), ash_opts)
      :action_fail ->
        Ash.update(Ash.Changeset.for_update(record, :action_fail, params), ash_opts)
      :action_cancel ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :action_start]
      :update -> [:update, :action_start, :action_cancel]
      :action_start -> [:action_reach, :action_fail, :action_cancel]
      :action_reach -> [:action_fail]
      :action_fail -> [:action_cancel]
      :action_cancel -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :action_start -> %{max_attempts: 1, backoff_ms: 0}
      :action_reach -> %{max_attempts: 1, backoff_ms: 0}
      :action_fail -> %{max_attempts: 1, backoff_ms: 0}
