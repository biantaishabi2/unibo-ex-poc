defmodule UniboExPoc.POS.Workflows.PosPayment.PaymentCreationWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.POS.PosPayment
...
{"steps":[{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"}],"workflow":"payment_creation"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(PosPayment, :create, params), ash_opts)
      _ -> {:ok, record}
    end
  end
