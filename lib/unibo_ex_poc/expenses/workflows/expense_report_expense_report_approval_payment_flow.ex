...
{"steps":[{"idempotency_key":null,"next":["submit"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["approve"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"submit"},{"idempotency_key":null,"next":["post"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"approve"},{"idempotency_key":null,"next":["register_payment"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"post"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"register_payment"}],"workflow":"expense_report_approval_payment_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(ExpenseReport, :create, params), ash_opts)
      :submit ->
      ...
      :register_payment ->
        Ash.update(Ash.Changeset.for_update(record, :register_payment, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :post -> %{max_attempts: 1, backoff_ms: 0}
      :register_payment -> %{max_attempts: 1, backoff_ms: 0}
