defmodule UniboExPoc.Rating.Workflows.RatingCriteria.RatingCriteriaLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Rating.RatingCriteria

  def steps do
    [:create, :update, :reorder, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","reorder","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["reorder","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reorder"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"rating_criteria_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(RatingCriteria, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :reorder ->
        Ash.update(Ash.Changeset.for_update(record, :reorder, params), ash_opts)
      :destroy ->
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :reorder, :destroy]
      :update -> [:reorder, :destroy]
      :reorder -> [:update, :destroy]
      :destroy -> []
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :reorder -> %{max_attempts: 1, backoff_ms: 0}
