...
{"steps":[{"idempotency_key":null,"next":["update","activate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["activate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"activate"},{"idempotency_key":null,"next":["reactivate","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"archive"},{"idempotency_key":null,"next":["update","archive"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"reactivate"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"custom_model_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(CustomModel, :create, params), ash_opts)
      :update ->
  ...
  defp next_candidates(step) do
    case step do
      ...
      :archive -> [:reactivate, :destroy]
      :reactivate -> [:update, :archive]
      :destroy -> []
      _ -> []
    end
  end
