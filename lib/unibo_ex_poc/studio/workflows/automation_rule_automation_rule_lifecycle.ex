defmodule UniboExPoc.Studio.Workflows.AutomationRule.AutomationRuleLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Studio.AutomationRule

  def steps do
    [:create, :update, :execute, :destroy]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","execute","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["execute","destroy"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["update","execute"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"execute"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"destroy"}],"workflow":"automation_rule_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(AutomationRule, :create, params), ash_opts)
      :update ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :execute -> %{max_attempts: 1, backoff_ms: 0}
