...
{"steps":[{"idempotency_key":null,"next":["mark_in_progress"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["mark_done"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_in_progress"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"mark_done"}],"workflow":"answering_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(UserInput, :create, params), ash_opts)
      :mark_in_progress ->
        Ash.update(Ash.Changeset.for_update(record, :mark_in_progress, params), ash_opts)
      :mark_done ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :mark_in_progress -> %{max_attempts: 1, backoff_ms: 0}
