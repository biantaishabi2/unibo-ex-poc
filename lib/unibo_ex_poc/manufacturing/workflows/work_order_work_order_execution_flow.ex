...
{"steps":[{"idempotency_key":null,"next":["start"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["pause"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start"},{"idempotency_key":null,"next":["resume"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"pause"},{"idempotency_key":null,"next":["finish"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"resume"},{"idempotency_key":null,"next":["complete"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"finish"},{"idempotency_key":null,"next":["cancel"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"complete"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"cancel"}],"workflow":"work_order_execution_flow"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(WorkOrder, :create, params), ash_opts)
      :start ->
      ...
      :resume ->
        Ash.update(Ash.Changeset.for_update(record, :resume, params), ash_opts)
      :finish ->
        Ash.update(Ash.Changeset.for_update(record, :finish, params), ash_opts)
      :complete ->
  ...
  defp retry_policy(step) do
    case step do
      ...
      :resume -> %{max_attempts: 1, backoff_ms: 0}
      :finish -> %{max_attempts: 1, backoff_ms: 0}
