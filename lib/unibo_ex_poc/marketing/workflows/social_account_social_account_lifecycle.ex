defmodule UniboExPoc.Marketing.Workflows.SocialAccount.SocialAccountLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Marketing.SocialAccount

  def steps do
    [:create, :connect, :disconnect, :refresh_token]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["connect"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["disconnect","refresh_token"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"connect"},{"idempotency_key":null,"next":["refresh_token"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"disconnect"},{"idempotency_key":null,"next":["disconnect","refresh_token"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"refresh_token"}],"workflow":"social_account_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SocialAccount, :create, params), ash_opts)
      :connect ->
        Ash.update(Ash.Changeset.for_update(record, :connect, params), ash_opts)
      :disconnect ->
        Ash.update(Ash.Changeset.for_update(record, :disconnect, params), ash_opts)
      :refresh_token ->
        Ash.update(Ash.Changeset.for_update(record, :refresh_token, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:connect]
      :connect -> [:disconnect, :refresh_token]
      :disconnect -> [:refresh_token]
      :refresh_token -> [:disconnect, :refresh_token]
      _ -> []
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :connect -> %{max_attempts: 1, backoff_ms: 0}
      :disconnect -> %{max_attempts: 1, backoff_ms: 0}
      :refresh_token -> %{max_attempts: 1, backoff_ms: 0}
