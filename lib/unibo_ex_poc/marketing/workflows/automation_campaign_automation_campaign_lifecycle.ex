defmodule UniboExPoc.Marketing.Workflows.AutomationCampaign.AutomationCampaignLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Marketing.AutomationCampaign

  def steps do
    [:create, :update, :start_campaign, :stop_campaign]
  end

  @workflow_semantics_json ~S"""
{"steps":[{"idempotency_key":null,"next":["update","start_campaign"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["start_campaign"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["stop_campaign"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"start_campaign"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"stop_campaign"}],"workflow":"automation_campaign_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(AutomationCampaign, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :start_campaign ->
        Ash.update(Ash.Changeset.for_update(record, :start_campaign, params), ash_opts)
      :stop_campaign ->
        Ash.update(Ash.Changeset.for_update(record, :stop_campaign, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :start_campaign -> %{max_attempts: 1, backoff_ms: 0}
      :stop_campaign -> %{max_attempts: 1, backoff_ms: 0}
