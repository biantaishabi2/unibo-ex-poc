defmodule UniboExPoc.Marketing.Workflows.SocialPost.SocialPostLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias UniboExPoc.Marketing.SocialPost
...
{"steps":[{"idempotency_key":null,"next":["update","publish_now","schedule"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"create"},{"idempotency_key":null,"next":["publish_now","schedule"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"update"},{"idempotency_key":null,"next":["sync_stats"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"publish_now"},{"idempotency_key":null,"next":["sync_stats"],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"schedule"},{"idempotency_key":null,"next":[],"on_error":[],"retry":{"backoff_ms":0,"max_attempts":1},"step":"sync_stats"}],"workflow":"social_post_lifecycle"}
"""
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(SocialPost, :create, params), ash_opts)
      :update ->
        Ash.update(Ash.Changeset.for_update(record, :update, params), ash_opts)
      :publish_now ->
        Ash.update(Ash.Changeset.for_update(record, :publish_now, params), ash_opts)
      :schedule ->
        Ash.update(Ash.Changeset.for_update(record, :schedule, params), ash_opts)
      :sync_stats ->
        Ash.update(Ash.Changeset.for_update(record, :sync_stats, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp next_candidates(step) do
    case step do
      :create -> [:update, :publish_now, :schedule]
      :update -> [:publish_now, :schedule]
  ...
  defp retry_policy(step) do
    case step do
      ...
      :publish_now -> %{max_attempts: 1, backoff_ms: 0}
      :schedule -> %{max_attempts: 1, backoff_ms: 0}
      :sync_stats -> %{max_attempts: 1, backoff_ms: 0}
