defmodule UniboV4.Membership.Workflows.ResPartner.PartnerMembershipInvoiceFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  ...
  defp apply_step(record, step, params, opts) do
    ...
    case step do
      :create_membership_invoice ->
        Ash.update(Ash.Changeset.for_update(record, :create_membership_invoice, params), ash_opts)
      _ -> {:ok, record}
    end
  end
  ...
  defp step_skipped?(step, record) do
    case step do
      :create_membership_invoice -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      :create_membership_invoice -> %{max_attempts: 1, backoff_ms: 0}
