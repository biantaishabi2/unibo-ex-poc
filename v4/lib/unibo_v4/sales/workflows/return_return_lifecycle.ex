defmodule UniboV4.Sales.Workflows.Return.ReturnLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  """

  alias UniboV4.Sales.Return
  ...
  defp apply_step(record, step, opts) do
    ...
    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(Return, :create, params), actor: actor)
      :approve ->
        Ash.update(Ash.Changeset.for_update(record, :approve, params), actor: actor)
      :receive ->
        Ash.update(Ash.Changeset.for_update(record, :receive, params), actor: actor)
      :complete ->
