defmodule UniboV4.Loyalty.Workflows.GiftCard.GiftcardLifecycleFlowWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  """

  alias UniboV4.Loyalty.GiftCard

  def steps do
    [:create, :activate, :topup, :charge, :cancel_card, :expire_card]
  end

  def run(record, opts \\ []) do
    Enum.reduce_while(steps(), {:ok, record}, fn step, {:ok, current} ->
      case apply_step(current, step, opts) do
        {:ok, next_record} -> {:cont, {:ok, next_record}}
        {:error, reason} -> {:halt, {:error, %{step: step, reason: reason}}}
      end
    end)
  end

  defp apply_step(record, step, opts) do
    actor = Keyword.get(opts, :actor)
    params_by_step = Keyword.get(opts, :params, %{})
    params = Map.get(params_by_step, step, %{})

    case step do
      :create ->
        Ash.create(Ash.Changeset.for_create(GiftCard, :create, params), actor: actor)
      :activate ->
        Ash.update(Ash.Changeset.for_update(record, :activate, params), actor: actor)
      :topup ->
        Ash.update(Ash.Changeset.for_update(record, :topup, params), actor: actor)
      :charge ->
        Ash.update(Ash.Changeset.for_update(record, :charge, params), actor: actor)
      :cancel_card ->
        Ash.update(Ash.Changeset.for_update(record, :cancel_card, params), actor: actor)
      :expire_card ->
        Ash.update(Ash.Changeset.for_update(record, :expire_card, params), actor: actor)
      _ -> {:ok, record}
    end
  end
end
