...
- 说明：机场主数据（Travel 层，来源 OFBiz Geo）
...
- 说明：火车站主数据（Travel 层，来源 OFBiz Geo）
...
- Travel::TravelHotel 引用 ID（优先）（hotel_ref_id）：唯一标识
- 酒店名称（hotel_name）：文本（必填）
- 城市编码：文本（必填），说明 城市编码
- 城市参考编号：唯一标识，说明 Core::Ecommerce.TravelCity 引用 ID（优先）
- 房型编码（room_type_code）：文本（必填）
- Travel::TravelRoomType 引用 ID（优先）（room_type_ref_id）：唯一标识
...
- 航班号（flight_no）：文本（必填）
- Travel::TravelAirline 引用 ID（优先）（airline_ref_id）：唯一标识
- 出发机场编码（departure_airport_code）：文本（必填）
- Travel::TravelAirport 出发机场引用 ID（优先）（departure_airport_ref_id）：唯一标识
- 到达机场编码（arrival_airport_code）：文本（必填）
- Travel::TravelAirport 到达机场引用 ID（优先）（arrival_airport_ref_id）：唯一标识
- 离职：日期时间（必填），说明 起飞时间
- 到达：日期时间（必填），说明 到达时间
- 舱等（cabin_class）：文本（必填）
- Travel::TravelCabinClass 引用 ID（优先）（cabin_class_ref_id）：唯一标识
- 运价族（fare_family）：文本
- 对客展示价快照（listed_price）：金额（必填），必须 ≥ 0（listed_price 不能为负数）
- 结算价格：金额，必须 ≥ 0（settlement_price 不能为负数），说明 结算价快照
- 币种：文本，默认值 CNY
...
- 离职城市参考编号：唯一标识，说明 Core::Ecommerce.TravelCity 出发城市引用 ID（优先）
- 目的地编码：文本（必填），说明 目的地编码
- 目的地参考编号：唯一标识，说明 Core::Ecommerce.TravelCity 目的地引用 ID（优先）
...
- Travel::TravelStation 出发站引用 ID（优先）（departure_station_ref_id）：唯一标识
- 出发站名称（departure_station_name）：文本（必填）
- 到达站编码（arrival_station_code）：文本（必填）
- Travel::TravelStation 到达站引用 ID（优先）（arrival_station_ref_id）：唯一标识
...
- 酒店 offer 引用（hotel_offer_id）：唯一标识
- 机票 offer 引用（flight_offer_id）：唯一标识
- 假期录用通知编号：唯一标识，说明 度假 offer 引用
- 火车票 offer 引用（train_offer_id）：唯一标识
- 订单无：文本（必填），说明 订单号
- 产品类型：枚举，默认值 hotel，可选值：hotel / flight / 假期 / train，说明 商品类型
- train 订单预订模式（booking_mode）：枚举，默认值 标准，可选值：标准 / waitlist
- 客户编号：唯一标识（必填），说明 买家 ID（关联 Sales 域 Customer）
- 付款编号：唯一标识，说明 关联的支付事务 ID（关联 Payment 域 Payment）
- 联系人：文本（必填）
- 联系电话：文本（必填）
...
- 创建订单时，hotel_offer_id conditional_present（hotel 订单必须绑定 hotel_offer_id）
- 创建订单时，flight_offer_id conditional_present（flight 订单必须绑定 flight_offer_id）
- 创建订单时，假期录用通知编号 conditional_present（vacation 订单必须绑定 vacation_offer_id）
- 创建订单时，train_offer_id conditional_present（train 订单必须绑定 train_offer_id）
...
- 编号：唯一标识（主键、自动生成）
- 租户编号：唯一标识（必填）
- travel_order_id：唯一标识（必填）
- 发货编号：唯一标识，说明 关联的物流发货单 ID（可选，关联 Delivery 域 Shipment）
...
#### 关系
- 订单：多对一 -> TravelOrder，外键 travel_order_id
- 发货：多对一 -> 发货，外键 发货编号
