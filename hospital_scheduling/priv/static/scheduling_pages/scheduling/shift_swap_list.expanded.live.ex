defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  # status.keys preview (first ~40): rows, rows[], rows[].max_night_shifts, rows[].notes, rows[].preferred_shift_tags, rows[].unavailable_dates, rows_empty, shift_preference, shift_preference.page, shift_preference.total_pages
  ...
  @impl true
  def handle_event("shift_preference_page_next", params, socket) do
    # UI action event name: shift_preference_page_next
    socket = dispatch_backend("shift_preference_page_next", params, socket)
    {:noreply, socket}
  end

  @impl true
  def handle_event("shift_preference_page_prev", params, socket) do
    # UI action event name: shift_preference_page_prev
    socket = dispatch_backend("shift_preference_page_prev", params, socket)
    {:noreply, socket}
  end
