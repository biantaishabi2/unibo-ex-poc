defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  @page_id "shift_type_list"
  @page_title "Untitled Page"

  # status.keys preview (first ~40): page_title, rows, rows[], rows[].code, rows[].color, rows[].duration_hours, rows[].enabled, rows[].end_time, rows[].id, rows[].is_night, rows[].name, rows[].sort_order, rows[].start_time, rows_empty
  # Defaults are used for dev/mock transitions (e.g. toggle_list_empty restore).
  @status_defaults_raw Jason.decode!("{
  \"page_title\": \"ShiftType 列表\",
  \"rows_empty\": true,
  \"rows\": [
    {
      \"code\": \"\",
  ...
  @backend_load_selection "code color duration_hours: durationHours enabled end_time: endTime id is_night: isNight name sort_order: sortOrder start_time: startTime"
  @backend_load_assigns %{}
  @backend_params_accept []
  @backend_info_reload_messages ["page_host_reload"]
  @backend_api_map %{
    "list" => %{module: __MODULE__.Backend, fun: :handle_event, api: "Scheduling.ShiftType.list"}
  }
