...
test.describe("scheduling_constraint_detail", () => {
  test('DTO smoke', async ({ page }) => {
    await page.goto("http://localhost:4002/scheduling/scheduling_constraint_detail");
    ...
    await expect(page.locator(`#scheduling_constraint_bc_current`)).toContainText(`详情`, { timeout: 15000 });
  });
  ...
  test('E2E flow', async ({ page }) => {
    ...
    await test.step("页面加载 + 结构可见", async () => {
      await expect(page.locator(`[data-page='scheduling_constraint_detail']`)).toBeVisible({ timeout: 15000 });
    });
    ...
    await test.step("动作冒烟：action_destroy", async () => {
      {
        const loc = page.locator(`#scheduling_constraint_delete_btn`);
        await loc.waitFor({ state: 'visible', timeout: 15000 });
        await loc.scrollIntoViewIfNeeded();
        await loc.click({ timeout: 15000 });
      }
