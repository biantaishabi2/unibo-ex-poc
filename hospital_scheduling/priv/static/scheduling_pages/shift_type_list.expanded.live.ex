defmodule MyAppWeb.Pages.StitchGeneratedLive do
  ...
  # status.keys preview (first ~40): page_title, rows, rows[], rows[].code, rows[].color, rows[].duration_hours, rows[].enabled, rows[].end_time, rows[].is_night, rows[].name, rows[].sort_order, rows[].start_time, rows_empty
