defmodule HospitalScheduling.Scheduling.Integrations.SolverRun.StartRunRustSolverInvokeBridge do
  use Ash.Resource.Change

  @provider "rust_solver_invoke"
  @queue "solver_jobs"
  @max_attempts 3
  @idempotency_key_source "period_id"
  @dead_letter_queue nil
  @request_bindings []

  @impl true
  def change(changeset, _opts, context) do
    request_id = resolve_context_path(context, "correlation.request_id") || Map.get(context, :request_id) || "solver_run.start_run.rust_solver_invoke"
    dedup_key = resolve_dedup_key(changeset, context, request_id)
    payload =
      %{
        "entity" => "solver_run",
        "action" => "start_run",
        "integration" => "rust_solver_invoke",
        "provider" => @provider,
    ...
    if Code.ensure_loaded?(HospitalScheduling.AsyncRuntime.Queue) and function_exported?(HospitalScheduling.AsyncRuntime.Queue, :enqueue, 1) do
      case HospitalScheduling.AsyncRuntime.Queue.enqueue(task) do
  ...
  defp resolve_dedup_key(changeset, context, request_id) do
    ...
    "integration:solver_run:start_run:rust_solver_invoke:" <> key
  end
