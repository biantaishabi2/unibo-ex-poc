defmodule HospitalScheduling.Scheduling.SolverAdapter do
  ...
  @spec solve(map(), keyword()) :: {:ok, map()} | {:error, term()}
  def solve(snapshot, opts \\ []) when is_map(snapshot) do
    with {:ok, input_snapshot} <- Jason.encode(snapshot),
         {:ok, %{decoded: decoded}} <- run_snapshot(input_snapshot, opts) do
      {:ok, decoded}
    end
  end
