defmodule HospitalScheduling.Scheduling.Workflows.SchedulingPeriod.SchedulingPeriodLifecycleWorkflow do
  @moduledoc """
  自动生成的工作流编排模块。
  支持分支路由、失败回退、重试与幂等扩展钩子。
  """

  alias HospitalScheduling.Scheduling.SchedulingPeriod

  def steps do
    [:s1_create, :s2_start_generating, :s3_mark_generated, :s4_mark_adjusted, :s5_publish]
  end
  ...
  defp next_candidates(step) do
    case step do
      :s1_create -> [:s2_start_generating, :update, :destroy]
      :s2_start_generating -> [:s3_mark_generated]
      :s3_mark_generated -> [:s4_mark_adjusted, :s5_publish, :s2_start_generating]
      :s4_mark_adjusted -> [:s5_publish, :s2_start_generating]
      :s5_publish -> [:s2_start_generating]
      _ -> []
    end
  end
  ...
  defp branch_next(step, _record) do
    case step do
      :s1_create -> nil
      :s2_start_generating -> nil
      :s3_mark_generated -> nil
      :s4_mark_adjusted -> nil
      :s5_publish -> nil
      _ -> nil
    end
  end
  ...
  defp step_skipped?(step, _record) do
    case step do
      :s1_create -> false
      :s2_start_generating -> false
      :s3_mark_generated -> false
      :s4_mark_adjusted -> false
      :s5_publish -> false
      _ -> false
    end
  end
  ...
  defp retry_policy(step) do
    case step do
      ...
      :s2_start_generating -> %{max_attempts: 1, backoff_ms: 0}
      :s3_mark_generated -> %{max_attempts: 1, backoff_ms: 0}
      :s4_mark_adjusted -> %{max_attempts: 1, backoff_ms: 0}
      :s5_publish -> %{max_attempts: 1, backoff_ms: 0}
