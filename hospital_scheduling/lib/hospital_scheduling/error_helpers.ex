defmodule HospitalScheduling.ErrorHelpers do
  @moduledoc """
  ...
  ## 用法
      ...
      # 翻译单条错误消息
      translated = HospitalScheduling.ErrorHelpers.translate_error(error)

      # 翻译 Ash changeset 返回的所有错误
      translated_errors = HospitalScheduling.ErrorHelpers.translate_errors(errors)

      # 在 Phoenix ErrorView / ErrorJSON 中使用
      def render("error.json", %{changeset: changeset}) do
        errors = Ash.Error.to_error_list(changeset)
        %{errors: HospitalScheduling.ErrorHelpers.translate_errors(errors)}
      end
  """

  @doc "翻译单条错误消息字符串"
  def translate_error(%{message: message, vars: vars}) do
    translated = Gettext.dgettext(HospitalScheduling.Gettext, "errors", message)
  ...
  def translate_error(%{message: message}) do
    Gettext.dgettext(HospitalScheduling.Gettext, "errors", message)
  end
