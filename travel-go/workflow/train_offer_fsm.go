...
// TrainOfferTrainOfferLifecycleFSM 管理 TrainOffer 实体的 TrainOfferLifecycle 状态转换
// 火车票 offer 生命周期
// ```mermaid
// stateDiagram-v2
//   [*] --> create
...
//   destroy --> [*]
// ```
type TrainOfferTrainOfferLifecycleFSM struct {
	FSM *fsm.FSM
}

// NewTrainOfferTrainOfferLifecycleFSM 创建新的 TrainOfferLifecycle 状态机实例
func NewTrainOfferTrainOfferLifecycleFSM(initialState string) *TrainOfferTrainOfferLifecycleFSM {
	o := &TrainOfferTrainOfferLifecycleFSM{}
	o.FSM = fsm.NewFSM(
		initialState,
		fsm.Events{
...
// DefaultInitialState 返回 TrainOfferLifecycle 的默认初始状态
func TrainOfferTrainOfferLifecycleFSMDefaultInitialState() string {
	return "create"
}

// TrainOfferTrainOfferLifecycleFSMWorkflowSemanticsJSON 返回编排语义快照（供跨端回放比对）
func TrainOfferTrainOfferLifecycleFSMWorkflowSemanticsJSON() string {
